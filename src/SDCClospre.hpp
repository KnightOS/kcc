...
struct assignment_lospre {
  boost::tuple<float, float>
      s; // First entry: Calculation costs, second entry: Lifetime costs.
  lospreset_t local;
...
bool assignments_lospre_locally_same(const assignment_lospre &a1,
                                     const assignment_lospre &a2) {
...
typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::bidirectionalS,
                              cfg_lospre_node, float>
    cfg_lospre_t; // The edge property is the cost of subdividing the edge and
                  // inserting an instruction (for now we always use 1,
                  // optimizing for code size, but relative execution frequency
                  // could be used when optimizing for speed or total energy
                  // consumption; aggregates thereof can be a good idea as
                  // well).
typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::bidirectionalS,
                              tree_dec_lospre_node>
    tree_dec_t;
...
void tree_dec_lospre_forget(
  ...
  for (ai = alist.begin(); ai != alist.end(); ++ai) {
    ...
    {
      typedef typename boost::graph_traits<cfg_lospre_t>::out_edge_iterator
          n_iter_t;
      n_iter_t n, n_end;
      for (boost::tie(n, n_end) = boost::out_edges(i, G); n != n_end; ++n) {
        if (ai->local.find(boost::target(*n, G)) == ai->local.end() ||
            (ai->global[i] && !G[i].invalidates) >=
                (ai->global[boost::target(*n, G)] ||
                 G[boost::target(*n, G)].uses))
          continue;
    ...
    {
      typedef
          typename boost::graph_traits<cfg_lospre_t>::in_edge_iterator n_iter_t;
      n_iter_t n, n_end;
      for (boost::tie(n, n_end) = boost::in_edges(i, G); n != n_end; ++n) {
        if (ai->local.find(boost::source(*n, G)) == ai->local.end() ||
            (ai->global[boost::source(*n, G)] &&
             !G[boost::source(*n, G)].invalidates) >=
                (ai->global[i] || G[i].uses))
          continue;
  ...
  for (ai = alist.begin(); ai != alist.end();) {
    ...
    for (++ai;
         ai != alist.end() && assignments_lospre_locally_same(*aif, *ai);) {
      if (aif->s > ai->s) {
        alist.erase(aif);
        aif = ai;
        ++ai;
...
void tree_dec_safety_forget(
  ...
  for (ai = alist.begin(); ai != alist.end();) {
    ...
    // Since we want the union of all paths between invalidating nodes without
    // uses, by definition there may not be a use in it.
    if (G[i].uses) {
      ai = alist.erase(ai);
      continue;
    }
    ...
    // At least one successor needs to be in the path or invalid.
    {
      ...
      for (ok = false, boost::tie(n, n_end) = boost::out_edges(i, G);
           !ok && n != n_end; ++n)
        if (ai->global[boost::target(*n, G)] ||
            G[boost::target(*n, G)].invalidates &&
                !G[boost::target(*n, G)].uses)
          ok = true;
    ...
    // At least one predecessor needs to be in the path or invalid.
    {
      ...
      for (ok = false, boost::tie(n, n_end) = boost::in_edges(i, G);
           !ok && n != n_end; ++n)
        if (ai->global[boost::source(*n, G)] ||
            G[boost::source(*n, G)].invalidates)
          ok = true;
...
split_edge(T_t &T, G_t &G, typename boost::graph_traits<G_t>::edge_descriptor e,
...
#ifdef DEBUG_LOSPRE
  std::cout << "Calculating " << OP_SYMBOL_CONST(tmpop)->name << " at ic "
            << newic->key << "\n";
#endif

  // Update tree-decomposition.
  // TODO: More efficiently.
  for (typename boost::graph_traits<T_t>::vertex_descriptor n1 = 0;
       n1 < boost::num_vertices(T); ++n1) {
    ...
    typename boost::graph_traits<T_t>::vertex_descriptor n2 =
        boost::add_vertex(T);
...
template <class G_t>
static void forward_lospre_assignment(
    G_t &G, typename boost::graph_traits<G_t>::vertex_descriptor i,
    const iCode *ic, const assignment_lospre &a) {
  typedef
  ...
  for (;;) {
    ...
    if (isOperandEqual(IC_RESULT(ic), IC_LEFT(nic)) &&
        IS_UNSIGNED(operandType(tmpop)) ==
            IS_UNSIGNED(operandType(IC_LEFT(nic))) &&
        nic->op != ADDRESS_OF && nic->op != PCALL &&
        (!POINTER_GET(nic) || !IS_PTR(operandType(IC_LEFT(nic))) ||
...
#ifdef DEBUG_LOSPRE
      std::cout << "Forward substituted left operand "
                << OP_SYMBOL_CONST(IC_LEFT(nic))->name << " at " << nic->key
                << "\n";
#endif
...
#ifdef DEBUG_LOSPRE
      std::cout << "Forward substituted right operand "
                << OP_SYMBOL_CONST(IC_RIGHT(nic))->name << " at " << nic->key
                << "\n";
#endif
...
#ifdef DEBUG_LOSPRE
      std::cout << "Forward substituted result operand "
                << OP_SYMBOL_CONST(IC_RESULT(nic))->name << " at " << nic->key
                << "\n";
#endif
    ...
    if (nic->op == LABEL) // Reached label. Continue only if all edges goining
                          // here are safe.
    {
      typedef
          typename boost::graph_traits<G_t>::in_edge_iterator in_edge_iter_t;
    ...
    if (isOperandEqual(IC_RESULT(ic), IC_RESULT(nic)) &&
        !POINTER_SET(nic) /*|| G[i].uses*/)
      break;
    ...
    if (nic->op == GOTO || nic->op == IFX || nic->op == JUMPTABLE) {
      adjacency_iter_t c, c_end;
      for (boost::tie(c, c_end) = boost::adjacent_vertices(i, G); c != c_end;
           ++c) {
        if (!(a.global[i] && !G[i].invalidates) &&
            a.global[*c]) // Calculation edge
          continue;
...
template <class T_t, class G_t>
static int implement_lospre_assignment(
    assignment_lospre a, T_t &T, G_t &G,
    const iCode *ic) // Assignment has to be passed as a copy (not reference),
                     // since the transformations on the tree-decomposition will
                     // invalidate it otherwise.
{
  ...
  std::set<edge_desc_t>
      calculation_edges; // Use descriptor, not iterator due to possible
                         // invalidation of iterators when inserting vertices or
                         // edges.
  edge_iter_t e, e_end;
  for (boost::tie(e, e_end) = boost::edges(G); e != e_end; ++e)
    if (!((a.global[boost::source(*e, G)] & true) &&
          !G[boost::source(*e, G)].invalidates) &&
        (a.global[boost::target(*e, G)] & true))
  ...
  for (typename std::set<edge_desc_t>::iterator i = calculation_edges.begin();
       i != calculation_edges.end(); ++i) {
    typename boost::graph_traits<G_t>::vertex_descriptor n =
  ...
  for (boost::tie(v, v_end) = boost::vertices(G); v != v_end; ++v) {
    ...
    if (a.global.size() <= *v)
      continue;
    if (!((a.global[*v] & true) && !G[*v].invalidates ||
          boost::source(*e, G) < a.global.size() &&
              (a.global[boost::source(*e, G)] & true)))
      continue;
#ifdef DEBUG_LOSPRE
    ...
    {
      typedef typename boost::graph_traits<G_t>::adjacency_iterator
          adjacency_iter_t;
      adjacency_iter_t c, c_end;
  ...
  if (substituted <= 0) {
    std::cerr << "Introduced " << OP_SYMBOL_CONST(tmpop)->name
              << ", but did not substitute any calculations.\n";
    return (-1);
  }

  if (substituted < split) // Todo: Remove this warning when optimization for
                           // speed instead of code size is implemented!
    std::cout << "Introduced " << OP_SYMBOL_CONST(tmpop)->name
              << ", but did substitute only " << substituted
              << " calculations, while introducing " << split << ".\n";
  std::cout.flush();
...
/*template <class T_t, class G_t>*/
static int tree_dec_lospre(tree_dec_t /*T_t*/ &T, cfg_lospre_t /*G_t*/ &G,
                           const iCode *ic) {
  if (tree_dec_lospre_nodes(T, find_root(T), G))
    return (-1);

  wassert(T[find_root(T)].assignments.begin() !=
          T[find_root(T)].assignments.end());
...
/* Using a template here confuses debugging tools such as valgrind. */
/*template <class T_t, class G_t>*/
static int tree_dec_safety(tree_dec_t /*T_t*/ &T, cfg_lospre_t /*G_t*/ &G,
                           const iCode *ic) {
