...
#define IS_DIR_SEPARATOR(c)                                                    \
  ((c) == DIR_SEPARATOR_CHAR || (c) == UNIX_DIR_SEPARATOR_CHAR)
/* Note that IS_ABSOLUTE_PATH accepts d:foo as well, although it is
   only semi-absolute.  This is because the users of IS_ABSOLUTE_PATH
   want to know whether to prepend the current working directory to
   a file name, which should not be done with a name like d:foo.  */
#define IS_ABSOLUTE_PATH(f)                                                    \
  (IS_DIR_SEPARATOR((f)[0]) || (((f)[0]) && ((f)[1] == ':')))
...
#define STACK_FULL(stack)                                                      \
  ((p_##stack) >= (stack + sizeof(stack) / sizeof(*stack) - 1))

#define STACK_PUSH_(stack, x) (*++p_##stack = (x))
#define STACK_POP_(stack) (*p_##stack--)

#define STACK_PUSH(stack, x)                                                   \
  (STACK_FULL(stack) ? (STACK_ERR(1, stack), *p_##stack)                       \
                     : STACK_PUSH_(stack, x))

#define STACK_POP(stack)                                                       \
  (STACK_EMPTY(stack) ? (STACK_ERR(-1, stack), *stack) : STACK_POP_(stack))

#define STACK_PEEK(stack)                                                      \
  (STACK_EMPTY(stack) ? (STACK_ERR(0, stack), *stack) : *p_##stack)

#define STACK_ERR(o, stack)                                                    \
  (fatal(1, E_STACK_VIOLATION, #stack,                                         \
         (o < 0) ? "underflow" : (o > 0) ? "overflow" : "empty"))
...
struct options {
  ...
  int out_fmt;      /* 0 = undefined, 'i' = intel Hex format, 's' = motorola S19
                       format, 'E' = elf format, 'Z' = gb format */
  ...
  int protect_sp_update; /* DS390 - will disable interrupts during ESP:SP
                            updates */
  int parms_in_bank1;    /* DS390 - use reg bank1 to pass parameters */
  int stack_size;   /* MCS51/DS390 - Tells the linker to allocate this space for
                       stack */
  int no_pack_iram; /* MCS51/DS390 - Deprecated: Tells the linker not to pack
  ...
  int no_ret_without_call; /* MCS51 - Do not use ret independent of acall/lcall
                            */
  int use_non_free; /* Search / include non-free licensed libraries and header
                       files */
  ...
  int printSearchDirs; /* display the directories in the compiler's search path
                        */
  int vc_err_style; /* errors and warnings are compatible with Micro$oft visual
                       studio */
  ...
  /*  set *olaysSet;               * not implemented yet: overlay segments used
   * in #pragma OVERLAY */
  int max_allocs_per_node; /* Maximum number of allocations / combinations
                              considered at each node in the tree-decomposition
                              based algorithms */
...
extern const char
    *moduleName; /* module name is source file without path and extension */
                 /* can be NULL while linking without compiling */
