...
bool convertIListToConstList(initList *src, literalList **lList, int size) {
  ...
  while (iLoop) {
    ...
    if (!IS_AST_LIT_VALUE(
            decorateType(resolveSymbols(iLoop->init.node), RESULT_TYPE_NONE))) {
      return FALSE;
    }
    iLoop = iLoop->next;
    cnt++;
  ...
  /* We've now established that the initializer list contains only literal
   * values. */
...
/*------------------------------------------------------------------*/
value *list2val(initList *val, int check) {
  if (!val)
    return NULL;
...
/*-----------------------------------------------------------------*/
designation *newDesignation(int type, void *designator) {
...
/*------------------------------------------------------------------*/
designation *revDesignation(designation *val) {
...
/*------------------------------------------------------------------*/
designation *copyDesignation(designation *src) {
...
static void moveNestedInit(initList *deepParent, initList *src) {
  ...
  if (deepParent->type != INIT_DEEP) {
    werrorfl(deepParent->filename, deepParent->lineno, E_INIT_STRUCT,
             "<unknown>");
    return;
  }
...
/*-----------------------------------------------------------------*/
static int findStructField(symbol *fields, symbol *target) {
  int i;
...
initList *reorderIlist(sym_link *type, initList *ilist) {
  ...
  if (IS_ARRAY(type) && IS_CHAR(type->next) && ilist &&
      ilist->type == INIT_NODE) {
  ...
  for (idx = 0; idx < size; idx++) {
    ...
    } while (sflds && IS_BITFIELD(sflds->type) && SPEC_BUNNAMED(sflds->etype));
  }
...
unsigned long double2ul(double val) {
   ...
   * type, the behavior is undefined. This shows up on Mac OS X i386 platform
   * which useus SSE unit instead of the x87 FPU for floating-point operations
   */
  /*
   * on Mac OS X ppc (long) 2147483648.0 equals to 2147483647, so we explicitely
   * convert it to 0x80000000 on other known platforms (long) 2147483648.0
   * equals to -2147483648
   */
  return ((val) < 0)
             ? (((val) < -2147483647.0) ? 0x80000000UL
...
/*--------------------------------------------------------------------*/
CCR_RESULT
checkConstantRange(sym_link *var, sym_link *lit, int op,
  ...
  signExtMask = varBits >= sizeof(TYPE_TARGET_ULONGLONG) * 8
                    ? 0
                    : ~((1ull << varBits) - 1);
  ...
  if (SPEC_USIGN(reType)) {
    ...
    TYPE_TARGET_ULONGLONG opBitsMask =
        reBits >= sizeof(opBitsMask) * 8 ? ~0ull : ((1ull << reBits) - 1);
...
/*-----------------------------------------------------------------*/
value *constFloatVal(const char *s) {
  value *val = newValue();
  double sval;
  char *p;
...
/*-----------------------------------------------------------------*/
value *constFixed16x16Val(const char *s) {
  value *val = newValue();
  double sval;
  char *p;
...
/*-----------------------------------------------------------------*/
value *constIntVal(const char *s) {
  char *p, *p2;
  double dval;
  long long int llval;
  value *val = newValue();
  ...
  if (u_suffix) // Choose first of unsigned int, unsigned long int, unsigned
                // long long int that fits.
  {
    SPEC_USIGN(val->type) = 1;
    ...
    if (decimal) // Choose first of int, long int, long long int that fits.
    {
      if (ll_suffix || dval > 0x7fffffff || dval < -0x80000000ll) {
        ...
        SPEC_LONG(val->type) = 1;
    } else // Choose first of int, unsigned int, long int, unsigned long int,
           // long long int, unsigned long long int that fits.
    {
...
value *constCharacterVal(unsigned long v, char type) {
  ...
  case 0: // character constant
    ...
    SPEC_CVAL(val->type).v_int =
        options.signed_char ? (signed char)v : (unsigned char)v;
    break;
...
/*-----------------------------------------------------------------*/
value *constCharVal(unsigned char v) { return constCharacterVal(v, 0); }
...
static const TYPE_UDWORD *
utf_32_from_utf_8(size_t *utf_32_len, const char *utf_8, size_t utf_8_len) {
  size_t allocated = 0;
  ...
  for (*utf_32_len = 0; utf_8_len; (*utf_32_len)++) {
    ...
    wassert(seqlen <= 6); // seqlen 5 and 6 are deprecated in current unicode
                          // standard, but for now, allow them.
...
// TODO: Move this function to SDCCutil?
static const TYPE_UWORD *utf_16_from_utf_32(size_t *utf_16_len,
                                            const TYPE_UDWORD *utf_32,
                                            size_t utf_32_len) {
  size_t allocated = 0;
  TYPE_UWORD *utf_16 = 0;
  TYPE_UDWORD codepoint;
  ...
  for (*utf_16_len = 0; utf_32_len; utf_32_len--, utf_32++) {
    ...
    if (codepoint < 0xd7ff ||
        codepoint >= 0xe000 &&
            codepoint <= 0xffff) // Code in basic multilingual plane.
    {
...
value *strVal(const char *s) {
  ...
  // Convert input string (mixed UTF-8 and UTF-32) to UTF-8 first (handling all
  // escape sequences, etc).
  utf_8 = copyStr(s[0] == '"' ? s : s + 1, &utf_8_size);

  if (s[0] == '"') // UTF-8 string literal (any prefix u8 or L in the source
                   // would already have been stripped by earlier stages)
  {
    ...
    size_t utf_32_size;
    // Convert to UTF-32 next, since converting UTF-32 to UTF-16 is easier than
    // UTF-8 to UTF-16.
    const TYPE_UDWORD *utf_32 =
        utf_32_from_utf_8(&utf_32_size, utf_8, utf_8_size);
    ...
    {
      size_t utf_16_size;
      const TYPE_UWORD *utf_16 =
          utf_16_from_utf_32(&utf_16_size, utf_32, utf_32_size);
...
/*------------------------------------------------------------------*/
value *rawStrVal(const char *s, size_t size) {
  struct dbuf_s dbuf;
  value *val = newValue();
...
value *charVal(const char *s) {
  ...
  } else if (type) // Wide character constant
  {
    ...
    free((void *)ustr);
    return (val);
  } else // Character constant that is not wide - compability with legacy
         // encodings.
...
/*------------------------------------------------------------------*/
unsigned long ulFromVal(const value *val) {
  if (!val)
    return 0;
...
/*------------------------------------------------------------------*/
unsigned char byteOfVal(value *val, int offset) {
  unsigned char *p;
  int shift = 8 * offset;
  ...
  if (SPEC_NOUN(val->etype) == V_FIXED16X16)
    return offset < 4 ? (SPEC_CVAL(val->etype).v_fixed16x16 >> shift) & 0xff
                      : 0;

  if (SPEC_LONGLONG(val->etype)) {
    if (SPEC_USIGN(val->etype))
      return offset < 8 ? (SPEC_CVAL(val->etype).v_ulonglong >> shift) & 0xff
                        : 0;
    else
...
ullFromLit(sym_link *lit) {
  ...
  if (SPEC_NOUN(etype) == V_FIXED16X16)
    return double2ul(doubleFromFixed16x16(
        SPEC_CVAL(etype).v_fixed16x16)); /* FIXME: this loses bits */
...
/*------------------------------------------------------------------*/
unsigned long long ullFromVal(value *val) {
  if (!val)
    return 0;
...
/*------------------------------------------------------------------*/
int csdOfVal(int *topbit, int *nonzero, unsigned long long *csd_add,
             unsigned long long *csd_sub, value *val) {
...
/*------------------------------------------------------------------*/
int isEqualVal(value *val, int k) {
  if (IS_SPEC(val->type)) {
...
value *valUnaryPM(value *val) {
  ...
  if (SPEC_NOUN(val->etype) == V_FLOAT)
    ...
    SPEC_CVAL(val->etype).v_fixed16x16 =
        (TYPE_TARGET_ULONG) - ((long)SPEC_CVAL(val->etype).v_fixed16x16);
...
value *valMult(value *lval, value *rval) {
  ...
  val = newValue();
  val->type = val->etype =
      computeType(lval->etype, rval->etype, RESULT_TYPE_INT, '*');
  ...
  if (IS_FLOAT(val->type))
    ...
    SPEC_CVAL(val->type).v_fixed16x16 =
        fixed16x16FromDouble(floatFromVal(lval) * floatFromVal(rval));
  /* signed and unsigned mul are the same, as long as the precision of the
    ...
    SPEC_CVAL(val->type).v_ulonglong = (TYPE_TARGET_ULONGLONG)ullFromVal(lval) *
                                       (TYPE_TARGET_ULONGLONG)ullFromVal(rval);
  else if (SPEC_LONG(val->type))
  ...
  else if (SPEC_USIGN(val->type)) /* unsigned int */
  {
    TYPE_TARGET_ULONG ul =
        (TYPE_TARGET_UINT)ulFromVal(lval) * (TYPE_TARGET_UINT)ulFromVal(rval);
...
value *valDiv(value *lval, value *rval) {
  ...
  if (isEqualVal(rval, 0) &&
      !IS_FLOAT(computeType(lval->etype, rval->etype, RESULT_TYPE_INT, '/'))) {
  ...
  if (IS_FLOAT(val->type))
    ...
    if (SPEC_USIGN(val->type))
      SPEC_CVAL(val->type).v_ulonglong =
          (TYPE_TARGET_ULONGLONG)ullFromVal(lval) /
          (TYPE_TARGET_ULONGLONG)ullFromVal(rval);
    else
      SPEC_CVAL(val->type).v_longlong = (TYPE_TARGET_LONGLONG)ullFromVal(lval) /
                                        (TYPE_TARGET_LONGLONG)ullFromVal(rval);
  } else if (SPEC_LONG(val->type)) {
    if (SPEC_USIGN(val->type))
      SPEC_CVAL(val->type).v_ulong = (TYPE_TARGET_ULONG)ulFromVal(lval) /
                                     (TYPE_TARGET_ULONG)ulFromVal(rval);
    else
      SPEC_CVAL(val->type).v_long =
          (TYPE_TARGET_LONG)ulFromVal(lval) / (TYPE_TARGET_LONG)ulFromVal(rval);
  } else {
    if (SPEC_USIGN(val->type))
...
value *valShift(value *lval, value *rval, int lr) {
  ...
  if (SPEC_LONGLONG(val->type)) {
    if (SPEC_USIGN(val->type)) {
      SPEC_CVAL(val->type).v_ulonglong =
          lr ? (TYPE_TARGET_ULONGLONG)ullFromVal(lval)
                   << (TYPE_TARGET_ULONGLONG)ullFromVal(rval)
             : (TYPE_TARGET_ULONGLONG)ullFromVal(lval) >>
                   (TYPE_TARGET_ULONGLONG)ullFromVal(rval);
    } else {
      SPEC_CVAL(val->type).v_longlong =
          lr ? (TYPE_TARGET_LONGLONG)ullFromVal(lval)
                   << (TYPE_TARGET_ULONGLONG)ullFromVal(rval)
             : (TYPE_TARGET_LONGLONG)ullFromVal(lval) >>
                   (TYPE_TARGET_ULONGLONG)ullFromVal(rval);
    }
  } else if (SPEC_LONG(val->type)) {
    if (SPEC_USIGN(val->type)) {
      SPEC_CVAL(val->type).v_ulong =
          lr ? (TYPE_TARGET_ULONG)ulFromVal(lval)
                   << (TYPE_TARGET_ULONG)ulFromVal(rval)
             : (TYPE_TARGET_ULONG)ulFromVal(lval) >>
                   (TYPE_TARGET_ULONG)ulFromVal(rval);
    } else {
      SPEC_CVAL(val->type).v_long =
          lr ? (TYPE_TARGET_LONG)ulFromVal(lval)
                   << (TYPE_TARGET_ULONG)ulFromVal(rval)
             : (TYPE_TARGET_LONG)ulFromVal(lval) >>
                   (TYPE_TARGET_ULONG)ulFromVal(rval);
    }
  } else {
    if (SPEC_USIGN(val->type)) {
      SPEC_CVAL(val->type).v_uint =
          lr ? (TYPE_TARGET_UINT)ulFromVal(lval)
                   << (TYPE_TARGET_ULONG)ulFromVal(rval)
             : (TYPE_TARGET_UINT)ulFromVal(lval) >>
                   (TYPE_TARGET_ULONG)ulFromVal(rval);
    } else {
      SPEC_CVAL(val->type).v_int =
          lr ? (TYPE_TARGET_INT)ulFromVal(lval)
                   << (TYPE_TARGET_ULONG)ulFromVal(rval)
             : (TYPE_TARGET_INT)ulFromVal(lval) >>
                   (TYPE_TARGET_ULONG)ulFromVal(rval);
    }
  }
  return cheapestVal(val);
...
value *valCompare(value *lval, value *rval, int ctype) {
  ...
  case EQ_OP:
    if (SPEC_NOUN(lval->type) == V_FLOAT || SPEC_NOUN(rval->type) == V_FLOAT) {
      SPEC_CVAL(val->type).v_int = floatFromVal(lval) == floatFromVal(rval);
    } else if (SPEC_NOUN(lval->type) == V_FIXED16X16 ||
...
value *valRecastLitVal(sym_link *dtype, value *val) {
  ...
  if (IS_SPEC(otype) &&
      (SPEC_NOUN(otype) == V_FIXED16X16 || SPEC_NOUN(otype) == V_FLOAT)) {
...
int getNelements(sym_link *type, initList *ilist) {
  ...
  if (IS_ARRAY(type) &&
      (IS_CHAR(type->next) || IS_INT(type->next) && IS_UNSIGNED(type->next)) &&
      !ilist->next) {
    ast *iast = ilist->init.node;
    value *v = (iast->type == EX_VALUE ? iast->opval.val : NULL);

    if (v && IS_ARRAY(v->type) &&
        (IS_CHAR(v->etype) || IS_INT(v->etype) && IS_UNSIGNED(v->etype) &&
                                  IS_LONG(type->next) == IS_LONG(v->etype)))
    /* yep, it's a string */
    {
  ...
  if (IS_AST_OP(arrExpr->left)) {
    if (arrExpr->left->opval.op == '[')
      ...
      if (IS_ADDRESS_OF_OP(arrExpr->left->left))
        lval =
            valForStructElem(arrExpr->left->left->left, arrExpr->left->right);
      else if (IS_AST_VALUE(arrExpr->left->left) &&
               IS_PTR(arrExpr->left->left->ftype))
        lval = valForStructElem(arrExpr->left->left, arrExpr->left->right);
    } else
      return NULL;
...
value *valForStructElem(ast *structT, ast *elemT) {
  ...
  if (IS_AST_OP(structT)) {
    if (structT->opval.op == '[')
      ...
      if (IS_AST_LIT_VALUE(structT->right) && !IS_AST_OP(structT->left)) {
        idxoff = (int)(AST_ULONG_VALUE(structT->right) *
                       getSize(structT->left->ftype->next));
        sast = structT->left;
      } else if (IS_AST_LIT_VALUE(structT->left) &&
                 !IS_AST_OP(structT->right)) {
        idxoff = (int)(AST_ULONG_VALUE(structT->left) *
                       getSize(structT->right->ftype->next));
        sast = structT->right;
      } else
        return NULL;
    } else if (structT->opval.op == '-') {
      if (IS_AST_LIT_VALUE(structT->right) && !IS_AST_OP(structT->left)) {
        idxoff = 0 - (int)(AST_ULONG_VALUE(structT->right) *
                           getSize(structT->left->ftype->next));
        sast = structT->left;
      } else
        return NULL;
  ...
  if (IS_AST_LIT_VALUE(structT) && IS_PTR(structT->ftype)) {
    ...
    if (sast)
      SNPRINTF(val->name, sizeof(val->name), "(%s + (%d))",
               AST_SYMBOL(sast)->rname, ((int)sym->offset) + idxoff);
    else
      SNPRINTF(val->name, sizeof(val->name), "(%s + %d)",
...
value *valForCastAggr(ast *aexpr, sym_link *type, ast *cnst, int op) {
  ...
  SNPRINTF(val->name, sizeof(val->name), "(%s %c %d)", AST_SYMBOL(aexpr)->rname,
           op, getSize(type->next) * AST_ULONG_VALUE(cnst));
...
/*                 with no constant                                */
/*-----------------------------------------------------------------*/
value *valForCastArr(ast *aexpr, sym_link *type) {
  value *val;
