...
void copyAstValues(ast *dest, ast *src) {
  ...
  case SWITCH:
    dest->values.switchVals.swVals =
        copyValueChain(src->values.switchVals.swVals);
...
bool hasSEFcalls(ast *tree) {
  ...
  if (tree->type == EX_OP &&
      (tree->opval.op == CALL || tree->opval.op == PCALL ||
       tree->opval.op == '=' || tree->opval.op == INC_OP ||
...
/*-----------------------------------------------------------------*/
static int isAstEqual(ast *t1, ast *t2) {
  if (!t1 && !t2)
    return 1;
...
ast *resolveSymbols(ast *tree) {
  ...
  if (tree->type == EX_OP && tree->opval.op == FOR) {
    ...
    if (AST_FOR(tree, trueLabel)) {
      if ((csym = findSym(LabelTab, AST_FOR(tree, trueLabel),
                          AST_FOR(tree, trueLabel)->name)))
        AST_FOR(tree, trueLabel) = csym;
    }
    if (AST_FOR(tree, falseLabel)) {
      if ((csym = findSym(LabelTab, AST_FOR(tree, falseLabel),
                          AST_FOR(tree, falseLabel)->name)))
        AST_FOR(tree, falseLabel) = csym;
    }
    if (AST_FOR(tree, continueLabel)) {
      if ((csym = findSym(LabelTab, AST_FOR(tree, continueLabel),
                          AST_FOR(tree, continueLabel)->name)))
        AST_FOR(tree, continueLabel) = csym;
    }
    if (AST_FOR(tree, condLabel)) {
      if ((csym = findSym(LabelTab, AST_FOR(tree, condLabel),
                          AST_FOR(tree, condLabel)->name)))
  ...
  if (IS_AST_VALUE(tree) && tree->opval.val->sym &&
    ...
    symbol *csym =
        findSym(LabelTab, tree->opval.val->sym, tree->opval.val->sym->name);
  ...
  if (IS_AST_VALUE(tree) && tree->opval.val->sym &&
    ...
    if (csym && tree->opval.val->sym != csym) {
      ...
      if (*csym->rname) {
        SNPRINTF(tree->opval.val->name, sizeof(tree->opval.val->name), "%s",
                 csym->rname);
      } else {
        SNPRINTF(tree->opval.val->name, sizeof(tree->opval.val->name), "_%s",
                 csym->name);
      }
    }
    ...
    if (!csym && !tree->opval.val->sym->implicit && !tree->opval.val->type) {
      ...
      if (tree->funcName) {
        ...
        tree->opval.val->sym->type->next = tree->opval.val->sym->etype =
            newIntLink();
...
/*-----------------------------------------------------------------*/
symbol *funcOfType(const char *name, sym_link *type, sym_link *argType,
                   int nArgs, int rent) {
  symbol *sym;
  /* create the symbol */
...
/*-----------------------------------------------------------------*/
symbol *funcOfTypeVarg(const char *name, const char *rtype, int nArgs,
                       const char **atypes) {
  symbol *sym;
  int i;
  /* create the symbol */
...
/*-----------------------------------------------------------------*/
static void reverseParms(ast *ptree, int r) {
  ast *ttree;
  if (!ptree)
    return;
...
/*-----------------------------------------------------------------*/
static int processParms(ast *func, value *defParm, ast **actParm,
  ...
  if (IS_AST_PARAM(*actParm)) {
    ...
    if ((*actParm)->reversed) {
      return (
          processParms(func, defParm, &(*actParm)->right, parmNumber, FALSE) ||
          processParms(func, defParm ? defParm->next : NULL, &(*actParm)->left,
                       parmNumber, rightmost));
    } else {
      return (
          processParms(func, defParm, &(*actParm)->left, parmNumber, FALSE) ||
          processParms(func, defParm ? defParm->next : NULL, &(*actParm)->right,
                       parmNumber, rightmost));
    }
  ...
  /* decorate parameter */
  resultType =
      defParm ? getResultTypeFromType(defParm->type) : RESULT_TYPE_NONE;
  *actParm = decorateType(*actParm, resultType);
  ...
  if (!defParm && *actParm && IFFUNC_HASVARARGS(functype)) {
    ...
    /* don't perform integer promotion of explicitly typecasted variable
     * arguments if sdcc extensions are enabled */
    if (options.std_sdcc && (IS_CAST_OP(*actParm) ||
                             (IS_AST_SYM_VALUE(*actParm) &&
                              AST_VALUES(*actParm, cast.removedCast)) ||
                             (IS_AST_LIT_VALUE(*actParm) &&
                              AST_VALUES(*actParm, cast.literalFromCast)))) {
  ...
  /* reentrant */
  if (!defParm && *actParm && options.stackAuto) {
    return 0;
  }
  ...
  if ((IS_ARRAY((*actParm)->ftype) && IS_PTR(defParm->type)) ||
    ...
    *actParm = newNode(CAST, newAst_LINK(defParm->type), pTree);
    ...
    (*actParm)->lineno = (*actParm)->right->lineno;
    AST_VALUES(*actParm, cast.implicitCast) = 1;
    *actParm = decorateType(
        *actParm, IS_GENPTR(defParm->type) ? RESULT_TYPE_GPTR : resultType);
  }

  /* make a copy and change the regparm type to the defined parm */
  (*actParm)->etype =
      getSpec((*actParm)->ftype = copyLinkChain((*actParm)->ftype));
...
/*-----------------------------------------------------------------*/
static ast *createIvalType(ast *sym, sym_link *type, initList *ilist) {
  ast *iExpr;
...
/*------------------------------------------------------------------*/
static initList *moveNestedInit(initList *src) {
  initList *dst;
...
/*-----------------------------------------------------------------*/
static symbol *findStructField(symbol *fields, symbol *target) {
...
/*-----------------------------------------------------------------*/
static ast *createIvalStruct(ast *sym, sym_link *type, initList *ilist,
                             ast *rootValue) {
  ast *rast = NULL;
  ast *lAst;
  ...
  for (sflds = SPEC_STRUCT(type)->fields;; sflds = sflds->next) {
    ...
    if (aggregateIsAutoVar && (SPEC_STRUCT(type)->type != UNION))
      for (ps = old_sflds; ps != sflds && ps != NULL; ps = ps->next) {
        ps->implicit = 1;
        lAst = newNode(PTR_OP, newNode('&', sym, NULL),
                       newAst_VALUE(symbolVal(ps)));
        lAst = decorateType(resolveSymbols(lAst), RESULT_TYPE_NONE);
        rast = decorateType(resolveSymbols(createIval(lAst, ps->type, NULL,
                                                      rast, rootValue, 1)),
                            RESULT_TYPE_NONE);
      }
    ...
    sflds->implicit = 1;
    lAst = newNode(PTR_OP, newNode('&', sym, NULL),
                   newAst_VALUE(symbolVal(sflds)));
    lAst = decorateType(resolveSymbols(lAst), RESULT_TYPE_NONE);
    rast = decorateType(resolveSymbols(createIval(lAst, sflds->type, iloop,
                                                  rast, rootValue, 1)),
                        RESULT_TYPE_NONE);
...
/*-----------------------------------------------------------------*/
static ast *createIvalArray(ast *sym, sym_link *type, initList *ilist,
                            ast *rootValue) {
  ast *rast = NULL;
  initList *iloop;
  ...
  /* char *p = {"abc"}; */
  if ((IS_CHAR(type->next) || IS_INT(type->next) && IS_UNSIGNED(type->next)) &&
      ilist && ilist->type == INIT_DEEP && ilist->init.deep &&
      ilist->init.deep->type == INIT_NODE)
    if ((rast = createIvalCharPtr(
             sym, type,
             decorateType(resolveSymbols(list2expr(ilist->init.deep)),
                          RESULT_TYPE_NONE),
             rootValue)))
      return decorateType(resolveSymbols(rast), RESULT_TYPE_NONE);
  ...
  if (!iloop && (!lcnt || !DCL_ELEM(type) || !AST_SYMBOL(rootValue)->islocal ||
                 SPEC_STAT(etype))) {
    return NULL;
  }

  if (port->arrayInitializerSuppported &&
      convertIListToConstList(reorderIlist(type, ilist), &literalL, lcnt)) {
    ast *aSym;
    ...
    for (;;) {
      ast *aSym;
      if (!iloop && ((lcnt && size >= lcnt) || !DCL_ELEM(type) ||
                     !AST_SYMBOL(rootValue)->islocal)) {
        break;
      }
      ...
      if (iloop && (lcnt && size > lcnt)) {
        ...
        werrorfl(iloop->filename, iloop->lineno, W_EXCESS_INITIALIZERS, "array",
                 name);
...
/*-----------------------------------------------------------------*/
static ast *createIvalCharPtr(ast *sym, sym_link *type, ast *iexpr,
                              ast *rootVal) {
  ast *rast = NULL;
  unsigned size = 0;
  ...
  if (IS_PTR(type) &&
      ((IS_LITERAL(iexpr->etype) ||
        (IS_SPEC(iexpr->etype) && SPEC_SCLS(iexpr->etype) == S_CODE)) &&
  ...
  if (!iexpr) {
    ...
    for (i = 0; i < symsize; ++i) {
      rast = newNode(
          NULLOP, rast,
          newNode('=', newNode('[', sym, newAst_VALUE(valueFromLit((float)i))),
      ...
      if (size > symsize) {
        ...
        if (options.std_c99 && c == '\0' && size == symsize + 1) {
          if (!options.lessPedantic)
            werrorfl(iexpr->filename, iexpr->lineno,
                     W_STRING_CANNOT_BE_TERMINATED, name);
        } else
...
/*-----------------------------------------------------------------*/
static ast *createIvalPtr(ast *sym, sym_link *type, initList *ilist,
                          ast *rootVal) {
  ast *rast;
  ast *iexpr;
...
/*-----------------------------------------------------------------*/
static ast *createIval(ast *sym, sym_link *type, initList *ilist, ast *wid,
                       ast *rootValue, int omitStatic) {
  ast *rast = NULL;

  if (!ilist && (!AST_SYMBOL(rootValue)->islocal ||
                 (omitStatic && SPEC_STAT(getSpec(type)))))
    return NULL;
...
/*                  values                                         */
/*-----------------------------------------------------------------*/
static ast *gatherAutoInit(symbol *autoChain) {
  ast *init = NULL;
  ast *work;
  symbol *sym;
  ...
  for (sym = autoChain; sym; sym = sym->next) {
    ...
    if (IS_STATIC(sym->etype) && sym->ival && SPEC_SCLS(sym->etype) != S_CODE) {
      ...
      if (IS_AGGREGATE(sym->type)) {
        ...
        work =
            newNode('=', newAst_VALUE(symbolVal(newSym)), list2expr(sym->ival));
      }
...
value *stringToSymbol(value *val) {
  ...
  for (sp = statsg->syms; sp; sp = sp->next) {
    ...
    if (sym->isstrlit && size == getSize(val->type) &&
        !memcmp(SPEC_CVAL(sym->etype).v_char, SPEC_CVAL(val->etype).v_char,
                size)) {
...
value *constExprValue(ast *cexpr, int check) {
  ...
  if (!IS_LITERAL(cexpr->ftype)) {
    /* then check if this is a literal array
       in code segment */
    if (SPEC_SCLS(cexpr->etype) == S_CODE && SPEC_CVAL(cexpr->etype).v_char &&
    ...
    if (IS_CAST_OP(cexpr) && IS_LITERAL(cexpr->right->ftype)) {
      return valCastLiteral(
          cexpr->ftype, floatFromVal(cexpr->right->opval.val),
          (TYPE_TARGET_ULONGLONG)ullFromVal(cexpr->right->opval.val));
    }
    ...
    if (check)
      werrorfl(cexpr->filename, cexpr->lineno, E_CONST_EXPECTED,
               "found expression");
...
/*-----------------------------------------------------------------*/
static bool isLoopCountable(ast *initExpr, ast *condExpr, ast *loopExpr,
  ...
  if (IS_AST_OP(condExpr)) {
    ...
    case '!':
      if (IS_AST_OP(condExpr->left) && condExpr->left->opval.op == '>' &&
  ...
  if (loopExpr->opval.op == INC_OP) {
    ...
    /* check for += */
    if (loopExpr->opval.op ==
        ADD_ASSIGN) /* seems to never happen, createRMW() absorbed */
    {
...
/*-----------------------------------------------------------------*/
static bool astHasDeref(ast *tree) {
...
/*-----------------------------------------------------------------*/
static bool isLoopReversible(ast *loop, symbol **loopCntrl, ast **init,
                             ast **end) {
  ...
  /* for loop has to be of the form
     ...
     [<sym>++] | [<sym> += 1] | [<sym> = <sym> + 1] )
     forBody */
  if (!isLoopCountable(AST_FOR(loop, initExpr), AST_FOR(loop, condExpr),
                       AST_FOR(loop, loopExpr), loopCntrl, init, end))
    return 0;
...
ast *reverseLoop(ast *loop, symbol *sym, ast *init, ast *end) {
  ...
  rloop = newNode(
      NULLOP,
      newNode('=', newAst_VALUE(symbolVal(sym)), newNode('-', end, init)),
      createLabel(
          AST_FOR(loop, continueLabel),
          newNode(NULLOP, loop->left,
                  newNode(NULLOP,
                          newNode(SUB_ASSIGN, newAst_VALUE(symbolVal(sym)),
...
/*-----------------------------------------------------------------*/
static int replLoopSymByVal(ast *body, symbol *sym, value *val) {
  int changed;
  /* reached end */
  if (!body || IS_AST_LINK(body))
    return 0;
...
/*-----------------------------------------------------------------*/
static bool isInitiallyTrue(ast *initExpr, ast *condExpr) {
  symbol *sym;
  ast *init;
...
/*-----------------------------------------------------------------*/
static ast *searchLitOp(ast *tree, ast **parent, const char *ops) {
  ast *ret;

  if (tree && optimize.global_cse) {
    /* is there a literal operand? */
    if (tree->right && IS_AST_OP(tree->right) && tree->right->right &&
        (tree->right->opval.op == (unsigned)ops[0] ||
         tree->right->opval.op == (unsigned)ops[1])) {
    ...
    if (tree->left && IS_AST_OP(tree->left) && tree->left->right &&
        (tree->left->opval.op == (unsigned)ops[0] ||
         tree->left->opval.op == (unsigned)ops[1])) {
...
const char *getResultTypeName(RESULT_TYPE resultType) {
  switch (resultType) {
...
/*-----------------------------------------------------------------*/
static ast *addCast(ast *tree, RESULT_TYPE resultType, bool promote) {
  sym_link *newLink;
  bool upCasted = FALSE;
...
/*-----------------------------------------------------------------*/
static RESULT_TYPE resultTypePropagate(ast *tree, RESULT_TYPE resultType) {
  ...
  case '+':
  case '-':
    if ((IS_AST_VALUE(tree->left) &&
         !IS_INTEGRAL(tree->left->opval.val->etype)) ||
        (IS_AST_VALUE(tree->right) &&
         !IS_INTEGRAL(tree->right->opval.val->etype)))
      return RESULT_TYPE_NONE;
    return resultType;
  case '&':
    if (!tree->right)
...
/*-----------------------------------------------------------------*/
static RESULT_TYPE getLeftResultType(ast *tree, RESULT_TYPE resultType) {
  switch (tree->opval.op) {
  case '=':
  case CAST:
  ...
  case '[':
    ...
    if (DCL_ELEM(LTYPE(tree)) > 0 && getSize(tree->left->ftype) < 128)
      return RESULT_TYPE_CHAR; // TODO: Instead of doing this optimization here,
                               // do it later on the iCode (where it probably
                               // could use a more efficient unsigned char).
    return resultType;
  default:
    return resultType;
  }
...
void gatherImplicitVariables(ast *tree, ast *block) {
  ...
  if (tree->type == EX_VALUE && !(IS_LITERAL(tree->opval.val->etype)) &&
      tree->opval.val->type == NULL && tree->opval.val->sym &&
...
void checkPtrCast(sym_link *newType, sym_link *orgType, bool implicit,
  ...
  {
    if (!IS_PTR(orgType) && !IS_FUNC(orgType) &&
        !IS_AGGREGATE(orgType)) // from a non pointer
    {
      if (IS_INTEGRAL(orgType)) {
        // maybe this is NULL, then it's ok.
        if (!(IS_LITERAL(orgType) && (SPEC_CVAL(orgType).v_ulong == 0))) {
          if (GPTRSIZE > FARPTRSIZE && IS_GENPTR(newType) &&
              !IS_FUNCPTR(newType)) {
        ...
        if (!IS_VOID(getSpec(newType)) &&
            !(IS_CODEPTR(newType) && IS_FUNC(newType->next) &&
              IS_FUNC(orgType))) {
    ...
    {
      if (implicit &&
          getAddrspace(newType->next) != getAddrspace(orgType->next)) {
        errors += werror(E_INCOMPAT_PTYPES);
      } else if (IS_GENPTR(newType) &&
                 IS_VOID(newType->next)) // cast to void* is always allowed
      {
        if (IS_FUNCPTR(orgType))
          errors += werror(FUNCPTRSIZE > GPTRSIZE ? E_INCOMPAT_PTYPES
                                                  : W_INCOMPAT_PTYPES);
      } else if (IS_GENPTR(orgType) &&
                 IS_VOID(orgType->next)) // cast from void* is always allowed -
                                         // as long as we cast to a pointer to
                                         // an object type
      {
        if (IS_FUNCPTR(newType) &&
            !orgIsNullPtrConstant) // cast to pointer to function is only
                                   // allowed for null pointer constants
          errors += werror(W_INCOMPAT_PTYPES);
      } else if (GPTRSIZE >
                 FARPTRSIZE /*!TARGET_IS_Z80 && !TARGET_IS_GBZ80 */) {
        // if not a pointer to a function
        if (!(IS_CODEPTR(newType) && IS_FUNC(newType->next) &&
              IS_FUNC(orgType))) {
          if (implicit) // if not to generic, they have to match
          {
            if (!IS_GENPTR(newType) &&
                !((DCL_TYPE(orgType) == DCL_TYPE(newType)) ||
                  ((DCL_TYPE(orgType) == POINTER) &&
...
/*-----------------------------*/
static void checkZero(value *val) {
  if (!val)
    return;
...
/*--------------------------------------------------------*/
static bool isAstInAst(ast *tree, ast *search) {
  if (tree == search)
    return TRUE;
  if (!tree)
    return FALSE;
...
/*---------------------------------------*/
static ast *copyAstNode(ast *tree) {
  ast *newtree;
...
/*-------------------------------------------------*/
static void rewriteAstGatherSideEffects(ast *tree, set **sideEffects) {
  if (!tree)
    return;
...
/*-------------------------------------------------------------------*/
static void rewriteAstJoinSideEffects(ast *tree, ast *oLeft, ast *oRight) {
  ...
  for (sefTree = setFirstItem(sideEffects); sefTree;
       sefTree = setNextItem(sideEffects)) {
...
static void optStdLibCall(ast *tree, RESULT_TYPE resulttype) {
  ast *parms = tree->right;
  ast *func = tree->left;

  if (!IS_FUNC(func->ftype) || IS_LITERAL(func->ftype) ||
      func->type != EX_VALUE || !func->opval.val->sym)
    return;
  ...
  ast *parm;
  for (parm = parms; parm && parm->type == EX_OP && parm->opval.op == PARAM;
       parm = parm->right)
    if (parm->left)
      nparms++;
  if (parm)
  ...
  if (!strcmp(funcname, "printf") && nparms == 1 &&
      resulttype == RESULT_TYPE_NONE) {
    ast *parm = parms;
    ...
    if (parm->type != EX_VALUE || !IS_ARRAY(parm->opval.val->type) ||
        !parm->opval.val->sym)
      return;
    ...
    if (strsym->isstrlit != 1 || !strlink || !IS_SPEC(strlink) ||
        SPEC_NOUN(strlink) != V_CHAR)
      return;
    ...
    // but in a way that works better with the reuse of string symbols
    {
      ...
      parm->opval.val =
          stringToSymbol(rawStrVal(dbuf_get_buf(&dbuf), strlength - 1));
      dbuf_destroy(&dbuf);
  ...
  // Optimize strcpy() to memcpy().
  else if ((!strcmp(funcname, "strcpy") ||
            !strcmp(funcname, "__builtin_strcpy")) &&
           nparms == 2) {
    ...
    if (!strsym->isstrlit || !strlink || !IS_SPEC(strlink) ||
        SPEC_NOUN(strlink) != V_CHAR)
      return;
    ...
    // TODO:Check for other targets when memcpy() is a better choice than
    // strcpy;
    minlength = SIZE_MAX;
    ...
    symbol *memcpy_sym = findSym(
        SymbolTab, NULL,
        !strcmp(funcname, "__builtin_strcpy") ? "__builtin_memcpy" : "memcpy");

    if (!memcpy_sym)
      return;

    ast *lengthparm =
        newAst_VALUE(valCastLiteral(newIntLink(), strlength, strlength));
    decorateType(lengthparm, RESULT_TYPE_NONE);
    ast *node = newAst_OP(PARAM);

    node->left = newNode(
        CAST, newAst_LINK(copyLinkChain(FUNC_ARGS(memcpy_sym->type)->type)),
        parm);
...
/*--------------------------------------------------------------*/
static void rewriteAstNodeOp(ast *tree, int op, ast *left, ast *right) {
...
/* side-effects with a comma operator                       */
/*----------------------------------------------------------*/
static void rewriteAstNodeVal(ast *tree, value *val) {
  ast *oLeft = (tree->type == EX_OP) ? tree->left : NULL;
  ast *oRight = (tree->type == EX_OP) ? tree->right : NULL;
...
ast *decorateType(ast *tree, RESULT_TYPE resultType) {
  ...
  if (tree->isError || (tree->left && tree->left->isError) ||
  ...
  if (tree->type == EX_VALUE) {
    ...
    if (tree->opval.val->sym) {
      ...
      if (tree->opval.val->sym->undefined) {
        werrorfl(tree->filename, tree->lineno, E_ID_UNDEF,
                 tree->opval.val->sym->name);
        /* assume int */
        TTYPE(tree) = TETYPE(tree) = tree->opval.val->type =
            tree->opval.val->sym->type = tree->opval.val->etype =
                tree->opval.val->sym->etype = copyLinkChain(INTTYPE);
  ...
  case '[':
    ...
    if (IS_LITERAL(RTYPE(tree))) {
      ...
      if (arraySize && arrayIndex >= arraySize) {
        werrorfl(tree->filename, tree->lineno, W_IDX_OUT_OF_BOUNDS, arrayIndex,
                 arraySize);
      }
    }
    ...
    if (IS_PTR(LTYPE(
            tree)) /* && !IS_LITERAL (TETYPE (tree)) caused bug #2850 */) {
  ...
  case '.':
    ...
    TTYPE(tree) = structElemType(
        LTYPE(tree),
        (tree->right->type == EX_VALUE ? tree->right->opval.val : NULL));
    TETYPE(tree) = getSpec(TTYPE(tree));
    return tree;
  ...
  case '&': /* can be unary */
    ...
    if (tree->right) /* not a unary operation */
    {
      ...
      if (!IS_INTEGRAL(LTYPE(tree)) || !IS_INTEGRAL(RTYPE(tree))) {
        ...
        werrorfl(tree->filename, tree->lineno, W_CONTINUE,
      ...
      if (IS_LITERAL(RTYPE(tree)) && IS_BOOLEAN(LTYPE(tree))) {
        ...
        tree->right =
            decorateType(newAst_VALUE(constBoolVal(litval & 1)), resultType);
      }
      ...
      if (IS_LITERAL(RTYPE(tree)) &&
          getSize(RTYPE(tree)) == getSize(LTYPE(tree)))
        SPEC_USIGN(RTYPE(tree)) = SPEC_USIGN(LTYPE(tree));

      LRVAL(tree) = RRVAL(tree) = 1;

      TTYPE(tree) =
          computeType(LTYPE(tree), RTYPE(tree), resultType, tree->opval.op);
      TETYPE(tree) = getSpec(TTYPE(tree));
    ...
    if (IS_BITFIELD(tree->left->etype)) {
      werrorfl(tree->filename, tree->lineno, E_ILLEGAL_ADDR,
               "address of bit variable");
      goto errorTreeReturn;
    }
  ...
  case '|':
    ...
    if (IS_LITERAL(RTYPE(tree)) && IS_BOOLEAN(LTYPE(tree)) &&
        IS_INTEGRAL(RTYPE(tree)) && resultType == RESULT_TYPE_BOOL) {
      unsigned long litval = AST_ULONG_VALUE(tree->right);
      tree->right =
          decorateType(newAst_VALUE(constBoolVal(litval != 0)), resultType);
    }
  ...
  case '^':
    ...
    if (IS_LITERAL(RTYPE(tree)) && IS_LITERAL(LTYPE(tree))) {
      rewriteAstNodeVal(tree,
                        valBitwise(valFromType(LETYPE(tree)),
                                   valFromType(RETYPE(tree)), tree->opval.op));
      return decorateType(tree, resultType);
    }
  ...
  case '/':
    ...
    if (IS_LITERAL(RTYPE(tree)) && IS_LITERAL(LTYPE(tree))) {
      rewriteAstNodeVal(
          tree, valDiv(valFromType(LETYPE(tree)), valFromType(RETYPE(tree))));
      return decorateType(tree, resultType);
    }
    ...
    TETYPE(tree) =
        getSpec(TTYPE(tree) = computeType(LTYPE(tree), RTYPE(tree), resultType,
                                          tree->opval.op));
  ...
  case '%':
    ...
    if (IS_LITERAL(RTYPE(tree)) && IS_LITERAL(LTYPE(tree))) {
      rewriteAstNodeVal(
          tree, valMod(valFromType(LETYPE(tree)), valFromType(RETYPE(tree))));
      return decorateType(tree, resultType);
    }
    LRVAL(tree) = RRVAL(tree) = 1;
    TETYPE(tree) =
  ...
  case '*': /* can be unary  : if right is null then unary operation */
    if (!tree->right) {
      ...
      TETYPE(tree) = getSpec(TTYPE(tree));
      /* adjust the storage class */
      if (DCL_TYPE(tree->left->ftype) != ARRAY &&
          DCL_TYPE(tree->left->ftype) != FUNCTION)
    ...
    if (IS_LITERAL(RTYPE(tree)) && IS_LITERAL(LTYPE(tree))) {
      rewriteAstNodeVal(
          tree, valMult(valFromType(LETYPE(tree)), valFromType(RETYPE(tree))));
      return decorateType(tree, resultType);
    }
  ...
  case '+':
    ...
    if (IS_LITERAL(RTYPE(tree)) && IS_LITERAL(LTYPE(tree))) {
      ...
      rewriteAstNodeVal(
          tree, valPlus(valFromType(LETYPE(tree)), valFromType(RETYPE(tree))));
      return decorateType(tree, resultType);
    }
    ...
    if (IS_PTR(LTYPE(tree)) || IS_AGGREGATE(LTYPE(tree)))
      ...
      tree->right = addCast(tree->right, resultTypeProp, TRUE);
      TETYPE(tree) =
          getSpec(TTYPE(tree) = computeType(LTYPE(tree), RTYPE(tree),
  ...
  case '-': /* can be unary   */
    ...
    if (!tree->right) {
      ...
      tree->left = addCast(tree->left, resultTypeProp, TRUE);
      TETYPE(tree) =
          getSpec(TTYPE(tree) = computeType(LTYPE(tree), NULL, resultType,
                                            tree->opval.op));
      LRVAL(tree) = 1;
      return tree;
    }
    ...
    if (!(IS_PTR(RTYPE(tree)) || IS_ARRAY(RTYPE(tree)) ||
    ...
    if (IS_LITERAL(RTYPE(tree)) && IS_LITERAL(LTYPE(tree))) {
      ...
      tree->right = addCast(tree->right, resultTypeProp, TRUE);
      rewriteAstNodeVal(
          tree, valMinus(valFromType(LETYPE(tree)), valFromType(RETYPE(tree))));
      return decorateType(tree, resultType);
    }
  ...
  case '~':
    ...
    if (resultType == RESULT_TYPE_BOOL && IS_UNSIGNED(tree->left->etype) &&
  ...
  case '!':
    /* can be pointer */
    if (!IS_ARITHMETIC(LTYPE(tree)) && !IS_PTR(LTYPE(tree)) &&
        !IS_FUNC(LTYPE(tree)) && !IS_ARRAY(LTYPE(tree))) {
      werrorfl(tree->filename, tree->lineno, E_UNARY_OP, tree->opval.op);
      goto errorTreeReturn;
    }
    ...
    TTYPE(tree) = TETYPE(tree) =
        (resultTypeProp == RESULT_TYPE_BOOL) ? newBoolLink() : newCharLink();
  ...
  case RIGHT_OP:
    ...
    /* rewrite the tree */
    if (IS_LITERAL(RTYPE(tree)) && IS_LITERAL(LTYPE(tree))) {
      rewriteAstNodeVal(tree, valShift(valFromType(LETYPE(tree)),
                                       valFromType(RETYPE(tree)),
    ...
    if (IS_LITERAL(RTYPE(tree)) &&
      ...
      werrorfl(tree->filename, tree->lineno, W_SHIFT_NEGATIVE,
  ...
  case CAST: /* change the type   */
    ...
    if (compareTypeExact(LTYPE(tree), RTYPE(tree), -1) == 1) {
       ...
       * for proper processing (no integer promotion) of explicitly typecasted
       * variable arguments */
    ...
    if (IS_LITERAL(RETYPE(tree))) {
      ...
      if (!IS_PTR(LTYPE(tree))) {
        ...
        tree->opval.val = valCastLiteral(
            LTYPE(tree), floatFromVal(valFromType(RTYPE(tree))),
            (TYPE_TARGET_ULONGLONG)ullFromVal(valFromType(RTYPE(tree))));
        TTYPE(tree) = tree->opval.val->type;
        tree->left = NULL;
        tree->right = NULL;
        tree->values.cast.literalFromCast = 1;
        ...
        checkPtrCast(LTYPE(tree), RTYPE(tree), tree->values.cast.implicitCast,
                     !ullFromVal(valFromType(RTYPE(tree))));
        LRVAL(tree) = 1;
        tree->type = EX_VALUE;
        tree->opval.val =
            valCastLiteral(LTYPE(tree), gpVal | pVal, gpVal | pVal);
        TTYPE(tree) = tree->opval.val->type;
        tree->left = NULL;
        tree->right = NULL;
        tree->values.cast.literalFromCast = 1;
  ...
  case OR_OP:
    ...
    /* rewrite the tree */
    if (IS_LITERAL(RTYPE(tree)) && IS_LITERAL(LTYPE(tree))) {
      rewriteAstNodeVal(tree, valLogicAndOr(valFromType(LETYPE(tree)),
                                            valFromType(RETYPE(tree)),
                                            tree->opval.op));
      return decorateType(tree, resultType);
    }
  ...
  case NE_OP: {
    ...
    /* if they are pointers they must be castable */
      ...
      if (tree->opval.op == EQ_OP && !IS_GENPTR(LTYPE(tree)) &&
          IS_GENPTR(RTYPE(tree))) {
    ...
    /* else they should be promotable to one another */
      ...
      if (!((IS_PTR(LTYPE(tree)) && IS_LITERAL(RTYPE(tree))) ||
            ...
            ((tree->opval.op == EQ_OP || tree->opval.op == NE_OP) &&
             ((IS_FUNC(LTYPE(tree)) && IS_LITERAL(RTYPE(tree)) &&
               !ullFromVal(valFromType(RTYPE(tree)))) ||
              (IS_FUNC(RTYPE(tree)) && IS_LITERAL(LTYPE(tree)) &&
               !ullFromVal(valFromType(LTYPE(tree))))))))
...

    {
      ...
      if (IS_INTEGRAL(LTYPE(tree)) && !IS_LITERAL(LTYPE(tree)) &&
          IS_LITERAL(RTYPE(tree)))
        ccr_result =
            checkConstantRange(LTYPE(tree), RTYPE(tree), tree->opval.op, FALSE);
      if (ccr_result == CCR_OK && IS_INTEGRAL(RTYPE(tree)) &&
          !IS_LITERAL(RTYPE(tree)) && IS_LITERAL(LTYPE(tree)))
        ccr_result =
            checkConstantRange(RTYPE(tree), LTYPE(tree), tree->opval.op, TRUE);
      switch (ccr_result) {
      ...
      case CCR_ALWAYS_FALSE:
        ...
        newResult = newAst_VALUE(
            constBoolVal((unsigned char)(ccr_result == CCR_ALWAYS_TRUE)));
    ...
    if (tree->opval.op == '>' && SPEC_USIGN(LETYPE(tree)) &&
        IS_LITERAL(RTYPE(tree)) &&
      ...
      /* (unsigned value) ? 1 : 0 */ /* TODO: Casts to _Bools tend to result in
                                        far more efficient code than '?' */
      tree->opval.op = '?';
      tree->right =
          newNode(':', newAst_VALUE(constBoolVal(1)), tree->right); /* val 0 */
      tree->right->filename = tree->filename;
      tree->right->lineno = tree->lineno;
    ...
    if (IS_LITERAL(RETYPE(tree)) &&
        floatFromVal(valFromType(RTYPE(tree))) == 1 &&
        IS_BOOLEAN(LETYPE(tree)) && tree->opval.op == EQ_OP &&
        (resultType == RESULT_TYPE_IFX || resultType == RESULT_TYPE_BOOL)) {
      tree = tree->left;
      return decorateType(tree, resultType);
    }
    ...
    if (IS_LITERAL(RETYPE(tree)) && IS_LITERAL(LETYPE(tree))) {
      rewriteAstNodeVal(tree,
                        valCompare(valFromType(LETYPE(tree)),
                                   valFromType(RETYPE(tree)), tree->opval.op));
      return decorateType(tree, resultType);
    }
    ...
    if (IS_CHAR(RTYPE(tree)) && IS_CHAR(LTYPE(tree)) &&
      ...
      /* Small literal integers are 'optimized' to 'unsigned char' but chars in
         single quotes are 'char'. Try to figure out, if it's possible to do
         without a cast to integer */
    ...
    LRVAL(tree) = RRVAL(tree) = 1;
    TTYPE(tree) = TETYPE(tree) =
        (resultType == RESULT_TYPE_BOOL) ? newBoolLink() : newCharLink();
  ...
  case '?':
    ...
    if (IS_LITERAL(LTYPE(tree))) {
      ...
      if (IS_LITERAL(TETYPE(heir)))
        TTYPE(heir) =
            valRecastLitVal(TTYPE(tree->right), valFromType(TETYPE(heir)))
                ->type;
      else
    ...
    if (IS_AST_LIT_VALUE(tree->right->left) &&
        IS_AST_LIT_VALUE(tree->right->right) &&
        ((resultType == RESULT_TYPE_IFX) || (resultType == RESULT_TYPE_BOOL))) {
      ...
      if (!assoc->left) {
        ...
        if (compareType(type, assoc->left->opval.lnk) > 0 &&
            !(SPEC_NOUN(type) == V_CHAR &&
              type->select.s.b_implicit_sign !=
                  assoc->left->opval.lnk->select.s.b_implicit_sign)) {
          if (found_expr) {
  ...
  case ':':
    ...
    if ((compareType(LTYPE(tree), RTYPE(tree)) == 0) &&
      ...
      werrorfl(tree->filename, tree->lineno, E_TYPE_MISMATCH,
               "conditional operator", " ");
  ...
  case '=':
    ...
    if (compareType(LTYPE(tree), RTYPE(tree)) == 0) {
      if (IS_CODEPTR(LTYPE(tree)) &&
          IS_FUNC(LTYPE(tree)->next)) /* function pointer */
      {
        werrorfl(tree->filename, tree->lineno, E_INCOMPAT_TYPES);
        printFromToType(RTYPE(tree), LTYPE(tree)->next);
      } else {
        int lineno = tree->lineno ? tree->lineno
                                  : tree->left->lineno ? tree->left->lineno
                                                       : tree->right->lineno;
    ...
    if (tree->initMode && SPEC_STAT(getSpec(LTYPE(tree))) &&
        !constExprTree(tree->right))
  ...
  case ',':
    ...
    /*----------------------------*/
  case CALL:
    if (IFFUNC_ISCRITICAL(LTYPE(tree)) &&
    ...
    if (!tree->right || !tree->right->decorated) {
      ...
      if (processParms(tree->left, FUNC_ARGS(functype), &tree->right,
                       &parmNumber, TRUE))
        goto errorTreeReturn;
      ...
      if (options.stackAuto && !IFFUNC_ISBUILTIN(functype))
  ...
  case RETURN:
    if (tree->right) {
      ...
      if (typecompat == -1) {
        tree->right =
            newNode(CAST, newAst_LINK(copyLinkChain(currFunc->type->next)),
                    tree->right);
        ...
        tree->right = decorateType(tree->right, IS_GENPTR(currFunc->type->next)
                                                    ? RESULT_TYPE_GPTR
                                                    : RESULT_TYPE_NONE);
      } else if (!typecompat) {
  ...
  case FOR:
       ...
       do */
    {
      ...
      else if (isInitiallyTrue(AST_FOR(tree, initExpr),
                               AST_FOR(tree, condExpr))) {
        tree =
            createDoFor(AST_FOR(tree, trueLabel), AST_FOR(tree, continueLabel),
                        AST_FOR(tree, falseLabel), AST_FOR(tree, condLabel),
                        AST_FOR(tree, initExpr), AST_FOR(tree, condExpr),
                        AST_FOR(tree, loopExpr), tree->left, tree->right);
        return decorateType(tree, RESULT_TYPE_NONE);
      } else {
        tree = createFor(AST_FOR(tree, trueLabel), AST_FOR(tree, continueLabel),
                         AST_FOR(tree, falseLabel), AST_FOR(tree, condLabel),
                         AST_FOR(tree, initExpr), AST_FOR(tree, condExpr),
                         AST_FOR(tree, loopExpr), tree->left, tree->right);
        return decorateType(tree, RESULT_TYPE_NONE);
...
/*-----------------------------------------------------------------*/
static ast *backPatchLabels(ast *tree, symbol *trueLabel, symbol *falseLabel) {
  if (!tree)
    return NULL;
...
ast *createCase(ast *swStat, ast *caseVal, ast *stmnt) {
  ...
  dbuf_init(&dbuf, 128);
  dbuf_printf(&dbuf, "_case_%d_%d", swStat->values.switchVals.swNum,
...
ast *createIf(ast *condAst, ast *ifBody, ast *elseBody) {
  ...
  if (elseBody) {
    ifBody = newNode(NULLOP, ifBody,
                     newNode(GOTO, newAst_VALUE(symbolVal(ifEnd)), NULL));
...
/*        _dobreak_n:                                              */
/*-----------------------------------------------------------------*/
ast *createDo(symbol *trueLabel, symbol *continueLabel, symbol *falseLabel,
              ast *condAst, ast *doBody) {
  ast *doTree;
...
ast *createFor(symbol *trueLabel, symbol *continueLabel, symbol *falseLabel,
  ...
  loopExpr = newNode(NULLOP, loopExpr,
                     newNode(GOTO, newAst_VALUE(symbolVal(condLabel)), NULL));
  if (continueLabelAst) {
    continueLabelAst->right = loopExpr;
    loopExpr = continueLabelAst;
  } else
...
/*      _while_break_n:                                            */
/*-----------------------------------------------------------------*/
ast *createWhile(symbol *trueLabel, symbol *continueLabel, symbol *falseLabel,
                 ast *condExpr, ast *whileBody) {
  ast *whileTree;
  ...
  whileBody = newNode(NULLOP, whileBody,
                      newNode(GOTO, newAst_VALUE(symbolVal(continueLabel)),
...
/*-----------------------------------------------------------------*/
static ast *isShiftRightLitVal_BitAndLitVal(ast *tree) {
...
/*-----------------------------------------------------------------*/
static int isBitAndPow2(ast *tree) {
  /* if this is not a bit and */
  if (!IS_BITAND(tree))
    return -1;
...
ast *optimizeGetHbit(ast *tree, RESULT_TYPE resultType) {
  ...
  if (expr) {
    if ((AST_ULONG_VALUE(tree->right) != 1) ||
        ((bit = AST_ULONG_VALUE(tree->left->right)) !=
...
/*-----------------------------------------------------------------*/
static ast *optimizeCompare(ast *root) {
  ast *optExpr = NULL;
  value *vleft;
  value *vright;
  ...
  if (vleft && vright && IS_BITVAR(vleft->etype) &&
      IN_BITSPACE(SPEC_OCLS(vleft->etype)) && IS_LITERAL(vright->etype)) {
...
/*------------------------------------------------------------------*/
static void fixupInlineLabel(symbol *sym) {
  struct dbuf_s dbuf;
...
/*------------------------------------------------------------------*/
static void copyAstLoc(ast *dest, ast *src) {
  dest->filename = src->filename;
  dest->lineno = src->lineno;
  dest->level = src->level;
...
/*-----------------------------------------------------------------*/
static void fixupInlineInDeclarators(struct initList *ival, long level) {
...
static void fixupInline(ast *tree, long level) {
  ...
  if (tree->type == EX_OP && tree->opval.op == RETURN) {
    ...
    if (inlineState.retsym && tree->right) {
      assignTree =
          newNode('=', newAst_VALUE(symbolVal(copySymbol(inlineState.retsym))),
                  tree->right);
...
/*                 is nonzero.                                     */
/*-----------------------------------------------------------------*/
static void inlineAddDecl(symbol *sym, ast *block, int addSymTab, int toFront) {
  sym->reqv = NULL;
  SYM_SPIL_LOC(sym) = NULL;
  sym->key = 0;
  if (block != NULL) {
...
/*-----------------------------------------------------------------*/
static symbol *inlineTempVar(sym_link *type, long level) {
  symbol *sym;
...
/*-----------------------------------------------------------------*/
static ast *inlineFindParm(ast *parms, int index) {
  if (!parms)
    return 0;
...
/*                                  inline functions               */
/*-----------------------------------------------------------------*/
static void expandInlineFuncsInDeclarators(struct initList *ival, ast *block) {
  for (; ival; ival = ival->next) {
    switch (ival->type) {
    case INIT_NODE:
...
static void expandInlineFuncs(ast *tree, ast *block) {
  if (IS_AST_OP(tree) && (tree->opval.op == CALL) && tree->left &&
    ...
    if (IFFUNC_ISINLINE(func->type) && !IFFUNC_HASVARARGS(func->type) &&
      ...
      retlab = newSymbol(genSymName(tree->level + LEVEL_UNIT),
                         tree->level + LEVEL_UNIT);
      retlab->isitmp = 1;
      retlab->islbl = 1;
      ...
      for (; args; args = args->next, argIndex++) {
        ...
        assigntree = newNode('=', newAst_VALUE(symbolVal(parm)),
      ...
      if (inlineFindParm(tree->right, argIndex) &&
          !IFFUNC_HASVARARGS(func->type))
        werror(E_TOO_MANY_PARMS);
...
/*------------------------------------------------------------*/
symbol *createFunctionDecl(symbol *name) {
  symbol *csym;
  value *args;
  sym_link *type;
...
ast *createFunction(symbol *name, ast *body) {
  ...
  allocParms(FUNC_ARGS(name->type),
             IFFUNC_ISSMALLC(name->type)); /* allocate the parameters */
  ...
  SNPRINTF(name->rname, sizeof(name->rname), "%s%s", port->fun_prefix,
           name->name);

  body = resolveSymbols(body); /* resolve the symbols */
  body = decorateType(
      body, RESULT_TYPE_NONE); /* propagateType & do semantic checks */
  ...
  if (staticAutos) {
    ...
    eBBlockFromiCode(iCodeFromAst(
        decorateType(resolveSymbols(staticAutos), RESULT_TYPE_NONE)));
    staticAutos = NULL;
  }
...
void ast_print(ast *tree, FILE *outfile, int indent) {
  ...
  if (tree->opval.op == FUNCTION) {
    ...
    fprintf(outfile, "FUNCTION (%s=%p) type (", tree->left->opval.val->name,
            tree);
  ...
  if (tree->opval.op == BLOCK) {
    ...
    fprintf(outfile, "{ L%ld:%ld B%d\n", tree->level / LEVEL_UNIT,
            tree->level % LEVEL_UNIT, tree->block);
    while (decls) {
      INDENT(indent + 2, outfile);
      fprintf(outfile, "DECLARE SYMBOL (L%ld:%ld B%d %s=%p) type (",
              decls->level / LEVEL_UNIT, decls->level % LEVEL_UNIT,
              decls->block, decls->name, decls);
  ...
  if (tree->type == EX_VALUE) {
    if (IS_LITERAL(tree->opval.val->etype)) {
      ...
      fprintf(outfile, "(L%ld:%ld B%d %s=%p @ %p)",
              tree->opval.val->sym->level / LEVEL_UNIT,
              tree->opval.val->sym->level % LEVEL_UNIT,
              tree->opval.val->sym->block, tree->opval.val->sym->name, tree,
              tree->opval.val->sym);
    }
    if (tree->ftype) {
  ...
  case SWITCH: {
    ...
    for (val = tree->values.switchVals.swVals; val; val = val->next) {
      ...
      fprintf(outfile, "CASE 0x%x GOTO _case_%d_%d%s\n", (int)ulFromVal(val),
              tree->values.switchVals.swNum, (int)ulFromVal(val),
              tree->values.switchVals.swSuffix
                  ? tree->values.switchVals.swSuffix
                  : "");
    }
    ast_print(tree->right, outfile, indent);
...
void PA(ast *t) { ast_print(t, stdout, 0); }
...
int astErrors(ast *t) {
  ...
  if (t) {
    ...
    if (t->type == EX_VALUE && t->opval.val->sym &&
...
static ast *offsetofOp_rec(sym_link *type, ast *snd, sym_link **result_type) {
  /* make sure the type is complete and sane */
  ...
  if (IS_ARRAY_OP(snd)) {
    ...
    *result_type = tmp->next;
    return newNode('+', o,
                   newNode('*', newAst_VALUE(valueFromLit(getSize(tmp->next))),
                           snd->right));
  }
