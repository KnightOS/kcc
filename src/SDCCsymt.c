...
/*-----------------------------------------------------------------*/
void addSym(bucket **stab, void *sym, char *sname, long level, int block,
            int checkType) {
...
/*-----------------------------------------------------------------*/
void *findSymWithBlock(bucket **stab, symbol *sym, int block, long level) {
  bucket *bp;
  ...
  while (bp) {
    if (strcmp(bp->name, sym->name) == 0 &&
        (bp->block == block || (bp->block < block && bp->level < level)))
      break;
    bp = bp->next;
  }
...
/*------------------------------------------------------------------*/
symbol *newSymbol(const char *name, long scope) {
  symbol *sym;
...
/*------------------------------------------------------------------*/
sym_link *newLink(SYM_LINK_CLASS select) {
  sym_link *p;
...
/*------------------------------------------------------------------*/
structdef *newStruct(const char *tag) {
  structdef *s;
...
void checkTypeSanity(sym_link *etype, const char *name) {
  ...
  if ((SPEC_NOUN(etype) == V_BOOL || SPEC_NOUN(etype) == V_CHAR ||
       SPEC_NOUN(etype) == V_FLOAT || SPEC_NOUN(etype) == V_FIXED16X16 ||
       SPEC_NOUN(etype) == V_DOUBLE || SPEC_NOUN(etype) == V_VOID) &&
      (SPEC_SHORT(etype) || SPEC_LONG(etype) ||
       SPEC_LONGLONG(etype))) { // long or short for char float double or void
    werror(E_LONG_OR_SHORT_INVALID, noun, name);
  }
  if ((SPEC_NOUN(etype) == V_BOOL || SPEC_NOUN(etype) == V_FLOAT ||
       SPEC_NOUN(etype) == V_FIXED16X16 || SPEC_NOUN(etype) == V_DOUBLE ||
       SPEC_NOUN(etype) == V_VOID) &&
      (etype->select.s.b_signed ||
       SPEC_USIGN(etype))) { // signed or unsigned for float double or void
  ...
  if (!SPEC_NOUN(etype)) {
    ...
    if (!(SPEC_SHORT(etype) || SPEC_LONG(etype) || SPEC_LONGLONG(etype) ||
          SPEC_SIGN(etype) || SPEC_USIGN(etype)))
  ...
  if (etype->select.s.b_signed && SPEC_USIGN(etype)) { // signed AND unsigned
    werror(E_SIGNED_AND_UNSIGNED_INVALID, noun, name);
  }
  if (SPEC_SHORT(etype) && SPEC_LONG(etype)) { // short AND long
...
/*------------------------------------------------------------------*/
sym_link *finalizeSpec(sym_link *lnk) {
  sym_link *p = lnk;
  while (p && !IS_SPEC(p))
    p = p->next;
...
/*------------------------------------------------------------------*/
sym_link *mergeSpec(sym_link *dest, sym_link *src, const char *name) {
  unsigned int i;
  ...
  if ((SPEC_SHORT(src) || SPEC_LONG(src) || SPEC_LONGLONG(src)) &&
    ...
    if (!(options.std_c99 && SPEC_LONG(src) &&
          SPEC_LONG(dest))) /* C99 has long long */
...
/*------------------------------------------------------------------*/
sym_link *mergeDeclSpec(sym_link *dest, sym_link *src, const char *name) {
...
unsigned int getSize(sym_link *p) {
  ...
  if (IS_SPEC(p)) {         /* if this is the specifier then */
    ...
    case V_INT:
      return (IS_LONGLONG(p) ? LONGLONGSIZE
                             : (IS_LONG(p) ? LONGSIZE : INTSIZE));
    case V_FLOAT:
      return FLOATSIZE;
    case V_FIXED16X16:
      return (4);
...
/*------------------------------------------------------------------*/
static int checkStructFlexArray(symbol *sym, sym_link *p) {
...
/*------------------------------------------------------------------*/
symbol *copySymbolChain(const symbol *src) {
  symbol *dest;
...
/*------------------------------------------------------------------*/
symbol *copySymbol(const symbol *src) {
  symbol *dest;
  ...
  for (sym = *symHead; sym != NULL; sym = sym->next) {
    ...
    if (!sym->level && !(IS_SPEC(sym->etype) && IS_TYPEDEF(sym->etype)))
      ...
      if (IS_ARRAY(sym->type) && !DCL_ELEM(sym->type))
        elemsFromIval = DCL_ELEM(sym->type) =
            getNelements(sym->type, sym->ival);
    }

    /* if already exists in the symbol table on the same level, ignoring
     * sublevels */
    if ((csym = findSymWithLevel(SymbolTab, sym)) &&
        csym->level / LEVEL_UNIT == sym->level / LEVEL_UNIT) {
...
/*------------------------------------------------------------------*/
sym_link *structElemType(sym_link *stype, value *id) {
  ...
  if (fields && id) {
    ...
    while (fields) {
      if (strcmp(fields->rname, id->name) == 0) {
        ...
        etype = getSpec(type);
        SPEC_SCLS(etype) =
            (SPEC_SCLS(petype) == S_REGISTER ? SPEC_SCLS(etype)
                                             : SPEC_SCLS(petype));
        SPEC_OCLS(etype) =
            (SPEC_SCLS(petype) == S_REGISTER ? SPEC_OCLS(etype)
                                             : SPEC_OCLS(petype));
  ...
  while (field) {
    ...
    if (!*field->name && IS_STRUCT(field->type)) {
      ...
      if (SPEC_STRUCT(field->type)->tagsym != NULL)
        werrorfl(field->fileDef, field->lineDef, E_ANONYMOUS_STRUCT_TAG,
                 SPEC_STRUCT(field->type)->tag);
      ...
      for (;;) {
        ...
        while (dupfield && dupfield != subfield) {
          if (*subfield->name && !strcmp(dupfield->name, subfield->name)) {
            werrorfl(subfield->fileDef, subfield->lineDef, E_DUPLICATE_MEMBER,
                     su == STRUCT ? "struct" : "union", subfield->name);
...
/*------------------------------------------------------------------*/
static void checkSClass(symbol *sym, int isProto) {
  sym_link *t;
  ...
  if (IS_ABSOLUTE(sym->etype) && SPEC_SCLS(sym->etype) == S_SFR) {
  ...
  if ((sym->level == 0 || SPEC_STAT(sym->etype)) &&
      SPEC_SCLS(sym->etype) == S_FIXED && !IS_FUNC(sym->type)) {
    /* find the first non-array link */
    t = sym->type;
    while (IS_ARRAY(t))
      t = t->next;
  ...
  if ((sym->level == 0 || SPEC_STAT(sym->etype)) &&
      SPEC_SCLS(sym->etype) == S_CODE && port->mem.code_ro) {
    /* find the first non-array link */
    t = sym->type;
    while (IS_ARRAY(t))
      t = t->next;
  ...
  if (IS_BITVAR(sym->etype) &&
      (SPEC_SCLS(sym->etype) != S_FIXED && SPEC_SCLS(sym->etype) != S_SBIT &&
  ...
  if (sym->level && (options.stackAuto)) {
    if (SPEC_SCLS(sym->etype) != S_BIT && SPEC_SCLS(sym->etype) != S_REGISTER) {
  ...
  if (sym->level && !IS_STATIC(sym->etype) && SPEC_ABSA(sym->etype) &&
      (options.stackAuto)) {
  ...
  if (sym->level && !IS_STATIC(sym->etype) &&
      (IS_DECL(sym->type) ? DCL_PTR_ADDRSPACE(sym->type)
                          : SPEC_ADDRSPACE(sym->type)) &&
      (options.stackAuto)) {
  ...
  if ((IS_ARRAY(sym->type) || IS_PTR(sym->type)) &&
      (SPEC_NOUN(sym->etype) == V_BIT || SPEC_NOUN(sym->etype) == V_SBIT ||
       ...
       SPEC_NOUN(sym->etype) == V_BBITFIELD ||
  ...
  if (!isProto) {
    ...
    if (SPEC_SCLS(sym->etype) == S_CODE && sym->ival == NULL && !sym->_isparm &&
        IS_AUTO(sym) && port->mem.code_ro && !SPEC_ABSA(sym->etype) &&
...
/*------------------------------------------------------------------*/
sym_link *copyLinkChain(const sym_link *p) {
  sym_link *head, *loop;
  const sym_link *curr;
...
/*------------------------------------------------------------------*/
static sym_link *computeTypeOr(sym_link *etype1, sym_link *etype2,
                               sym_link *reType) {
  /* sanity check */
...
/*------------------------------------------------------------------*/
sym_link *computeType(sym_link *type1, sym_link *type2, RESULT_TYPE resultType,
                      int op) {
  sym_link *rType;
  sym_link *reType;
  ...
  /* if both are bitvars choose the larger one */
    ...
    rType = SPEC_BLEN(etype1) >= SPEC_BLEN(etype2) ? copyLinkChain(type1)
                                                   : copyLinkChain(type2);
  ...
  if (!IS_FLOAT(reType) &&
      ((SPEC_USIGN(etype1)
       ...
       (SPEC_USIGN(etype2) && !(bitsForType(etype2) < bitsForType(reType)) &&
        !IS_CHAR(etype2)) || /* if both are 'unsigned char' and not promoted
                                let the result be unsigned too */
       (SPEC_USIGN(etype1) && SPEC_USIGN(etype2) && IS_CHAR(etype1) &&
        IS_CHAR(etype2) && IS_CHAR(reType))))
    SPEC_USIGN(reType) = 1;
  else
    SPEC_USIGN(reType) = 0;
...
int compareFuncType(sym_link *dest, sym_link *src) {
  ...
  if (FUNC_REGBANK(dest) !=
      FUNC_REGBANK(
          src)) { /* except for ISR's whose prototype need not match
  ...
  for (argCnt = 1; exargs && acargs;
    ...
    if (IFFUNC_ISREENT(dest) &&
        compareType(exargs->type, checkValue->type) <= 0) {
      return 0;
    }
    if (!IFFUNC_ISREENT(dest) &&
        compareTypeExact(exargs->type, checkValue->type, 1) <= 0) {
      return 0;
    }
  }
...
int comparePtrType(sym_link *dest, sym_link *src, bool bMustCast) {
  ...
  /* All function pointers can be cast (6.6 in the ISO C11 standard) TODO: What
   * about address spaces? */
  if (res == 0 && !bMustCast && IS_DECL(src) && IS_FUNC(src->next) &&
      IS_DECL(dest) && IS_FUNC(dest->next))
    return -1;
  else if (res == 1)
    return bMustCast ? -1 : 1;
  else if (res == -2)
...
int compareType(sym_link *dest, sym_link *src) {
  ...
  if (IS_DECL(dest)) {
    if (IS_DECL(src)) {
      ...
      if (IS_PTR(src) && (IS_GENPTR(dest) || ((DCL_TYPE(src) == POINTER) &&
                                              (DCL_TYPE(dest) == IPOINTER)))) {
  ...
  if (SPEC_NOUN(src) == V_BBITFIELD && SPEC_NOUN(dest) != V_BBITFIELD ||
  ...
  if (IS_BITFIELD(dest) && IS_BITFIELD(src) &&
      (SPEC_BLEN(dest) != SPEC_BLEN(src) || SPEC_BSTR(dest) != SPEC_BSTR(src)))
    return -1;

  /* it is a specifier */
  if (SPEC_NOUN(dest) != SPEC_NOUN(src)) {
    if ((SPEC_USIGN(dest) == SPEC_USIGN(src)) && IS_INTEGRAL(dest) &&
        IS_INTEGRAL(src) &&
        /* I would prefer
...
int compareTypeExact(sym_link *dest, sym_link *src, long level) {
    ...
    if (IS_DECL(src)) {
      if (DCL_TYPE(src) == DCL_TYPE(dest)) {
        ...
        if (IS_FUNC(src)) {
          ...
          /* for all the expected args do */
          for (; exargs && acargs;
               exargs = exargs->next, acargs = acargs->next) {
  ...
  if ((srcScls == S_FIXED || srcScls == S_AUTO) &&
      (port->mem.default_globl_map == xdata) && (destScls == S_XDATA) &&
      (level <= 0)) {
    srcScls = S_XDATA;
  }
...
/*------------------------------------------------------------------*/
static int calleeCmp(void *p1, void *p2) {
...
value *aggregateToPointer(value *val) {
  if (IS_AGGREGATE(val->type)) {
    ...
    case S_FIXED:
      ...
      } else { // this happens for (external) function parameters
...
int checkFunction(symbol *sym, symbol *csym) {
  ...
  for (argCnt = 1, acargs = FUNC_ARGS(sym->type); acargs;
       acargs = acargs->next, argCnt++) {
    if (!acargs->sym) {
      // this can happen for reentrant functions
      werrorfl(sym->fileDef, sym->lineDef, E_PARAM_NAME_OMITTED, sym->name,
               argCnt);
      // the show must go on: synthesize a name and symbol
      SNPRINTF(acargs->name, sizeof(acargs->name), "_%s_PARM_%d", sym->name,
               argCnt);
  ...
  if (compareType(csym->type, sym->type) <= 0) {
    werrorfl(sym->fileDef, sym->lineDef, E_PREV_DECL_CONFLICT, csym->name,
             "type", csym->fileDef, csym->lineDef);
  ...
  if (FUNC_ISISR(csym->type) != FUNC_ISISR(sym->type))
    werrorfl(sym->fileDef, sym->lineDef, E_PREV_DECL_CONFLICT, csym->name,
             "interrupt", csym->fileDef, csym->lineDef);
  ...
  if (SPEC_STAT(sym->etype) && SPEC_EXTR(sym->etype)) {
    werrorfl(sym->fileDef, sym->lineDef, E_TWO_OR_MORE_STORAGE_CLASSES,
             sym->name);
  }
  ...
  while (val) {
    ...
    if (val->sym && val->sym->name)
      for (value *val2 = val->next; val2; val2 = val2->next)
        if (val2->sym && val2->sym->name &&
            !strcmp(val->sym->name, val2->sym->name))
...
void PT(sym_link *type) { printTypeChain(type, 0); }
...
void dbuf_printTypeChain(sym_link *start, struct dbuf_s *dbuf) {
  ...
  while (type) {
    if (IS_DECL(type)) {
      ...
      case V_BBITFIELD:
        dbuf_printf(dbuf, "_Boolbitfield {%d,%d}", SPEC_BSTR(type),
                    SPEC_BLEN(type));
        break;
...
static const char *_mangleFunctionName(const char *in) {
...
/*            "Sc" -  signed char                                  */
/*-----------------------------------------------------------------*/
sym_link *typeFromStr(const char *s) {
...
void initCSupport(void) {
  const char *smuldivmod[] = {"mul", "div", "mod"};
  const char *sbwd[] = {
  ...
  const char *ssu[] = {"s", "su", "us", "u"};
  const char *srlrr[] = {"rl", "rr"};
  /* type as character codes for typeFromStr() */
  const char *sbwdCodes[] = {"c", "i", "l", "L", "Uc", "Ui", "Ul", "UL"};
  ...
  fps16x16_add = funcOfType("__fps16x16_add", fixed16x16Type, fixed16x16Type, 2,
                            options.float_rent);
  fps16x16_sub = funcOfType("__fps16x16_sub", fixed16x16Type, fixed16x16Type, 2,
                            options.float_rent);
  fps16x16_mul = funcOfType("__fps16x16_mul", fixed16x16Type, fixed16x16Type, 2,
                            options.float_rent);
  fps16x16_div = funcOfType("__fps16x16_div", fixed16x16Type, fixed16x16Type, 2,
                            options.float_rent);
  fps16x16_eq = funcOfType("__fps16x16_eq", charType, fixed16x16Type, 2,
                           options.float_rent);
  fps16x16_neq = funcOfType("__fps16x16_neq", charType, fixed16x16Type, 2,
                            options.float_rent);
  fps16x16_lt = funcOfType("__fps16x16_lt", charType, fixed16x16Type, 2,
                           options.float_rent);
  fps16x16_lteq = funcOfType("__fps16x16_lteq", charType, fixed16x16Type, 2,
                             options.float_rent);
  fps16x16_gt = funcOfType("__fps16x16_gt", charType, fixed16x16Type, 2,
                           options.float_rent);
  fps16x16_gteq = funcOfType("__fps16x16_gteq", charType, fixed16x16Type, 2,
                             options.float_rent);
    ...
    for (bwd = 0; bwd < 4; bwd++) {
      for (su = 0; su < 2; su++) {
        ...
        if (tofrom) {
          ...
          conv[tofrom][bwd][su] =
              funcOfType(dbuf_c_str(&dbuf), multypes[bwd][su], floatType, 1,
                         options.float_rent);
        } else {
          dbuf_printf(&dbuf, "__%s%s2fs", ssu[su * 3], sbwd[bwd]);
          conv[tofrom][bwd][su] =
              funcOfType(dbuf_c_str(&dbuf), floatType, multypes[bwd][su], 1,
                         options.float_rent);
        }
        dbuf_destroy(&dbuf);
      }
        ...
        if (tofrom) {
          ...
          if (bwd == 4)
            fp16x16conv[tofrom][bwd][su] =
                funcOfType(dbuf_c_str(&dbuf), floatType, fixed16x16Type, 1,
                           options.float_rent);
          else
            fp16x16conv[tofrom][bwd][su] =
                funcOfType(dbuf_c_str(&dbuf), multypes[bwd][su], fixed16x16Type,
                           1, options.float_rent);
        } else {
          dbuf_printf(&dbuf, "__%s%s2fps16x16", ssu[su * 3], fp16x16sbwd[bwd]);
          if (bwd == 4)
            fp16x16conv[tofrom][bwd][su] =
                funcOfType(dbuf_c_str(&dbuf), fixed16x16Type, floatType, 1,
                           options.float_rent);
          else
            fp16x16conv[tofrom][bwd][su] =
                funcOfType(dbuf_c_str(&dbuf), fixed16x16Type, multypes[bwd][su],
                           1, options.float_rent);
        }
        dbuf_destroy(&dbuf);
      }
            ...
            for (su = 0; su < 2; su++)
              {
                ...
                dbuf_init (&dbuf, 128);
                dbuf_printf (&dbuf, "_%s%s%s", smuldivmod[muldivmod], ssu[su*3],
    sbwd[bwd]); muldiv[muldivmod][bwd][su] = funcOfType
    (_mangleFunctionName(dbuf_c_str (&dbuf)), multypes[bwd][su],
    multypes[bwd][su], 2, options.intlong_rent); dbuf_destroy (&dbuf);
                FUNC_NONBANKED (muldiv[muldivmod][bwd][su]->type) = 1;
              }
          }
      }

    muluint() and mulsint() resp. mululong() and mulslong() return the same
    result. Therefore they've been merged into mulint() and mullong().
  */
  ...
  for (su = 0; su < 4; su++) {
    for (muldivmod = 0; muldivmod < 3; muldivmod++) {
      /* muluchar, mulschar, mulsuchar and muluschar are separate functions,
         because e.g. the z80
         port is sign/zero-extending to int before calling mulint() */
      /* div and mod : s8_t x s8_t -> s8_t should be s8_t x s8_t -> s16_t, see
       * below */
      struct dbuf_s dbuf;
      ...
      muldiv[muldivmod][bwd][su] =
          funcOfType(_mangleFunctionName(dbuf_c_str(&dbuf)),
                     multypes[bwd == 0 ? 1 : 0][su % 2], multypes[bwd][su / 2],
                     2, options.intlong_rent);
      dbuf_destroy(&dbuf);
    }
  }
  ...
  for (bwd = 1; bwd < 4; bwd++) {
    for (su = 0; su < 2; su++) {
      for (muldivmod = 1; muldivmod < 3; muldivmod++) {
        ...
        muldiv[muldivmod][bwd][su] =
            funcOfType(_mangleFunctionName(dbuf_c_str(&dbuf)),
                       multypes[(bwd == 0) ? 1 : bwd][su], multypes[bwd][su], 2,
  ...
  for (bwd = 1; bwd < 4; bwd++) {
    ...
    muldiv[muldivmod][bwd][0] =
        funcOfType(_mangleFunctionName(dbuf_c_str(&dbuf)), multypes[bwd][su],
...

  {
    ...
    muls16tos32[0] = port->support.has_mulint2long
                         ? funcOfTypeVarg("__mulsint2slong", "l", 2, iparams)
                         : 0;
    muls16tos32[1] = port->support.has_mulint2long
...
void initBuiltIns() {
  ...
  if (port->builtintable) {
    for (i = 0; port->builtintable[i].name; i++) {
      sym = funcOfTypeVarg(port->builtintable[i].name,
                           port->builtintable[i].rtype,
                           port->builtintable[i].nParms,
                           (const char **)port->builtintable[i].parm_types);
...
sym_link *validateLink(sym_link *l, const char *macro, const char *args,
                       const char select, const char *file, unsigned line) {
  ...
  fprintf(stderr,
          ...
          " expected %s, got %s\n",
          macro, args, file, line, DECLSPEC2TXT(select),
          l ? DECLSPEC2TXT(l->xclass) : "null-link");
  exit(EXIT_FAILURE);
...
/*--------------------------------------------------------------------*/
sym_link *newEnumType(symbol *enumlist) {
  int min, max, v;
  symbol *sym;
  sym_link *type;
