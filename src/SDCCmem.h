...
extern memmap *initialized; /* initialized data, such as initalized, nonzero
                               globals or local statics. */
extern memmap *initializer; /* a copy of the values for the initalized data from
                               initialized in code space */
...
#define IN_STACK(x)                                                            \
  (IS_SPEC(x) && (SPEC_OCLS(x) == xstack || SPEC_OCLS(x) == istack))
...
#define PTR_TYPE(map)                                                          \
  (map ? (map->ptrType ? map->ptrType : POINTER) : port->unqualified_pointer)

/* forward decls for functions    */
memmap *allocMap(char, char, char, char, char, char, unsigned, const char *,
                 char, int);
void initMem();
