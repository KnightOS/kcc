...
// Proceedings of the 16th International Workshop on Software and Compilers for
// Embedded Systems, M-SCOPES '13, pp. 23-30. Association for Computing
// Machinery, 2013.
...
bool assignments_naddr_locally_same(const assignment_naddr &a1,
                                    const assignment_naddr &a2) {
...
typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::bidirectionalS,
                              cfg_naddr_node, float>
    cfg_t; // The edge property is the cost of subdividing the edge and
...
void annotate_cfg_naddr(cfg_t &cfg,
  ...
  // Initialize the cfg vertices where there is information on the desired named
  // address space.
  for (vertex_t i = 0; i < boost::num_vertices(cfg); i++) {
    const iCode *ic = cfg[i].ic;
    const symbol *addrspace;

    // We do not know the current named address space when entering a function
    // or after calling one.
  ...
  do {
    ...
    for (vertex_t i = 0; i < boost::num_vertices(cfg); i++) {
      ...
      {
        /* MSVC 2010 doesn't like the typename here, though it accepts it
         * elsewhere */
        typedef /*typename*/ boost::graph_traits<cfg_t>::out_edge_iterator
            n_iter_t;
        n_iter_t n, n_end;
        for (boost::tie(n, n_end) = boost::out_edges(i, cfg); n != n_end; ++n) {
          vertex_t v = boost::target(*n, cfg);
          cfg[i].possible_naddrspaces.insert(
              cfg[v].possible_naddrspaces.begin(),
              cfg[v].possible_naddrspaces.end());
        }
      }
      {
        /* MSVC 2010 doesn't like the typename here, though it accepts it
         * elsewhere */
        typedef /*typename*/ boost::graph_traits<cfg_t>::in_edge_iterator
            n_iter_t;
        n_iter_t n, n_end;
...
void tree_dec_naddrswitch_leaf(
...
template <class T_t, class G_t>
int tree_dec_naddrswitch_introduce(
    T_t &T, typename boost::graph_traits<T_t>::vertex_descriptor t,
    const G_t &G) {
  typedef
  ...
  for (ai = alist.begin(); ai != alist.end(); ++ai) {
    ...
    for (ni = G[i].possible_naddrspaces.begin(),
        ni_end = G[i].possible_naddrspaces.end();
         ni != ni_end; ++ni) {
      ai->global[i] = *ni;
      alist2.push_back(*ai);
    }
...
template <class T_t, class G_t>
void tree_dec_naddrswitch_forget(
    T_t &T, typename boost::graph_traits<T_t>::vertex_descriptor t,
    const G_t &G) {
  typedef
      typename boost::graph_traits<T_t>::adjacency_iterator adjacency_iter_t;
  ...
  for (ai = alist.begin(); ai != alist.end();) {
    ...
    for (++ai;
         ai != alist.end() && assignments_naddr_locally_same(*aif, *ai);) {
      if (aif->s > ai->s) {
        alist.erase(aif);
        aif = ai;
        ++ai;
...
template <class T_t, class G_t>
void tree_dec_naddrswitch_join(
    T_t &T, typename boost::graph_traits<T_t>::vertex_descriptor t,
    const G_t &G) {
  typedef
      typename boost::graph_traits<T_t>::adjacency_iterator adjacency_iter_t;
  ...
  for (ai2 = alist2.begin(), ai3 = alist3.begin();
    ...
    if (assignments_naddr_locally_same(*ai2, *ai3)) {
      ...
      for (size_t i = 0; i < ai2->global.size(); i++)
        ai2->global[i] =
            ((ai2->global[i] != -2) ? ai2->global[i] : ai3->global[i]);
...
template <class T_t, class G_t>
int tree_dec_naddrswitch_nodes(
    T_t &T, typename boost::graph_traits<T_t>::vertex_descriptor t,
    const G_t &G) {
  typedef
      typename boost::graph_traits<T_t>::adjacency_iterator adjacency_iter_t;
...
template <class G_t>
static void implement_naddr_assignment(
    const assignment_naddr &a, const G_t &G,
    const std::map<naddrspace_t, const symbol *> addrspaces) {
  ...
  for (boost::tie(e, e_end) = boost::edges(G); e != e_end; ++e) {
    ...
    // This shouldn't happen with the CFGs sdcc generates and a cost function
    // based on code size.
...
void dump_cfg_naddr(const cfg_t &cfg) {
  std::ofstream dump_file((std::string(dstFileName) + ".dumpnaddrcfg" +
                           (currFunc ? currFunc->rname : "__global") + ".dot")
                              .c_str());
  ...
  for (unsigned int i = 0; i < boost::num_vertices(cfg); i++) {
    ...
    for (n = cfg[i].possible_naddrspaces.begin();
         n != cfg[i].possible_naddrspaces.end(); ++n)
      os << *n << " ";
    name[i] = os.str();
  }
