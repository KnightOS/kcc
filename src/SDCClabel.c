...
/*-------------------------------------------------------------------*/
static void deleteIfx(iCode *loop, int key) {
...
int labelIfx(iCode *ic) {
  ...
  for (loop = ic; loop; loop = loop->next) {
    ...
    /* if condition goto label */
    /* label:                  */
    if (loop->op == IFX && loop->next && loop->next->op == LABEL &&
    ...
    if (loop->op == IFX && loop->next && loop->next->op == GOTO &&
      ...
      if (IC_TRUE(loop) &&
              (IC_TRUE(loop))->key != (IC_LABEL(loop->next->next))->key ||
          (IC_FALSE(loop) &&
           (IC_FALSE(loop))->key != (IC_LABEL(loop->next->next))->key))
        continue;
      ...
      if (IC_TRUE(loop) && hTabItemWithKey(labelRef, (IC_TRUE(loop))->key)) {
        ...
        hTabDeleteItem(&labelRef, (IC_TRUE(loop))->key, loop, DELETE_ITEM,
                       NULL);
        IC_TRUE(loop) = NULL;
        ...
        hTabDeleteItem(&labelRef, (IC_LABEL(loop->next))->key, loop->next,
                       DELETE_ITEM, NULL);
      ...
      if (IC_FALSE(loop) && hTabItemWithKey(labelRef, (IC_FALSE(loop))->key)) {
        hTabDeleteItem(&labelRef, (IC_FALSE(loop))->key, loop, DELETE_ITEM,
                       NULL);
        IC_FALSE(loop) = NULL;
    ...
    /* Optimize hidden jump-to-jump:
       ...
       l0:
         v = 0;
       l1: */
    if (loop->op == LABEL && loop->next && loop->next->op == IFX &&
        (stat = hTabFirstItemWK(labelRef, (IC_LABEL(loop))->key)) &&
        !hTabNextItemWK(labelRef) && stat && stat->op == GOTO && stat->prev &&
        stat->prev->op == '=' && IS_OP_LITERAL(IC_RIGHT(stat->prev)) &&
        loop->prev && loop->prev->op == '=' &&
        IS_OP_LITERAL(IC_RIGHT(loop->prev)) &&
        ...
        (!operandLitValue(IC_RIGHT(stat->prev)) ^
         !operandLitValue(IC_RIGHT(loop->prev)))) {
      ...
      {
        hTabDeleteItem(&labelRef, (IC_LABEL(stat))->key, stat, DELETE_ITEM,
                       NULL);
        IC_LABEL(stat) =
            IC_TRUE(loop->next) ? IC_TRUE(loop->next) : IC_FALSE(loop->next);
        hTabAddItem(&labelRef, (IC_LABEL(stat))->key, stat);
        hTabDeleteItem(&labelRef, IC_LABEL(stat)->key, loop->next, DELETE_ITEM,
                       NULL);
        if (loop->next->next)
          loop->next->next->prev = loop;
        loop->next = loop->next->next;
...
/*-----------------------------------------------------------------*/
static symbol *replaceGotoGoto(const iCode *ic, const symbol *sLabel,
                               const iCode *target) {
  if (!target || !target->next)
    return 0;

  if (target->next->op != GOTO && target->next->op != LABEL ||
      target->next == ic)
    return 0;
...
int labelGotoGoto(iCode *ic) {
  ...
  for (loop = ic; loop; loop = loop->next) {
    ...
    case JUMPTABLE:
      ...
      for (sLabel = setFirstItem(IC_JTLABELS(loop)); sLabel;
           sLabel = setNextItem(IC_JTLABELS(loop)))
        if (repLabel = replaceGotoGoto(
                loop, sLabel, hTabItemWithKey(labelDef, sLabel->key))) {
          hTabDeleteItem(&labelRef, sLabel->key, loop, DELETE_ITEM, NULL);
    ...
    if (loop->op == GOTO || loop->op == RETURN ||
      ...
      for (tic = loop->next; tic && tic != loop2; tic = tic->next) {
        ...
        case IFX:
          ...
          if (IC_TRUE(tic))
            hTabDeleteItem(&labelRef, IC_TRUE(tic)->key, tic, DELETE_ITEM,
                           NULL);
          else
            hTabDeleteItem(&labelRef, IC_FALSE(tic)->key, tic, DELETE_ITEM,
                           NULL);
          break;
        default:
          werrorfl(tic->filename, tic->lineno, W_CODE_UNREACH);
...
iCode *iCodeLabelOptimize(iCode *ic) {
  if (!optimize.label1 && !optimize.label2 && !optimize.label3 &&
      !optimize.label4)
    return ic;
