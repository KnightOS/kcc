...
memmap *initialized = NULL; /* initialized data, such as initialized, nonzero
                               globals or local statics. */
memmap *initializer = NULL; /* a copz of the values for the initialized data
                               from initialized in code space */
...
void initMem() {
   ...
   */
  istack =
      allocMap(0, 0, 0, 0, 0, 0, options.stack_loc, ISTACK_NAME, 'B', POINTER);
  ...
  initialized = allocMap(0, 0, 0, 1, 0, 0, options.data_loc, INITIALIZED_NAME,
                         'E', POINTER);
  initializer = allocMap(0, 0, 0, 1, 0, 1, options.code_loc, INITIALIZER_NAME,
                         'C', CPOINTER);
  ...
  if (OVERLAY_NAME)
    overlay =
        allocMap(0, 0, 0, 1, 0, 0, options.data_loc, DATA_NAME, 'E', POINTER);
   ...
   */
  xdata =
      allocMap(0, 1, 0, 0, 0, 0, options.xdata_loc, XDATA_NAME, 'F', FPOINTER);
   ...
   */
  x_abs =
      allocMap(0, 1, 0, 0, 0, 0, options.xdata_loc, XABS_NAME, 'F', FPOINTER);
   ...
   */
  idata =
      allocMap(0, 0, 0, 0, 0, 0, options.idata_loc, IDATA_NAME, 'G', IPOINTER);
   ...
   */
  i_abs =
      allocMap(0, 0, 0, 0, 0, 0, options.data_loc, IABS_NAME, 'E', IPOINTER);
...
void allocGlobal(symbol *sym) {
  ...
  if (!sym->level) /* local statics can come here */
    SNPRINTF(sym->rname, sizeof(sym->rname), "%s%s", port->fun_prefix,
             sym->name);
  ...
  if (IS_FUNC(sym->type)) {
    ...
    if (FUNC_ISISR(sym->type) && !options.noiv &&
      ...
      if (interrupts[FUNC_INTNO(sym->type)])
        werror(E_INT_DEFINED, FUNC_INTNO(sym->type),
  ...
  if (SPEC_SCLS(sym->etype) == S_FIXED || SPEC_SCLS(sym->etype) == S_AUTO) {
...
void deallocParms(value *val) {
  ...
  for (lval = val; lval; lval = lval->next) {
    ...
    if (!lval->sym->isref) {
      lval->sym->allocreq = 0;
      werror(W_NO_REFERENCE, currFunc ? currFunc->name : "(unknown)",
    ...
    if (lval->sym->rname[0]) {
      ...
      addSym(SymbolTab, lval->sym, lval->sym->name, lval->sym->level,
             lval->sym->block, 1);
      lval->sym->_isparm = 1;
...
void allocLocal(symbol *sym) {
  ...
  SNPRINTF(sym->rname, sizeof(sym->rname), "%s%s_%s_%d_%d", port->fun_prefix,
...
void redoStackOffsets(void) {
  ...
  for (sym = setFirstItem(istack->syms); sym; sym = setNextItem(istack->syms)) {
    ...
    if (BTREE_STACK) {
      /* Remove them all, and let btree_alloc() below put them back in more
       * efficiently. */
      currFunc->stack -= size;
    ...
    /* Do it the old way - compared to the btree approach we waste space when
       allocating
...
#define SYM_BP(sym)                                                            \
  (SPEC_OCLS(sym->etype)->paged ? SP_BP("_spx", "_bpx") : SP_BP("sp", "_bp"))

/*-----------------------------------------------------------------*/
/* printAllocInfoSeg- print the allocation for a given section     */
/*-----------------------------------------------------------------*/
static int printAllocInfoSeg(memmap *map, symbol *func, struct dbuf_s *oBuf) {
  symbol *sym;
  int flg = FALSE;
...
static bool canOverlayLocals(eBBlock **ebbs, int count) {
  ...
  if (options.noOverlay || options.stackAuto ||
      (currFunc &&
