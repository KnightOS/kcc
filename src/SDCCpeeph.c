...
#define FBYNAME(x)                                                             \
  static int x(hTab *vars, lineNode *currPl, lineNode *endPl, lineNode *head,  \
               char *cmdLine)
...
static char *getPatternVar(hTab *vars, char **cmdLine) {
  int varNumber;
  char *digitend;
...
error:
  fprintf(stderr, "*** internal error: peephole restriction malformed: %s\n",
          *cmdLine);
  return NULL;
}
...
static int pcDistance(lineNode *cpos, char *lbl, bool back) {
  lineNode *pl = cpos;
...
/*-----------------------------------------------------------------*/
FBYNAME(flat24bitMode) { return (options.model == MODEL_FLAT24); }
...
FBYNAME(labelInRange) {
  ...
  do {
    ...
    dist = (pcDistance(currPl, lbl, TRUE) + pcDistance(currPl, lbl, FALSE));
...
/*-----------------------------------------------------------------*/
FBYNAME(optimizeReturn) { return (options.peepReturn >= 0); }
...
FBYNAME(okToRemoveSLOC) {
  ...
  for (pl = currPl->prev; pl; pl = pl->prev) {
    if (pl->line && !pl->isDebug && !pl->isComment && *pl->line != ';' &&
        strstr(pl->line, sloc))
      return FALSE;
  }
...
*/
static bool notVolatileVariable(const char *var, lineNode *currPl,
                                lineNode *endPl) {
  ...
  const bool global_not_volatile =
      currFunc ? !currFunc->funcUsesVolatile : false;

  /* Can't tell if indirect accesses are volatile or not, so
  ** assume they are (if there is a volatile access in the function at all),
  *just to be safe.
  */
  if (var[0] == '#')
    return true;
  ...
  *p = '\0';
    ...
    if (cl->ic && (cl->ic != last_ic)) {
      ...
      case IFX:
        ...
        if (IS_SYMOP(op) && (!strcmp(OP_SYMBOL(op)->rname, symname) ||
                             (OP_SYMBOL(op)->isspilt && SPIL_LOC(op) &&
  ...
  /* Couldn't find the symbol for some reason. Assume volatile if the current
   * function touches anything volatile. */
  return global_not_volatile;
}
...
/*------------------------------------------------------------------*/
static set *setFromConditionArgs(char *cmdLine, hTab *vars) {
  int varNumber;
  char *var;
  char *digitend;
  set *operands = NULL;
...
static const char *operandBaseName(const char *op) {
...
FBYNAME(canJoinRegs) {
  ...
  if (i < 0 || cmdLine[i + 1] != '%' ||
      (cmdLine[i + 1] &&
       (sscanf(&cmdLine[i + 2], "%d", &dstKey) != 1 || dstKey < 0))) {
    fprintf(stderr,
...
static long *immdGet(const char *pc, long *pl) {
  long s = 1;
...
static bool immdError(const char *info, const char *param, const char *cmd) {
  fprintf(stderr,
...
FBYNAME(immdInRange) {
  ...
  for (i = order = 0; order < 6;) {
    ...
    case 5: // result
      if (r[0] != '%' ||
          !(immdGet(r + 1, &h) || (r[1] == 'x' && immdGet(r + 2, &h))))
...
FBYNAME(inSequence) {
  ...
  if ((immdGet(op, &stride) == NULL) ||
      ((op = setNextItem(operands)) == NULL)) {
    fprintf(stderr,
            "*** internal error: inSequence peephole restriction"
            " malformed: %s\n",
            cmdLine);
  ...
  for (seq = LONG_MIN; op; op = setNextItem(operands)) {
    if ((immdGet(op, &val) == NULL) ||
        ((seq != LONG_MIN) && (val != seq + stride))) {
      deleteSet(&operands);
      return FALSE;
    }
    seq = val;
...
/*-----------------------------------------------------------------*/
static int callFuncByName(char *fname, hTab *vars,
...
/*-----------------------------------------------------------------*/
static peepRule *newPeepRule(lineNode *match, lineNode *replace, char *cond,
                             int restart, int barrier) {
  peepRule *pr;
...
/*-----------------------------------------------------------------*/
static void getPeepLine(lineNode **head, const char **bpp) {
...
/*-----------------------------------------------------------------*/
static void readRules(const char *bp) {
...
/*-----------------------------------------------------------------*/
static int keyForVar(const char *d) {
  int i = 0;
...
static void bindVar(int key, char **s, hTab **vtab) {
  ...
  while (*vvx && *vvx != ',' && !ISCHARSPACE(*vvx) && *vvx != '\n' &&
         *vvx != ':' && *vvx != ')') {
    char ubb = 0;
...
/*-----------------------------------------------------------------*/
static bool matchLine(char *s, const char *d, hTab **vars) {
  if (!s || !(*s))
    return FALSE;
  ...
  while (*s && *d) {
    ...
    if (*d == '%' && ISCHARDIGIT(*(d + 1)) && vars) {
      ...
      while (ISCHARDIGIT(*d))
        d++;
    } else if (ISCHARSPACE(*s) &&
               ISCHARSPACE(*d)) /* whitespace sequences match any whitespace
                                   sequences */
    {
      while (ISCHARSPACE(*s))
        s++;
      while (ISCHARSPACE(*d))
        d++;
               ...
               *d ==
                   ',') /* Allow comman to match comma followed by whitespace */
    {
      s++, d++;
      while (ISCHARSPACE(*s))
        s++;
...
/*-----------------------------------------------------------------*/
static bool matchRule(lineNode *pl, lineNode **mtail, peepRule *pr,
                      lineNode *head) {
  ...
  while (spl && rpl) {
    ...
    if (spl->line && (*spl->line == ';' || spl->isDebug)) {
      spl = spl->next;
      continue;
    }
...
static void reassociate_ic_down(lineNode *shead, lineNode *stail,
...
static void reassociate_ic_up(lineNode *shead, lineNode *stail, lineNode *rhead,
                              lineNode *rtail) {
  lineNode *csl; /* current source line */
  lineNode *crl; /* current replacement line */
...
/*------------------------------------------------------------------*/
static void reassociate_ic(lineNode *shead, lineNode *stail, lineNode *rhead,
                           lineNode *rtail) {
  lineNode *csl; /* current source line */
  lineNode *crl; /* current replacement line */
  bool single_iCode;
...
/* Not perfect, will not find all references yet.
   Will however find references in call on Z80, which is sufficient to fix
   #2970351. */
...
static void buildLabelRefCountHash(lineNode *head) {
  ...
  for (line = head; line; line = line->next) {
    ...
    /* run isLabelDefinition to:
       ...
       - calculate labelLen
    */
    if ((line->isLabel || line->isInline) &&
            isLabelDefinition(line->line, &label, &labelLen, FALSE) ||
      ...
      for (e = hTabFirstItemWK(labelHash, hashSymbolName(entry->name)); e;
           e = hTabNextItemWK(labelHash))
  ...
  for (line = head; line; line = line->next) {
    ...
    for (i = 0; i < HTAB_SIZE; i++) {
      ...
      while (thisEntry) {
        const char *s;
        if ((s = strstr(line->line, thisEntry->name)) &&
            !ISCHARALNUM(*(s + strlen(thisEntry->name))) &&
            (s == line->line || !ISCHARALNUM(*(s - 1))))
          thisEntry->refCount++;
