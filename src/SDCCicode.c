...
int dbuf_printOperand(operand *op, struct dbuf_s *dbuf) {
  ...
  case VALUE:
    ...
    if (IS_FLOAT(opetype))
      ...
      dbuf_printf(dbuf, "%g {",
                  doubleFromFixed16x16(SPEC_CVAL(opetype).v_fixed16x16));
    else if (IS_LONGLONG(opetype))
      dbuf_printf(
          dbuf, "0x%llx {",
          (unsigned long long)SPEC_CVAL(OP_VALUE(op)->etype).v_ulonglong);
    else
...
#define REGA 1
    ...
    if (REGA && !getenv("PRINT_SHORT_OPERANDS")) {
      dbuf_printf(dbuf,
                  "%s [k%d lr%d:%d so:%d]{ ia%d a2p%d re%d rm%d nos%d ru%d "
                  "dp%d}", /*{ar%d rm%d ru%d p%d a%d u%d i%d au%d k%d ks%d}"  ,
                            */
                  (OP_SYMBOL(op)->rname[0] ? OP_SYMBOL(op)->rname
                                           : OP_SYMBOL(op)->name),
                  op->key, OP_LIVEFROM(op), OP_LIVETO(op), OP_SYMBOL(op)->stack,
        ...
        if (OP_SYMBOL(op)->isspilt) {
          if (!OP_SYMBOL(op)->remat)
            if (OP_SYMBOL(op)->usl.spillLoc)
              dbuf_printf(dbuf, "[%s]",
                          (OP_SYMBOL(op)->usl.spillLoc->rname[0]
      ...
      if (getenv("PRINT_SHORT_OPERANDS")[0] < '1') {
        dbuf_printf(dbuf, "[lr%d:%d so:%d]", OP_LIVEFROM(op), OP_LIVETO(op),
                    OP_SYMBOL(op)->stack);
      }
...
PRINTFUNC(picIfx) {
  ...
  if (!IC_TRUE(ic))
    ...
    dbuf_printf(dbuf, " != 0 goto %s($%d)", IC_TRUE(ic)->name,
                IC_TRUE(ic)->key);
    if (IC_FALSE(ic))
...
PRINTFUNC(picInline) { dbuf_append_str(dbuf, IC_INLINE(ic)); }
...
int piCode(void *item, FILE *of) {
  ...
  fprintf(of, "%s(%d:%d:%d:%d:%d:%d)\t", ic->filename, ic->lineno, ic->seq,
          ic->key, ic->depth, ic->supportRtn, ic->block);
  dbuf_init(&dbuf, 1024);
...
void PICC(iCode *ic) { printiCChain(ic, stdout); }
...
void printiCChain(iCode *icChain, FILE *of) {
  ...
  for (loop = icChain; loop; loop = loop->next) {
    if ((icTab = getTableEntry(loop->op))) {
      ...
      fprintf(of, "%s(l%d:s%d:k%d:d%d:s%d:b%d)\t", loop->filename, loop->lineno,
              loop->seq, loop->key, loop->depth, loop->supportRtn, loop->block);
...
iCode *newiCode(int op, operand *left, operand *right) {
  ...
  // Err on the save side for now, settign this to false later is up to later
  // analysis.
...
/*-----------------------------------------------------------------*/
iCode *newiCodeCondition(operand *condition, symbol *trueLabel,
                         symbol *falseLabel) {
  iCode *ic;
...
/*-----------------------------------------------------------------*/
symbol *newiTempLabel(const char *s) {
  symbol *itmplbl;
...
/*-----------------------------------------------------------------*/
symbol *newiTempLoopHeaderLabel(bool pre) {
  symbol *itmplbl;
  struct dbuf_s dbuf;
...
operand *newiTempOperand(sym_link *type, char throwType) {
  ...
  if (type)
    itmp->etype =
        getSpec(itmp->type = (throwType ? type : copyLinkChain(type)));
...
/*-----------------------------------------------------------------*/
sym_link *operandType(const operand *op) {
  wassert(op);
...
int isOperandGlobal(const operand *op) {
  ...
  if (IS_SYMOP(op) &&
      (op->svt.symOperand->level == 0 || IS_STATIC(op->svt.symOperand->etype) ||
...
bool isOperandOnStack(operand *op) {
  ...
  etype = getSpec(operandType(op));
  if (IN_STACK(etype) || OP_SYMBOL(op)->onStack ||
...
/*-------------------------------------------------------------------*/
operand *detachiCodeOperand(operand **opp, iCode *ic) {
  operand *op = *opp;
...
/*-------------------------------------------------------------------*/
void attachiCodeOperand(operand *newop, operand **opp, iCode *ic) {
...
/*-----------------------------------------------------------------*/
unsigned long long operandLitValueUll(const operand *op) {
...
/*-----------------------------------------------------------------*/
double operandLitValue(const operand *op) {
  assert(isOperandLiteral(op));
...
/*-----------------------------------------------------------------*/
iCode *getBuiltinParms(iCode *fic, int *pcount, operand **parms) {
  sym_link *ftype;
  iCode *ic = fic;
...
/*-----------------------------------------------------------------*/
operand *operandOperation(operand *left, operand *right, int op,
                          sym_link *type) {
  ...
  switch (op) {
  case '+':
    retval = operandFromValue(
        valCastLiteral(type, operandLitValue(left) + operandLitValue(right),
  ...
  case '*':
    ...
    if (IS_INT(type) || !IS_SPEC(type)) {
      ...
      if (IS_LONGLONG(type) || !IS_SPEC(type))
        ...
        retval = operandFromValue(valCastLiteral(
            type, operandLitValue(left) * operandLitValue(right),
            operandLitValueUll(left) * operandLitValueUll(right)));
      ...
      else if (IS_UNSIGNED(type)) /* unsigned int */
      {
        ...
        TYPE_TARGET_ULONG ul =
            (TYPE_TARGET_UINT)double2ul(operandLitValue(left)) *
            (TYPE_TARGET_UINT)double2ul(operandLitValue(right));

        retval = operandFromValue(
            valCastLiteral(type, (TYPE_TARGET_UINT)ul, (TYPE_TARGET_UINT)ul));
      ...
      } else /* signed int */
      {
        ...
        retval = operandFromValue(
            valCastLiteral(type, (TYPE_TARGET_INT)l, (TYPE_TARGET_INT)l));
  ...
  case '/':
    ...
    if (IS_UNSIGNED(type)) {
      ...
      if (IS_LONGLONG(type))
        retval = operandFromValue(valCastLiteral(
            type, 0.0, operandLitValueUll(left) / operandLitValueUll(right)));
      else
        retval = operandFromValue(valCastLiteral(
            type,
            (TYPE_TARGET_ULONG)double2ul(operandLitValue(left)) /
  ...
  case LEFT_OP:
    ...
    if (IS_LONGLONG(type))
      retval = operandFromValue(valCastLiteral(
          type, (operandLitValueUll(left) << operandLitValueUll(right)),
          (operandLitValueUll(left) << operandLitValueUll(right))));
    else
      retval = operandFromValue(valCastLiteral(
          type,
  ...
  case RIGHT_OP:
    /* The number of right shifts is always unsigned. Signed doesn't make
       sense here. Shifting by a negative number is impossible. */
    retval = operandFromValue(
        valRecastLitVal(type, valShift(OP_VALUE(left), OP_VALUE(right), 0)));
    break;
  case EQ_OP:
  ...
  case RRC: {
    ...
    retval =
        operandFromLit((i >> (getSize(operandType(left)) * 8 - 1)) | (i << 1));
  } break;
  case RLC: {
    TYPE_TARGET_ULONG i = (TYPE_TARGET_ULONG)double2ul(operandLitValue(left));
  ...
  case GETABIT:
    retval =
        operandFromLit(((TYPE_TARGET_ULONG)double2ul(operandLitValue(left)) >>
                        (TYPE_TARGET_ULONG)double2ul(operandLitValue(right))) &
                       1);
    break;
  case GETBYTE:
  ...
  case UNARYMINUS:
    retval = operandFromValue(valCastLiteral(
        type, -1 * operandLitValue(left), (-1ll) * operandLitValueUll(left)));
    break;

  case '~':
    retval = operandFromValue(valCastLiteral(
        type, ~((TYPE_TARGET_ULONG)double2ul(operandLitValue(left))),
        ~((TYPE_TARGET_ULONGLONG)operandLitValueUll(left))));
    break;
  ...
  case ADDRESS_OF:
    retval = operandFromValue(
        valCastLiteral(type, operandLitValue(left),
                       (TYPE_TARGET_ULONGLONG)operandLitValueUll(left)));
    break;
...
int isOperandEqual(const operand *left, const operand *right) {
  ...
  case VALUE:
    return (
        compareType(left->svt.valOperand->type, right->svt.valOperand->type) &&
...
/*-----------------------------------------------------------------*/
operand *operandFromLit(double i) { return operandFromValue(valueFromLit(i)); }
...
void setOperandType(operand *op, sym_link *type) {
  ...
  case VALUE:
    op->svt.valOperand->etype =
        getSpec(op->svt.valOperand->type = copyLinkChain(type));
    return;

  case SYMBOL:
    if (op->svt.symOperand->isitmp) {
      op->svt.symOperand->etype =
          getSpec(op->svt.symOperand->type = copyLinkChain(type));
...
/*-----------------------------------------------------------------*/
static unsigned int getArraySizePtr(operand *op) {
...
static sym_link *usualBinaryConversions(operand **op1, operand **op2,
...
operand *geniCodeRValue(operand *op, bool force) {
  ...
  if ((IS_ARRAY(type) && !IS_FUNCPTR(type->next)) || IS_STRUCT(type) ||
  ...
  if (IS_SPEC(type) && IS_TRUE_SYMOP(op) &&
      (!(IN_FARSPACE(SPEC_OCLS(etype))) || (options.model == MODEL_FLAT24))) {
    op = operandFromOperand(op);
    op->isaddr = 0;
    return op;
  }
...
/*-----------------------------------------------------------------*/
static void checkPtrQualifiers(sym_link *ltype, sym_link *rtype,
                               int warn_const) {
...
/*-----------------------------------------------------------------*/
static operand *geniCodeCast(sym_link *type, operand *op, bool implicit) {
  iCode *ic;
  sym_link *optype;
  ...
  if (IS_LITERAL(opetype) && op->type == VALUE && !IS_PTR(type) &&
      !IS_PTR(optype)) {
    return operandFromValue(
        valCastLiteral(type, operandLitValue(op), operandLitValueUll(op)));
  }

  checkPtrCast(type, optype, implicit,
               IS_LITERAL(opetype) && !operandLitValue(op));
...
/*-----------------------------------------------------------------*/
static operand *geniCodeMultiply(operand *left, operand *right,
  ...
  if (getSize(ltype) > 1 || getSize(rtype) > 1) {
    if (IS_LITERAL(retype))
      ic = newiCode(
          '*', right,
          left); /* multiplication by support routine with one literal */
    else
...
static operand *geniCodeAdd(operand *left, operand *right,
                            RESULT_TYPE resultType, int lvl);
static operand *geniCodeLogic(operand *left, operand *right, int op, ast *tree);
operand *geniCodeRightShift(operand *left, operand *right);

/*-----------------------------------------------------------------*/
/* geniCodeDivision - gen intermediate code for division           */
/*-----------------------------------------------------------------*/
static operand *geniCodeDivision(operand *left, operand *right,
                                 RESULT_TYPE resultType, bool ptrdiffdiv) {
  iCode *ic;
  int p2 = 0;
  ...
  if (IS_LITERAL(retype) &&
      (!IS_FLOAT(letype) && !IS_FIXED(letype) && IS_UNSIGNED(letype) ||
       ptrdiffdiv) &&
  ...
  /* if the right is a literal & power of 2
  ...
  else if (IS_LITERAL(retype) && !IS_FLOAT(letype) && !IS_FIXED(letype) &&
           !IS_UNSIGNED(letype) &&
           ((p2 = powof2((TYPE_TARGET_ULONG)ulFromVal(OP_VALUE(right)))) > 0)) {
    operand *tmp;
    ...
    ic = newiCodeCondition(geniCodeLogic(tmp, operandFromLit(0), '<', 0), 0,
                           label);
    ADDTOCHAIN(ic);

    geniCodeAssign(tmp, geniCodeAdd(tmp, operandFromLit((1 << p2) - 1), 0, 0),
                   0, 0);
    geniCodeLabel(label);
    return (geniCodeCast(resType, geniCodeRightShift(tmp, operandFromLit(p2)),
                         TRUE));
  }
...
/*-----------------------------------------------------------------*/
static operand *geniCodeModulus(operand *left, operand *right,
                                RESULT_TYPE resultType) {
  iCode *ic;
  sym_link *resType;
  LRTYPE;
...
/*-----------------------------------------------------------------*/
static operand *geniCodeSubtract(operand *left, operand *right,
                                 RESULT_TYPE resultType) {
  iCode *ic;
  int isarray = 0;
  sym_link *resType;
  ...
  /* if they are both literal then we know the result */
  if (IS_LITERAL(letype) && IS_LITERAL(retype) && left->isLiteral &&
      right->isLiteral)
  ...
  if (IS_PTR(ltype) || IS_ARRAY(ltype)) {
    isarray = left->isaddr;
    right =
        geniCodeMultiply(right, operandFromLit(getSize(ltype->next)),
                         (getArraySizePtr(left) >= INTSIZE) ? RESULT_TYPE_INT
                                                            : RESULT_TYPE_CHAR);
...
static operand *geniCodeAdd(operand *left, operand *right,
  ...
  if (IS_LITERAL(retype) && right->isLiteral &&
      !floatFromVal(valFromType(rtype)))
    return left;

  /* if left is literal zero return right */
  if (!IS_PTR(ltype) && IS_LITERAL(letype) && left->isLiteral &&
  ...
  if (IS_PTR(ltype) || IS_ARRAY(ltype)) {
    ...
    if (nBytes == 0 && !IS_VOID(ltype->next))
      werror(E_UNKNOWN_SIZE,
             IS_SYMOP(left) ? OP_SYMBOL(left)->name : "<no name>");
    ...
    if (nBytes != 1) {
      ...
      right = geniCodeMultiply(right, size,
                               (ptrSize >= INTSIZE) ? RESULT_TYPE_INT
                                                    : RESULT_TYPE_CHAR);
...
/*-----------------------------------------------------------------*/
static operand *geniCodeArray2Ptr(operand *op) {
...
/*-----------------------------------------------------------------*/
static operand *geniCodeArray(operand *left, operand *right, int lvl) {
  iCode *ic;
  operand *size;
  sym_link *ltype = operandType(left);
  bool indexUnsigned;
  ...
  /* Even if right is a 'unsigned char', the result will be a 'signed int' due
     to the promotion rules. It doesn't make sense when accessing arrays, so
     let's fix it here: */
  if (indexUnsigned)
    SPEC_USIGN(getSpec(operandType(right))) = 1;
  ...
  IC_RESULT(ic) = newiTempOperand(
      ((IS_PTR(ltype) && !IS_AGGREGATE(ltype->next) && !IS_PTR(ltype->next)) ||
...
operand *geniCodeStruct(operand *left, operand *right, bool islval) {
  ...
  wassert(IS_STRUCT(type) ||
          ((IS_PTR(type) || IS_ARRAY(type)) && IS_STRUCT(type->next)));
...
operand *geniCodePostInc(operand *op) {
  ...
  operand *result;
  operand *rv =
      (IS_ITEMP(op) ? geniCodeRValue(
                          op, (!op->aggr2ptr && IS_PTR(optype)) ? TRUE : FALSE)
                    : op);
...
/*-----------------------------------------------------------------*/
operand *geniCodePreInc(operand *op, bool lvalue) {
  iCode *ic;
  sym_link *optype = operandType(op);
  operand *rop =
      (IS_ITEMP(op)
           ? geniCodeRValue(op,
                            ((!op->aggr2ptr && IS_PTR(optype)) ? TRUE : FALSE))
           : op);
  ...
  if (lvalue || (IS_TRUE_SYMOP(op) && !isOperandVolatile(op, FALSE)) ||
      IS_BITVAR(optype))
    return op;
  else
    return result;
...
operand *geniCodeDerefPtr(operand *op, int lvl) {
  ...
  op->isaddr =
      (IS_PTR(rtype) || IS_STRUCT(rtype) || IS_INT(rtype) || IS_BOOL(rtype) ||
       IS_CHAR(rtype) || IS_FLOAT(rtype) || IS_FIXED(rtype));
...
/*-----------------------------------------------------------------*/
static operand *geniCodeLogic(operand *left, operand *right, int op,
                              ast *tree) {
  iCode *ic;
  ...
  /* Avoid expensive comparisons when the type of the constant is bigger than
   * the type of the non-const operand */
  if (IS_INTEGRAL(ltype) && IS_LITERAL(rtype) &&
      getSize(ltype) < getSize(rtype))
    right->svt.valOperand = valCastLiteral(ltype, operandLitValue(right),
                                           operandLitValueUll(right));
  if (IS_INTEGRAL(rtype) && IS_LITERAL(ltype) &&
      getSize(rtype) < getSize(ltype))
    left->svt.valOperand =
        valCastLiteral(rtype, operandLitValue(left), operandLitValueUll(left));

  /* if one operand is a pointer and the other is a literal generic void
     pointer, change the type of the literal generic void pointer to match the
     other pointer */
  if (IS_GENPTR(ltype) && IS_VOID(ltype->next) && IS_ITEMP(left) &&
    ...
    if (ic && (ic->op == CAST) && isOperandLiteral(IC_RIGHT(ic))) {
      left =
          operandFromValue(valCastLiteral(rtype, operandLitValue(IC_RIGHT(ic)),
  ...
  if (IS_GENPTR(rtype) && IS_VOID(rtype->next) && IS_ITEMP(right) &&
    ...
    if (ic && (ic->op == CAST) && isOperandLiteral(IC_RIGHT(ic))) {
      right =
          operandFromValue(valCastLiteral(ltype, operandLitValue(IC_RIGHT(ic)),
                                          operandLitValueUll(IC_RIGHT(ic))));
      rtype = operandType(right);
    }
  }
  ...
  /* if comparing float
     and not a '==' || '!=' || '&&' || '||' (these
     will be inlined */
  if (IS_FLOAT(ctype) && op != EQ_OP && op != NE_OP && op != AND_OP &&
      op != OR_OP)
    ic->supportRtn = 1;
...
/*-----------------------------------------------------------------*/
static operand *geniCodeLogicAndOr(ast *tree, int lvl) {
  iCode *ic;
  sym_link *type;
...
/*-----------------------------------------------------------------*/
operand *geniCodeUnary(operand *op, int oper, sym_link *resType) {
...
/*-----------------------------------------------------------------*/
operand *geniCodeBinary(operand *left, operand *right, int oper,
                        sym_link *resType) {
...
/*-----------------------------------------------------------------*/
operand *geniCodeConditional(ast *tree, int lvl) {
  iCode *ic;
  symbol *falseLabel = newiTempLabel(NULL);
  symbol *exitLabel = newiTempLabel(NULL);
...
/*-----------------------------------------------------------------*/
static operand *checkTypes(operand *left, operand *right) {
  sym_link *ltype = operandType(left);
  sym_link *rtype = operandType(right);
...
operand *geniCodeAssign(operand *left, operand *right, int nosupdate,
  ...
  if (IS_TRUE_SYMOP(left) && !isOperandVolatile(left, FALSE) &&
...
/*-----------------------------------------------------------------*/
static void geniCodeDummyRead(operand *op) {
  iCode *ic;
  sym_link *type = operandType(op);
...
/*-----------------------------------------------------------------*/
static void geniCodeSEParms(ast *parms, int lvl) {
  if (!parms)
    return;
  ...
  if (IS_CAST_OP(parms) && IS_PTR(parms->ftype) &&
  ...
  AST_ARGREG(parms) =
      parms->etype ? SPEC_ARGREG(parms->etype) : SPEC_ARGREG(parms->ftype);
}

/*-----------------------------------------------------------------*/
/* geniCodeParms - generates parameters                            */
/*-----------------------------------------------------------------*/
value *geniCodeParms(ast *parms, value *argVals, int *iArg, int *stack,
  ...
  if ((IS_REGPARM(parms->etype) && !IFFUNC_HASVARARGS(ftype)) ||
    ...
    if (!(options.stackAuto || IFFUNC_ISREENT(ftype))) {
      ...
      *stack += getSize(IS_ARRAY(p) ? aggrToPtr(p, FALSE) : p);
      if ((IFFUNC_ISSMALLC(ftype)) && !IS_AGGREGATE(p) &&
          getSize(p) ==
              1) /* SmallC calling convention passes 8-bit parameters as 16-bit
                    values. So does pdk due to stack alignment requirements */
        (*stack)++;
      ADDTOCHAIN(ic);
    }
  }
...
/*-----------------------------------------------------------------*/
operand *geniCodeCall(operand *left, ast *parms, int lvl) {
  iCode *ic;
  operand *result;
  sym_link *type, *etype;
  sym_link *ftype;
...
/*-----------------------------------------------------------------*/
static void geniCodeReceive(value *args, operand *func) {
  ...
  while (args) {
    if (IS_REGPARM(args->etype)) {
      ...
      if (!sym->addrtaken && !IS_VOLATILE(sym->etype)) {
        ...
        if ((IN_FARSPACE(SPEC_OCLS(sym->etype))) && options.stackAuto == 0 &&
            (!(options.model == MODEL_FLAT24))) {
        } else {
...
int geniCodeJumpTable(operand *cond, value *caseVals, ast *tree) {
  ...
  sizeofJumpTable = (1 + max - min) * port->jumptableCost.sizeofElement +
                    port->jumptableCost.sizeofDispatch + sizeofMinCost +
                    sizeofMaxCost;
  ...
  /* first we rule out the boundary conditions */
  {
    ...
    if ((checkConstantRange(cetype, caseVals->etype, '<', FALSE) !=
         CCR_ALWAYS_FALSE) &&
    ...
    if (checkConstantRange(cetype, maxVal->etype, '>', FALSE) !=
        CCR_ALWAYS_FALSE) {
...
void geniCodeSwitch(ast *tree, int lvl) {
  ...
  if (IS_LITERAL(getSpec(operandType(cond)))) {
    ...
    while (caseVals) {
      ...
      if (caseVal == switchVal) {
        ...
        dbuf_init(&dbuf, 128);
        dbuf_printf(
            &dbuf, "_case_%d_%d%s", tree->values.switchVals.swNum, caseVal,
  ...
  while (caseVals) {
    operand *compare =
        geniCodeLogic(cond, operandFromValue(caseVals), EQ_OP, NULL);

    dbuf_init(&dbuf, 128);
    dbuf_printf(&dbuf, "_case_%d_%d%s", tree->values.switchVals.swNum,
...
/*-----------------------------------------------------------------*/
static void geniCodeArrayInit(ast *tree, operand *array) {
  iCode *ic;
...
/*-----------------------------------------------------------------*/
static void geniCodeCritical(ast *tree, int lvl) {
  iCode *ic;
  operand *op = NULL;
  sym_link *type;
...
/*-----------------------------------------------------------------*/
operand *ast2iCode(ast *tree, int lvl) {
  operand *left = NULL;
  operand *right = NULL;
  if (!tree)
    return NULL;
  ...
  /* if we find a nullop */
  if (tree->type == EX_OP &&
      (tree->opval.op == NULLOP || tree->opval.op == BLOCK)) {
    if (tree->left && tree->left->type == EX_VALUE)
      geniCodeDummyRead(ast2iCode(tree->left, lvl + 1));
    else
      ast2iCode(tree->left, lvl + 1);
  ...
  if (tree->opval.op != ':' && tree->opval.op != '?' &&
      tree->opval.op != CALL && tree->opval.op != IFX &&
      tree->opval.op != AND_OP && tree->opval.op != OR_OP &&
      tree->opval.op != LABEL && tree->opval.op != GOTO &&
      tree->opval.op != SWITCH && tree->opval.op != FUNCTION &&
      tree->opval.op != INLINEASM && tree->opval.op != CRITICAL) {
    if (IS_ASSIGN_OP(tree->opval.op) || IS_DEREF_OP(tree) ||
        IS_ADDRESS_OF_OP(tree)) {
      addLvaluereq(lvl);
      if ((!IS_ADDRESS_OF_OP(tree) && IS_ARRAY_OP(tree->left) &&
           IS_ARRAY_OP(tree->left->left) && tree->left->left->ftype &&
           IS_ARRAY(tree->left->left->ftype) && tree->left->left->ftype->next &&
           IS_ARRAY(tree->left->left->ftype->next)) ||
  ...
  case '^':
    return geniCodeBitwise(geniCodeRValue(left, FALSE),
                           geniCodeRValue(right, FALSE), tree->opval.op,
                           tree->ftype);

  case '/':
    return geniCodeDivision(geniCodeRValue(left, FALSE),
                            geniCodeRValue(right, FALSE),
                            getResultTypeFromType(tree->ftype), false);
  ...
  case '+':
    if (right)
      return geniCodeAdd(geniCodeRValue(left, FALSE),
                         geniCodeRValue(right, FALSE),
                         getResultTypeFromType(tree->ftype), lvl);
    else
  ...
  case GETHBIT: {
    operand *op =
        geniCodeUnary(geniCodeRValue(left, FALSE), tree->opval.op, tree->ftype);
    return op;
  }
  case GETABIT: {
  ...
  case '=': {
    sym_link *rtype = operandType(right);
    sym_link *ltype = operandType(left);
    if (IS_PTR(rtype) && IS_ITEMP(right) && right->isaddr &&
        compareType(rtype->next, ltype) == 1)
      right = geniCodeRValue(right, TRUE);
    else
      right = geniCodeRValue(right, FALSE);
  ...
  case MUL_ASSIGN:
    return geniCodeAssign(
        left,
        geniCodeMultiply(geniCodeRValue(operandFromOperand(left), FALSE),
                         geniCodeRValue(right, FALSE),
                         getResultTypeFromType(tree->ftype)),
        0, 1);
  ...
  case MOD_ASSIGN:
    return geniCodeAssign(
        left,
        geniCodeModulus(geniCodeRValue(operandFromOperand(left), FALSE),
                        geniCodeRValue(right, FALSE),
                        getResultTypeFromType(tree->ftype)),
        0, 1);
  case ADD_ASSIGN: {
    ...
    return geniCodeAssign(
        left,
        geniCodeAdd(geniCodeRValue(operandFromOperand(left), FALSE), right,
                    getResultTypeFromType(tree->ftype), lvl),
        0, 1);
  }
  case SUB_ASSIGN: {
    ...
    return geniCodeAssign(
        left,
        geniCodeSubtract(geniCodeRValue(operandFromOperand(left), FALSE), right,
                         getResultTypeFromType(tree->ftype)),
        0, 1);
  }
  case LEFT_ASSIGN:
  ...
  case AND_ASSIGN:
    return geniCodeAssign(
        ...
        geniCodeBitwise(geniCodeRValue(operandFromOperand(left), FALSE),
                        geniCodeRValue(right, FALSE), BITWISEAND,
                        operandType(left)),
        0, 1);
  case XOR_ASSIGN:
    return geniCodeAssign(
...
operand *validateOpType(operand *op, const char *macro, const char *args,
                        OPTYPE type, const char *file, unsigned line) {
  if (op && op->type == type) {
    return op;
  }
  fprintf(stderr,
          ...
          " expected %s, got %s\n",
          macro, args, file, line, opTypeToStr(type),
          op ? opTypeToStr(op->type) : "null op");
  exit(EXIT_FAILURE);
  return op; // never reached, makes compiler happy.
}

const operand *validateOpTypeConst(const operand *op, const char *macro,
                                   const char *args, OPTYPE type,
