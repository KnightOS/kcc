// Philipp Klaus Krause, philipp@informatik.uni-frankfurt.de, pkk@spth.de,
// 2011-2018
//
...
typedef boost::adjacency_list<boost::setS, boost::vecS, boost::undirectedS,
                              scon_node_t, scon_edge_t>
    scon_t; // Conflict graph for on-stack variables
...
static bool clash(const symbol *s1, const symbol *s2) {
  ...
  if (!s1->isspilt &&
      !(IS_AGGREGATE(s1->type) ||
        s1->allocreq &&
            (s1->addrtaken || isVolatile(s1->type)))) // Spill location
  {
    for (const symbol *s = (const symbol *)setFirstItem(s1->usl.itmpStack); s;
         s = (const symbol *)setNextItem(s1->usl.itmpStack))
      if (clash(s, s2))
        return (true);
    return (false);
  }
  if (!s2->isspilt &&
      !(IS_AGGREGATE(s2->type) ||
        s2->allocreq &&
            (s2->addrtaken || isVolatile(s2->type)))) // Spill location
  {
    for (const symbol *s = (const symbol *)setFirstItem(s2->usl.itmpStack); s;
         s = (const symbol *)setNextItem(s2->usl.itmpStack))
      if (clash(s1, s))
        return (true);
    return (false);
  }
...
static var_t
var_from_operand(const std::map<const symbol *, var_t> &symbol_to_sindex,
                 const operand *const op) {
  if (!op || !IS_SYMOP(op))
    return (-1);
  std::map<const symbol *, var_t>::const_iterator si =
      symbol_to_sindex.find(OP_SYMBOL_CONST(op));
...
static void set_spilt(G_t &G, const I_t &I, SI_t &scon) {
  ...
  // Add variables that need to be on the stack due to having had their address
  // taken (or for a few other reasons, such as being too large or too many to
  // behandled by the register allocator).
  for (sym = static_cast<symbol *>(setFirstItem(istack->syms)), j = 0; sym;
       sym = static_cast<symbol *>(setNextItem(istack->syms))) {
    if (sym->_isparm)
      continue;

    // std::cout << "set_spilt() 1: Considering " << sym->name << "[" <<
    // sym->isspilt << ", " << IS_AGGREGATE(sym->type) << ", " << sym->allocreq
    // << ", " << sym->addrtaken << "]\n";
    ...
    if (!sym->isspilt &&
        !(IS_AGGREGATE(sym->type) ||
          sym->allocreq &&
              (sym->addrtaken ||
               isVolatile(
                   sym->type)))) // Looks like a spill location - check if it is
                                 // already covered by live ranges below.
    {
      bool covered = true;
      for (const symbol *s = (const symbol *)setFirstItem(sym->usl.itmpStack);
           s; s = (const symbol *)setNextItem(sym->usl.itmpStack))
        if (!s->for_newralloc) {
#ifdef DEBUG_SALLOC
          std::cout << "Adding " << sym->name << " for " << s->name << "(" << s
                    << ") to be allocated to stack. (" << s->for_newralloc
                    << ")\n";
          std::cout.flush();
#endif
          covered = false;
  ...
  // Add edges due to scope (see C99 standard, verse 1233, which requires things
  // to have different addresses, not allowing us to allocate them to the same
  // location, even if we otherwise could).
  for (unsigned int i = 0; i < boost::num_vertices(scon); i++)
    for (unsigned int j = i + 1; j < boost::num_vertices(scon); j++) {
      if (!(scon[i].sym->addrtaken) || !(scon[i].sym->addrtaken))
        continue;
      short p =
          btree_lowest_common_ancestor(scon[i].sym->block, scon[j].sym->block);
  ...
  for (unsigned int i = 0; i < boost::num_vertices(G); i++) {
    ...
    for (unsigned int j = 0; j < boost::num_vertices(scon); j++) {
      short p =
          btree_lowest_common_ancestor(G[i].ic->block, scon[j].sym->block);
      if (p == G[i].ic->block || p == scon[j].sym->block) {
        G[i].stack_alive.insert(j);
        if (scon[j].sym->addrtaken ||
            IS_AGGREGATE(scon[j].sym->type)) // TODO: More accurate analysis.
  ...
  for (unsigned int i = 0; i < boost::num_vertices(G); i++) {
    ...
    for (v = G[i].alive.begin(), v_end = G[i].alive.end(); v != v_end; ++v) {
      ...
      if ((sym->regs[0] && !sym->isspilt) || sym->accuse || sym->remat ||
          !sym->nRegs || sym->usl.spillLoc && sym->usl.spillLoc->_isparm)
        continue;
  ...
  for (boost::tie(e, e_end) = boost::edges(I); e != e_end; ++e) {
    if (I[boost::source(*e, I)].v == I[boost::target(*e, I)].v ||
        iindex_to_sindex.find(I[boost::source(*e, I)].v) ==
            iindex_to_sindex.end() ||
        iindex_to_sindex.find(I[boost::target(*e, I)].v) ==
            iindex_to_sindex.end())
      continue;

    boost::add_edge(iindex_to_sindex[I[boost::source(*e, I)].v],
                    iindex_to_sindex[I[boost::target(*e, I)].v], scon);
  }

  // Add conflicts between variables that had their address taken and those that
  // have been spilt by register allocation.
  // TODO: More exact live range analysis for variables that had their address
  // taken (to reduce stack space consumption further, by reducing the number of
  // conflicts here).
  for (unsigned int i = 0; i < j_mark; i++)
    for (unsigned int j = 0; j < boost::num_vertices(scon); j++) {
      if (i == j)
        continue;
      if (!scon[i].sym->isspilt &&
          !(IS_AGGREGATE(scon[i].sym->type) ||
            scon[i].sym->allocreq &&
                (scon[i].sym->addrtaken ||
                 isVolatile(scon[i].sym->type)))) // Spill location
      {
  ...
  // Ugly hack: Regparms.
  for (sym = static_cast<symbol *>(setFirstItem(istack->syms)),
      j = boost::num_vertices(scon);
       sym; sym = static_cast<symbol *>(setNextItem(istack->syms))) {
    if (!sym->_isparm || !IS_REGPARM(sym->etype) || !sym->onStack ||
        !sym->allocreq)
      continue;
  ...
  for (unsigned int i = 0; i < boost::num_vertices(G); i++) {
    ...
    if (left >= 0 && !boost::edge(result, left, scon).second)
      scon[(boost::add_edge(result, left, scon)).first]
          .alignment_conflict_only = true;
    if (right >= 0 && !boost::edge(result, right, scon).second)
      scon[(boost::add_edge(result, right, scon)).first]
          .alignment_conflict_only = true;
  }
}
...
void color_stack_var(const var_t v, SI_t &SI, int start, int *ssize) {
  ...
  symbol *const ssym =
      (sym->isspilt && sym->usl.spillLoc) ? sym->usl.spillLoc : sym;
...
#ifdef DEBUG_SALLOC
  std::cout << "Placing " << sym->name << " (really " << ssym->name << ") at ["
            << start << ", " << (start + size - 1) << "]\n";
  std::cout.flush();
#endif
  ...
  for (boost::tie(n, n_end) = boost::adjacent_vertices(v, SI); n != n_end; ++n)
    if (!SI[boost::edge(v, *n, SI).first].alignment_conflict_only)
      SI[*n].free_stack -=
          boost::icl::discrete_interval<int>::type(start, start + size);
    else
      SI[*n].alignment_conflicts.insert(
          boost::icl::discrete_interval<int>::type(start, start + size));
}

// Place a single variable on the stack greedily.
template <class SI_t>
void color_stack_var_greedily(const var_t v, SI_t &SI, int alignment,
                              int *ssize) {
  int start;
  symbol *const sym = SI[v].sym;
  const int size = getSize(sym->type);
  ...
  for (si = SI[v].free_stack.begin();; ++si) {
    ...
    do {
      ...
      for (ai = SI[v].alignment_conflicts.begin(),
          ai_end = SI[v].alignment_conflicts.end();
           ai != ai_end; ++ai) {
...
static int get_alignment(sym_link *type) {
#if 1
  return (1);
#else
...
void chaitin_ordering(const SI_t &SI, std::list<var_t> &ordering) {
  ...
  for (num_marked = 0; num_marked < boost::num_vertices(SI); num_marked++) {
    ...
    for (i = 0; i < boost::num_vertices(SI); i++) {
      ...
      for (boost::tie(n, n_end) = boost::adjacent_vertices(i, SI), d = 0;
           n != n_end; ++n)
        d += !marked[*n];

      if (d < mind ||
          d == mind &&
              get_alignment(SI[i].sym->type) <
                  get_alignment(
                      SI[minn]
                          .sym->type)) // Coloring aligned variables first tends
                                       // to keep gaps from alignment small.
      {
        mind = d;
        minn = i;
      }
...
template <class SI_t> void chaitin_salloc(SI_t &SI) {
  std::list<var_t> ordering;

  chaitin_ordering(SI, ordering);

  for (unsigned int i = 0; i < boost::num_vertices(SI); i++)
    SI[i].free_stack.insert(
        boost::icl::discrete_interval<int>::type(0, 1 << 15));
  ...
  for (i = ordering.begin(), i_end = ordering.end(); i != i_end; ++i) {
    // Alignment, even when not required by the hardware helps avoid partially
    // overlapping stack operands (which are not supported by code generation in
    // some backends).
  ...
  if (currFunc) {
    if (TARGET_PDK_LIKE && (ssize % 2)) // Padauk requires even stack alignment.
      ssize++;
#ifdef DEBUG_SALLOC
    std::cout << "Function " << currFunc->name << " currFunc->stack: old "
              << currFunc->stack << ", new " << (currFunc->stack + ssize)
              << "\n";
#endif
...
static void dump_scon(const scon_t &scon) {
  if (!currFunc)
    return;

  std::ofstream dump_file(
      (std::string(dstFileName) + ".dumpsalloccon" + currFunc->rname + ".dot")
          .c_str());

  std::string *name = new std::string[boost::num_vertices(scon)];
  for (var_t i = 0;
       static_cast<boost::graph_traits<scon_t>::vertices_size_type>(i) <
       boost::num_vertices(scon);
       i++) {
    ...
    if (scon[i].sym->name)
      os << " : " << scon[i].sym->name << " : " << getSize(scon[i].sym->type)
         << " [" << start << "," << (start + getSize(scon[i].sym->type) - 1)
         << "]";
    name[i] = os.str();
  }
