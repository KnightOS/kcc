...
// A quick-and-dirty function to get the CFG from sdcc (a simplified version of
// the function from SDCCralloc.hpp).
void create_cfg_lospre(cfg_lospre_t &cfg, iCode *start_ic, ebbIndex *ebbi) {
  iCode *ic;
  ...
  for (ic = start_ic; ic; ic = ic->next) {
    if ((ic->op == '>' || ic->op == '<' || ic->op == LE_OP || ic->op == GE_OP ||
      ...
      boost::add_edge(key_to_index[ic->key], key_to_index[ic->next->key], 4.0f,
                      cfg); // Try not to separate op from ifx.
    else if (ic->op != GOTO && ic->op != RETURN && ic->op != JUMPTABLE &&
             ic->next)
      boost::add_edge(key_to_index[ic->key], key_to_index[ic->next->key], 3.0f,
                      cfg);

    if (ic->op == GOTO)
      boost::add_edge(
          key_to_index[ic->key],
          key_to_index[eBBWithEntryLabel(ebbi, ic->label)->sch->key], 6.0f,
          cfg);
    else if (ic->op == RETURN)
      boost::add_edge(
          key_to_index[ic->key],
          key_to_index[eBBWithEntryLabel(ebbi, returnLabel)->sch->key], 6.0f,
          cfg);
    else if (ic->op == IFX)
      boost::add_edge(
          key_to_index[ic->key],
...
static bool same_expression(const iCode *const lic, const iCode *const ric) {
  wassert(lic);
  wassert(ric);
...
static void get_candidate_set(std::set<int> *c, const iCode *const sic,
                              int lkey) {
  // TODO: For loop invariant code motion allow expression that only occurs
  // once, too - will be needed when optimizing for speed.
  for (const iCode *ic = sic; ic; ic = ic->next) {
    if (!candidate_expression(ic, lkey))
      continue;
    for (const iCode *pic = sic; pic != ic; pic = pic->next)
      if (candidate_expression(pic, lkey) && same_expression(ic, pic) &&
          c->find(pic->key) == c->end()) {
...
static bool setup_cfg_for_expression(cfg_lospre_t *const cfg,
  ...
  const bool uses_global =
      (eic->op == GET_VALUE_AT_ADDRESS || isOperandGlobal(eleft) ||
  ...
  // were it was not done before. This is important, if the compuation can have
  // side-effects, which depends on the target architecure. E.g. On some systems
  // division requires safety, since division by zero might result in an
  // interrupt. When there are memory-mapped devices or there is memory
  // management, reading from a pointer requires safety, since reading from an
  // unknown location could result in making the device do something or in a
  // SIGSEGV. On the other hand, addition is something that typically does not
  ...
  //
  // TODO: Replace the current one  by a more exact mechanism, that takes into
  // account information from (not yet implemented) generalized constant
  // propagation, pointer analysis, etc.
  ...
  // improve speed. This probably needs implementation of profile-guided
  // optimization though.
...
void dump_cfg_lospre(const cfg_lospre_t &cfg) {
  ...
  std::ofstream dump_file(
      (std::string(dstFileName) + ".dumplosprecfg" + currFunc->rname + ".dot")
          .c_str());
...
void lospre(iCode *sic, ebbIndex *ebbi) {
  ...
  for (bool change = true; change;) {
    ...
    for (ci = candidate_set.begin(), ci_end = candidate_set.end(); ci != ci_end;
         ++ci) {
      const iCode *ic;
      ...
      if (safety &&
          tree_dec_safety(tree_decomposition, control_flow_graph, ic) < 0)
        continue;

      change |=
          (tree_dec_lospre(tree_decomposition, control_flow_graph, ic) > 0);
    }
  }
}
