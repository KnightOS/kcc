...
*/
set *processStrSet(set *list, const char *pre, const char *post,
...
/** Split the path string to the directory and file name (including extension)
   components. The directory component doesn't contain trailing directory
   separator. Returns true if the path contains the directory separator. */
...
/** Split the path string to the file name (including directory) and file
   extension components. File extension component contains the extension
   separator. Returns true if the path contains the extension separator. */
...
/** Combine directory and the file name to a path string using the
 * DIR_SEPARATOR_CHAR.
 */
...
char *buildMacros(const char *cmd) { return eval_macros(_mainValues, cmd); }

char *buildCmdLine(const char **cmds, const char *p1, const char *p2,
...
hTab *getRuntimeVariables(void) { return _mainValues; }
