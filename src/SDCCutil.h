...
/** Prepend / append given strings to each item of string set. The result is in
 * a new string set.
 */
 ...
 * extension components. The file name component doesn't contain trailing
 * extension separator. Returns true if the path contains the extension
 * separator.
 */
int dbuf_splitFile(const char *path, struct dbuf_s *file, struct dbuf_s *ext);

/** Combile directory and the file name to a path string using the
 * DIR_SEPARATOR_CHAR.
 */
