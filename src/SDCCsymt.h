...
typedef struct specifier {
  ...
  bool b_implicit_sign : 1;  /* signedness not explicitly specified - needed to
                                keep char a separate type from signed char and
                                unsigned char. */
  ...
  union {                    /* Values if constant or enum */
    ...
    const TYPE_TARGET_ULONG
        *v_char32;             /*          char32_t character string      */
...
typedef enum { DECLARATOR = 1, SPECIFIER } SYM_LINK_CLASS;
#define DECLSPEC2TXT(select)                                                   \
  (select == DECLARATOR ? "DECLARATOR"                                         \
                        : select == SPECIFIER ? "SPECIFIER" : "UNKNOWN")
...
typedef struct sym_link {
  ...
  struct {
    ...
    unsigned wparam : 1;    /* first byte of arguments is passed via WREG (pic16
                               port) */
    ...
    unsigned z88dk_fastcall : 1; /* For the z80-related ports: Function has a
                                    single paramter of at most 32 bits that is
                                    passed in dehl */
    unsigned z88dk_callee : 1; /* Stack pointer adjustment for parameters passed
                                  on the stack is done by the callee */
    unsigned z88dk_shortcall : 1; /* Short call available via rst (see values
                                     later) (Z80 only) */
    unsigned
        z88dk_has_params_offset : 1; /* Has a parameter offset (Z80 only) */
    ...
    bool preserved_regs[9]; /* Registers preserved by the function - may be an
                               underestimate */
    unsigned char z88dk_shortcall_rst;  /* Rst for a short call */
    unsigned short z88dk_shortcall_val; /* Value for a short call */
    unsigned short
        z88dk_params_offset; /* Additional offset from for arguments */
  } funcAttrs;
...
typedef struct symbol {
  ...
  unsigned flexArrayLength; /* if the symbol specifies a struct
                               with a "flexible array member", then the
                               additional length in bytes for the "fam" is
                               stored here. Because the length can be different
                               from symbol to symbol AND v_struct isn't copied
                               in copyLinkChain(), it's located here in the
                               symbol and not in v_struct or the declarator */
  ...
  bool funcDivFlagSafe : 1;  /* we know this function is safe to call with
                                undocumented stm8 flag bit 6 set*/
  ...
  int bitVar; /* if bitVar != 0: this is a bit variable, bitVar is the size in
                 bits */
...
extern sym_link *validateLink(sym_link *l, const char *macro, const char *args,
                              const char select, const char *file,
                              unsigned line);
/* Easy Access Macros */
...
#define DCL_TYPE(l)                                                            \
  validateLink(l, "DCL_TYPE", #l, DECLARATOR, __FILE__, __LINE__)              \
      ->select.d.dcl_type
#define DCL_ELEM(l)                                                            \
  validateLink(l, "DCL_ELEM", #l, DECLARATOR, __FILE__, __LINE__)              \
      ->select.d.num_elem
#define DCL_PTR_CONST(l)                                                       \
  validateLink(l, "DCL_PTR_CONST", #l, DECLARATOR, __FILE__, __LINE__)         \
      ->select.d.ptr_const
#define DCL_PTR_VOLATILE(l)                                                    \
  validateLink(l, "DCL_PTR_VOLATILE", #l, DECLARATOR, __FILE__, __LINE__)      \
      ->select.d.ptr_volatile
#define DCL_PTR_RESTRICT(l)                                                    \
  validateLink(l, "DCL_PTR_RESTRICT", #l, DECLARATOR, __FILE__, __LINE__)      \
      ->select.d.ptr_restrict
#define DCL_PTR_ADDRSPACE(l)                                                   \
  validateLink(l, "DCL_PTR_ADDRSPACE", #l, DECLARATOR, __FILE__, __LINE__)     \
      ->select.d.ptr_addrspace
#define DCL_TSPEC(l)                                                           \
  validateLink(l, "DCL_TSPEC", #l, DECLARATOR, __FILE__, __LINE__)             \
      ->select.d.tspec
...
#define BANKED_FUNCTIONS false
#define IFFUNC_ISBANKEDCALL(x)                                                 \
  (IS_FUNC(x) && (FUNC_BANKED(x) || (BANKED_FUNCTIONS && !FUNC_NONBANKED(x))))

#define SPEC_NOUN(x)                                                           \
  validateLink(x, "SPEC_NOUN", #x, SPECIFIER, __FILE__, __LINE__)->select.s.noun
#define SPEC_LONG(x)                                                           \
  validateLink(x, "SPEC_LONG", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_long
#define SPEC_LONGLONG(x)                                                       \
  validateLink(x, "SPEC_LONGLONG", #x, SPECIFIER, __FILE__, __LINE__)          \
      ->select.s.b_longlong
#define SPEC_SHORT(x)                                                          \
  validateLink(x, "SPEC_LONG", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_short
#define SPEC_USIGN(x)                                                          \
  validateLink(x, "SPEC_USIGN", #x, SPECIFIER, __FILE__, __LINE__)             \
      ->select.s.b_unsigned
#define SPEC_SIGN(x)                                                           \
  validateLink(x, "SPEC_SIGN", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_signed
#define SPEC_SCLS(x)                                                           \
  validateLink(x, "SPEC_SCLS", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.sclass
#define SPEC_ENUM(x)                                                           \
  validateLink(x, "SPEC_ENUM", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_isenum
#define SPEC_OCLS(x)                                                           \
  validateLink(x, "SPEC_OCLS", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.oclass
#define SPEC_STAT(x)                                                           \
  validateLink(x, "SPEC_STAT", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_static
#define SPEC_EXTR(x)                                                           \
  validateLink(x, "SPEC_EXTR", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_extern
#define SPEC_CODE(x)                                                           \
  validateLink(x, "SPEC_CODE", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s._codesg
#define SPEC_ABSA(x)                                                           \
  validateLink(x, "SPEC_ABSA", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_absadr
#define SPEC_BANK(x)                                                           \
  validateLink(x, "SPEC_BANK", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s._regbank
#define SPEC_ADDR(x)                                                           \
  validateLink(x, "SPEC_ADDR", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s._addr
#define SPEC_STAK(x)                                                           \
  validateLink(x, "SPEC_STAK", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s._stack
#define SPEC_CVAL(x)                                                           \
  validateLink(x, "SPEC_CVAL", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.const_val
#define SPEC_BSTR(x)                                                           \
  validateLink(x, "SPEC_BSTR", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s._bitStart
#define SPEC_BLEN(x)                                                           \
  validateLink(x, "SPEC_BLEN", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s._bitLength
#define SPEC_BUNNAMED(x)                                                       \
  validateLink(x, "SPEC_BUNNAMED", #x, SPECIFIER, __FILE__, __LINE__)          \
      ->select.s.b_bitUnnamed
#define SPEC_NEEDSPAR(x)                                                       \
  validateLink(x, "SPEC_NEEDSPAR", #x, SPECIFIER, __FILE__, __LINE__)          \
      ->select.s.b_needspar
 ...
 */
#define SPEC_ISR_SAVED_BANKS(x)                                                \
  validateLink(x, "SPEC_NOUN", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s._bitStart
#define SPEC_VOLATILE(x)                                                       \
  validateLink(x, "SPEC_NOUN", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_volatile
#define SPEC_CONST(x)                                                          \
  validateLink(x, "SPEC_NOUN", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_const
#define SPEC_RESTRICT(x)                                                       \
  validateLink(x, "SPEC_NOUN", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_restrict
#define SPEC_ADDRSPACE(x)                                                      \
  validateLink(x, "SPEC_NOUN", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.addrspace
#define SPEC_STRUCT(x)                                                         \
  validateLink(x, "SPEC_NOUN", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.v_struct
#define SPEC_TYPEDEF(x)                                                        \
  validateLink(x, "SPEC_NOUN", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_typedef
#define SPEC_REGPARM(x)                                                        \
  validateLink(x, "SPEC_NOUN", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.b_isregparm
#define SPEC_ARGREG(x)                                                         \
  validateLink(x, "SPEC_NOUN", #x, SPECIFIER, __FILE__, __LINE__)              \
      ->select.s.argreg
#define SPEC_INLINE(x)                                                         \
  validateLink(x, "SPEC_INLINE", #x, SPECIFIER, __FILE__, __LINE__)            \
      ->select.s.b_inline
#define SPEC_NORETURN(x)                                                       \
  validateLink(x, "SPEC_NORETURN", #x, SPECIFIER, __FILE__, __LINE__)          \
      ->select.s.b_noreturn
#define SPEC_ALIGNAS(x)                                                        \
  validateLink(x, "SPEC_ALIGNAS", #x, SPECIFIER, __FILE__, __LINE__)           \
      ->select.s.b_alignas
...
#define IS_SMALL_PTR(x)                                                        \
  (IS_DECL(x) && (DCL_TYPE(x) == POINTER || DCL_TYPE(x) == IPOINTER ||         \
                  DCL_TYPE(x) == PPOINTER))
#define IS_PTR(x)                                                              \
  (IS_DECL(x) && (DCL_TYPE(x) == POINTER || DCL_TYPE(x) == FPOINTER ||         \
                  DCL_TYPE(x) == GPOINTER || DCL_TYPE(x) == IPOINTER ||        \
                  DCL_TYPE(x) == PPOINTER || DCL_TYPE(x) == EEPPOINTER ||      \
                  DCL_TYPE(x) == CPOINTER || DCL_TYPE(x) == UPOINTER))
...
#define IS_FUNCPTR(x)                                                          \
  (IS_DECL(x) && (DCL_TYPE(x) == CPOINTER || DCL_TYPE(x) == GPOINTER) &&       \
   IS_FUNC(x->next))
...
#define IS_INTEGRAL(x)                                                         \
  (IS_SPEC(x) &&                                                               \
   (x->select.s.noun == V_INT || x->select.s.noun == V_BOOL ||                 \
    x->select.s.noun == V_CHAR || x->select.s.noun == V_BITFIELD ||            \
    x->select.s.noun == V_BBITFIELD || x->select.s.noun == V_BIT ||            \
    x->select.s.noun == V_SBIT))
#define IS_BITFIELD(x)                                                         \
  (IS_SPEC(x) &&                                                               \
   (x->select.s.noun == V_BITFIELD || x->select.s.noun == V_BBITFIELD))
#define IS_BITVAR(x)                                                           \
  (IS_SPEC(x) &&                                                               \
   (x->select.s.noun == V_BITFIELD || x->select.s.noun == V_BBITFIELD ||       \
    x->select.s.noun == V_BIT || x->select.s.noun == V_SBIT))
#define IS_BIT(x)                                                              \
  (IS_SPEC(x) && (x->select.s.noun == V_BIT || x->select.s.noun == V_SBIT))
#define IS_BOOLEAN(x)                                                          \
  (IS_SPEC(x) &&                                                               \
   (x->select.s.noun == V_BIT || x->select.s.noun == V_SBIT ||                 \
    x->select.s.noun == V_BBITFIELD || x->select.s.noun == V_BOOL))
...
#define IS_VALID_PARAMETER_STORAGE_CLASS_SPEC(x)                               \
  (!SPEC_TYPEDEF(x) && !SPEC_EXTR(x) && !SPEC_STAT(x) && SPEC_SCLS(x) != S_AUTO)
