...
void dumpLiveRanges(int id, hTab *liveRanges) {
  ...
  for (sym = hTabFirstItem(liveRanges, &k); sym;
    ...
    fprintf(file, "%s [k%d lr%d:%d so:%d]{ re%d rm%d}",
            (sym->rname[0] ? sym->rname : sym->name), sym->key, sym->liveFrom,
            sym->liveTo, sym->stack, sym->isreqv, sym->remat);
...
void dumpEbbsToFileExt(int id, ebbIndex *ebbi) {
  ...
  for (i = 0; i < count; i++) {
    ...
    fprintf(of, "\nsuccessors: ");
    for (bb = setFirstItem(ebbs[i]->succList); bb;
         bb = setNextItem(ebbs[i]->succList)) {
      fprintf(of, "%s ", bb->entryLabel->name);
    }
    fprintf(of, "\npredecessors: ");
    for (bb = setFirstItem(ebbs[i]->predList); bb;
    ...
    for (d = 0; d < ebbs[i]->domVect->size; d++) {
      if (bitVectBitValue(ebbs[i]->domVect, d)) {
        fprintf(
            of, "%s ",
            ebbi->bbOrder[d]->entryLabel->name); // ebbs[d]->entryLabel->name);
      }
    }
    fprintf(of, "\n");
...
eBBlock *iCode2eBBlock(iCode *ic) {
  ...
  for (loop = ic->next; loop; loop = loop->next) {
    ...
    if (loop->next->op == LABEL || loop->op == GOTO || loop->op == JUMPTABLE ||
        loop->op == IFX)
      break;
  }
...
/*-----------------------------------------------------------------*/
ebbIndex *iCodeBreakDown(iCode *ic) {
  eBBlock **ebbs = NULL;
  iCode *loop = ic;
  ebbIndex *ebbi;
