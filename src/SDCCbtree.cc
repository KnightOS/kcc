...
static btree_t::vertex_descriptor
btree_lowest_common_ancestor_impl(btree_t::vertex_descriptor a,
                                  btree_t::vertex_descriptor b) {
  if (a == b)
    return (a);
  else if (a > b)
...
void btree_add_symbol(struct symbol *s) {
  int block;
  wassert(s);
  block = s->_isparm ? 0 : s->block; // This is essentially a workaround. TODO:
                                     // Ensure that the parameter block is
                                     // placed correctly in the btree instead!
...
static void btree_alloc_subtree(btree_t::vertex_descriptor v, int sPtr,
                                int cssize, int *ssize) {
  std::set<symbol *>::iterator s, s_end;
  wassert(v < boost::num_vertices(btree));
  for (s = btree[v].first.begin(), s_end = btree[v].first.end(); s != s_end;
       ++s) {
    struct symbol *const sym = *s;
    const int size = getSize(sym->type);

#ifdef BTREE_DEBUG
    std::cout << "Allocating symbol " << sym->name << " (" << v << ") of size "
              << size << " to " << sPtr << "\n";
#endif
