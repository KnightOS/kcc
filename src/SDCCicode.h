...
enum { CONDITIONAL = 0, EXPRESSION, STATEMENT, LEAF };

typedef enum { SYMBOL = 1, VALUE, TYPE } OPTYPE;
...
#define LRETYPE sym_link *letype = getSpec(ltype), *retype = getSpec(rtype);
...
extern operand *validateOpType(operand *op, const char *macro, const char *args,
                               OPTYPE type, const char *file, unsigned line);

extern const operand *validateOpTypeConst(const operand *op, const char *macro,
                                          const char *args, OPTYPE type,
                                          const char *file, unsigned line);

#define OP_SYMBOL(op)                                                          \
  validateOpType(op, "OP_SYMBOL", #op, SYMBOL, __FILE__, __LINE__)             \
      ->svt.symOperand
#define OP_SYMBOL_CONST(op)                                                    \
  validateOpTypeConst(op, "OP_SYMBOL", #op, SYMBOL, __FILE__, __LINE__)        \
      ->svt.symOperand
#define OP_VALUE(op)                                                           \
...
#define OP_SYM_TYPE(op)                                                        \
  validateOpType(op, "OP_SYM_TYPE", #op, SYMBOL, __FILE__, __LINE__)           \
      ->svt.symOperand->type
#define OP_SYM_ETYPE(op)                                                       \
  validateOpType(op, "OP_SYM_ETYPE", #op, SYMBOL, __FILE__, __LINE__)          \
      ->svt.symOperand->etype
#define SPIL_LOC(op)                                                           \
  validateOpType(op, "SPIL_LOC", #op, SYMBOL, __FILE__, __LINE__)              \
      ->svt.symOperand->usl.spillLoc
#define OP_LIVEFROM(op)                                                        \
  validateOpType(op, "OP_LIVEFROM", #op, SYMBOL, __FILE__, __LINE__)           \
      ->svt.symOperand->liveFrom
#define OP_LIVETO(op)                                                          \
  validateOpType(op, "OP_LIVETO", #op, SYMBOL, __FILE__, __LINE__)             \
      ->svt.symOperand->liveTo
#define OP_REQV(op)                                                            \
  validateOpType(op, "OP_REQV", #op, SYMBOL, __FILE__, __LINE__)               \
      ->svt.symOperand->reqv
#define OP_KEY(op)                                                             \
  validateOpType(op, "OP_REQV", #op, SYMBOL, __FILE__, __LINE__)               \
      ->svt.symOperand->key
#define OP_TYPE(op)                                                            \
  validateOpType(op, "OP_TYPE", #op, TYPE, __FILE__, __LINE__)->svt.typeOperand
...
typedef struct iCode {
  ...
  bitVect
      *rSurv; /* registers that survive this instruction (i.e. they are in use,
                 it is not their last use and they are not in the return) */
  union {
    struct {
...
/* useful macros */
#define SKIP_IC2(x)                                                            \
  (x->op == GOTO || x->op == LABEL || x->op == FUNCTION ||                     \
   x->op == INLINEASM || x->op == ENDFUNCTION)

#define SKIP_IC1(x) (x->op == CALL || SKIP_IC2(x))

#define SKIP_IC(x)                                                             \
  (x->op == PCALL || x->op == IPUSH || x->op == IPOP || x->op == JUMPTABLE ||  \
   x->op == RECEIVE || x->op == ARRAYINIT || SKIP_IC1(x) ||                    \
   x->op == CRITICAL || x->op == ENDCRITICAL || x->op == SEND)

#define SKIP_IC3(x) (SKIP_IC2(x) || x->op == JUMPTABLE)

#define IS_CONDITIONAL(x)                                                      \
  (x->op == EQ_OP || x->op == '<' || x->op == '>' || x->op == LE_OP ||         \
   x->op == GE_OP || x->op == NE_OP)

#define IS_TRUE_SYMOP(op) (op && IS_SYMOP(op) && !IS_ITEMP(op))

#define POINTER_SET(ic)                                                        \
  (ic && ic->op == '=' &&                                                      \
   (IS_ITEMP(IC_RESULT(ic)) || IS_OP_LITERAL(IC_RESULT(ic))) &&                \
   IC_RESULT(ic)->isaddr)

#define POINTER_GET(ic)                                                        \
  (ic && ic->op == GET_VALUE_AT_ADDRESS &&                                     \
   (IS_ITEMP(IC_LEFT(ic)) || IS_OP_LITERAL(IC_LEFT(ic))) &&                    \
   IC_LEFT(ic)->isaddr)

#define IS_ARITHMETIC_OP(x)                                                    \
  (x && (x->op == '+' || x->op == '-' || x->op == '/' || x->op == '*' ||       \
         x->op == '%'))
#define IS_BITWISE_OP(x)                                                       \
  (x && (x->op == BITWISEAND || x->op == '|' || x->op == '^'))

#define IS_COMMUTATIVE(x)                                                      \
  (x && (x->op == EQ_OP || x->op == NE_OP || x->op == '+' || x->op == '*' ||   \
         x->op == BITWISEAND || x->op == '|' || x->op == '^'))

#define ASSIGNMENT(ic) (ic && ic->op == '=')

#define ASSIGN_SYM_TO_ITEMP(ic)                                                \
  (ic && ic->op == '=' && IS_TRUE_SYMOP(IC_RIGHT(ic)) &&                       \
   IS_ITEMP(IC_RESULT(ic)))

#define ASSIGN_ITEMP_TO_SYM(ic)                                                \
  (ic && ic->op == '=' && IS_TRUE_SYMOP(IC_RESULT(ic)) &&                      \
   IS_ITEMP(IC_RIGHT(ic)))

#define ASSIGN_ITEMP_TO_ITEMP(ic)                                              \
  (ic && ic->op == '=' && !POINTER_SET(ic) && IS_ITEMP(IC_RIGHT(ic)) &&        \
   IS_ITEMP(IC_RESULT(ic)))

#define ADD_SUBTRACT_ITEMP(ic)                                                 \
  (ic && (ic->op == '+' || ic->op == '-') && IS_ITEMP(IC_RESULT(ic)) &&        \
   ((IS_ITEMP(IC_LEFT(ic))) || (IS_SYMOP(IC_LEFT(ic)))) &&                     \
   IS_OP_LITERAL(IC_RIGHT(ic)))

#define ASSIGNMENT_TO_SELF(ic)                                                 \
  (!POINTER_SET(ic) && !POINTER_GET(ic) && ic->op == '=' &&                    \
   IC_RESULT(ic)->key == IC_RIGHT(ic)->key)
...
#define OP_DEFS(op)                                                            \
  validateOpType(op, "OP_DEFS", #op, SYMBOL, __FILE__, __LINE__)               \
      ->svt.symOperand->defs
#define OP_USES(op)                                                            \
  validateOpType(op, "OP_USES", #op, SYMBOL, __FILE__, __LINE__)               \
      ->svt.symOperand->uses
/*-----------------------------------------------------------------*/
