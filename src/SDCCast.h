...
typedef enum { EX_OP = 0, EX_VALUE, EX_LINK, EX_OPERAND } ASTTYPE;
...
#define IS_BITAND(x)                                                           \
  (IS_AST_OP(x) && (x)->opval.op == '&' && (x)->left && (x)->right)
...
#define IS_COMPARE_OP(x)                                                       \
  (IS_AST_OP(x) && ((x)->opval.op == '>' || (x)->opval.op == '<' ||            \
                    (x)->opval.op == LE_OP || (x)->opval.op == GE_OP ||        \
                    (x)->opval.op == EQ_OP || (x)->opval.op == NE_OP))
...
#define IS_ADDRESS_OF_OP(x)                                                    \
  (IS_AST_OP(x) && (x)->opval.op == '&' && (x)->right == NULL)
#define IS_AST_LIT_VALUE(x)                                                    \
  (IS_AST_VALUE(x) && IS_LITERAL((x)->opval.val->etype))
...
#define CAN_EVAL(x)                                                            \
  ((x) == '[' || (x) == '.' || (x) == PTR_OP || (x) == '&' || (x) == '|' ||    \
   (x) == '^' || (x) == '*' || (x) == '-' || (x) == '+' || (x) == '~' ||       \
   (x) == '!' || (x) == LEFT_OP || (x) == RIGHT_OP || (x) == '/' ||            \
   (x) == '%' || (x) == '>' || (x) == '<' || (x) == LE_OP || (x) == GE_OP ||   \
   (x) == EQ_OP || (x) == NE_OP || (x) == AND_OP || (x) == OR_OP ||            \
   (x) == '=')
...
#define IS_MODIFYING_OP(x)                                                     \
  ((x) == INC_OP || (x) == DEC_OP || (x) == '=' || (x) == AND_ASSIGN ||        \
   (x) == OR_ASSIGN || (x) == XOR_ASSIGN)

#define IS_ASSIGN_OP(x)                                                        \
  ((x) == '=' || (x) == ADD_ASSIGN || (x) == SUB_ASSIGN ||                     \
   (x) == MUL_ASSIGN || (x) == DIV_ASSIGN || (x) == XOR_ASSIGN ||              \
   (x) == AND_ASSIGN || (x) == OR_ASSIGN || (x) == INC_OP || (x) == DEC_OP)
#define IS_DEREF_OP(x)                                                         \
  (((x)->opval.op == '*' && (x)->right == NULL) || (x)->opval.op == '.' ||     \
   (x)->opval.op == PTR_OP)
...
ast *createFor(symbol *, symbol *, symbol *, symbol *, ast *, ast *, ast *,
               ast *, ast *);
void eval2icode(ast *);
