...
void freeLocalCseDef(void *item) {
...
int isCseDefEqual(void *vsrc, void *vdest) {
  ...
  return (src->key == dest->key && src->diCode == dest->diCode);
}
...
void replaceAllSymBySym(iCode *ic, operand *from, operand *to,
  ...
  for (lic = ic; lic; lic = lic->next) {
    ...
    if (lic->op == IFX) {
      if (IS_SYMOP(to) && IC_COND(lic)->key == from->key) {
    ...
    if (lic->op == JUMPTABLE) {
      if (IS_SYMOP(to) && IC_JTCOND(lic)->key == from->key) {
    ...
    if (IS_SYMOP(to) && IC_RESULT(lic) && IC_RESULT(lic)->key == from->key) {
      /* maintain du chains */
      if (POINTER_SET(lic)) {
        bitVectUnSetBit(OP_USES(from), lic->key);
        OP_USES(to) = bitVectSetBit(OP_USES(to), lic->key);
    ...
    if (IS_SYMOP(to) && IC_RIGHT(lic) && IC_RIGHT(lic)->key == from->key) {
      bitVectUnSetBit(OP_USES(from), lic->key);
      OP_USES(to) = bitVectSetBit(OP_USES(to), lic->key);
    ...
    if (IS_SYMOP(to) && IC_LEFT(lic) && IC_LEFT(lic)->key == from->key) {
      bitVectUnSetBit(OP_USES(from), lic->key);
      OP_USES(to) = bitVectSetBit(OP_USES(to), lic->key);
...
DEFSETFUNC(findCheaperOp) {
      ...
      if (IS_OP_VOLATILE(IC_RESULT(cdp->diCode))) {
        *opp = IC_RESULT(cdp->diCode);
        ...
        /* if this is a straight assignment and
           ...
           true symbol */
        if (!POINTER_SET(cdp->diCode) && IS_ITEMP(IC_RESULT(cdp->diCode)) &&
  ...
  if (IS_TRUE_SYMOP(cop) && cdp->diCode->op == '=' &&
      !POINTER_SET(cdp->diCode) && cop->key == IC_RIGHT(cdp->diCode)->key &&
  ...
  if ((*opp) &&
      (isOperandLiteral(*opp) || !checkSign ||
       (checkSign && IS_SPEC(operandType(cop)) && IS_SPEC(operandType(*opp)) &&
    ...
    if (IS_SPEC(operandType(cop)) && IS_SPEC(operandType(*opp)) &&
      ...
      if (isOperandLiteral(*opp) && SPEC_NOUN(operandType(*opp)) == V_CHAR &&
...
DEFSETFUNC(findPrevIc) {
  ...
  if (ic->op == cdp->diCode->op && IS_COMMUTATIVE(ic) &&
...
DEFSETFUNC(ifOperandsHave) {
  ...
  if (IC_LEFT(cdp->diCode) && IS_SYMOP(IC_LEFT(cdp->diCode)) &&
      IC_LEFT(cdp->diCode)->key == op->key)
    return 1;

  if (IC_RIGHT(cdp->diCode) && IS_SYMOP(IC_RIGHT(cdp->diCode)) &&
      IC_RIGHT(cdp->diCode)->key == op->key)
    return 1;

  if (POINTER_SET(cdp->diCode) && IS_SYMOP(IC_RESULT(cdp->diCode)) &&
      IC_RESULT(cdp->diCode)->key == op->key)
    return 1;

  /* or if any of the operands are volatile */
  if (IC_LEFT(cdp->diCode) && IS_OP_VOLATILE(IC_LEFT(cdp->diCode)))
    return 1;

  if (IC_RIGHT(cdp->diCode) && IS_OP_VOLATILE(IC_RIGHT(cdp->diCode)))
    return 1;

  if (IC_RESULT(cdp->diCode) && IS_OP_VOLATILE(IC_RESULT(cdp->diCode)))
    return 1;
...
/*-----------------------------------------------------------------*/
static void algebraicOpts(iCode *ic, eBBlock *ebp) {
  /* we don't deal with the following iCodes
     here */
  if (ic->op == IFX || ic->op == IPUSH || ic->op == IPOP || ic->op == CALL ||
      ic->op == PCALL || ic->op == RETURN || POINTER_GET(ic))
    return;

  /* if both operands present & ! IFX */
  /* then if they are both literal we */
  /* perform the operation right now  */
  if (IC_RESULT(ic) && IC_RIGHT(ic) && IC_LEFT(ic) &&
      IS_OP_LITERAL(IC_LEFT(ic)) && IS_OP_LITERAL(IC_RIGHT(ic))) {
    IC_RIGHT(ic) = operandOperation(IC_LEFT(ic), IC_RIGHT(ic), ic->op,
  ...
  if (IC_RESULT(ic) && IC_LEFT(ic) && IS_OP_LITERAL(IC_LEFT(ic)) &&
      !IC_RIGHT(ic)) {
    IC_RIGHT(ic) = operandOperation(IC_LEFT(ic), IC_RIGHT(ic), ic->op,
                                    operandType(IC_RESULT(ic)));
    ic->op = '=';
  ...
  if (ic->op == GET_VALUE_AT_ADDRESS) {
    if (IS_ITEMP(IC_RESULT(ic)) && IS_TRUE_SYMOP(IC_LEFT(ic))) {
      ic->op = '=';
    ...
    if (IS_ITEMP(IC_LEFT(ic)) && IS_ITEMP(IC_RESULT(ic)) &&
        !IC_LEFT(ic)->isaddr) {
      ic->op = '=';
      IC_RIGHT(ic) = operandFromOperand(IC_LEFT(ic));
      IC_RIGHT(ic)->isaddr = 0;
  ...
  case '+':
    ...
    if (IC_LEFT(ic)->key == IC_RIGHT(ic)->key && !IS_OP_VOLATILE(IC_LEFT(ic)) &&
    ...
    if (IS_OP_LITERAL(IC_LEFT(ic)) && isEqualVal(OP_VALUE(IC_LEFT(ic)), 0)) {
      int typematch;
      typematch =
          compareType(operandType(IC_RESULT(ic)), operandType(IC_RIGHT(ic)));
    ...
    if (IS_OP_LITERAL(IC_RIGHT(ic)) && isEqualVal(OP_VALUE(IC_RIGHT(ic)), 0)) {
      int typematch;
      typematch =
          compareType(operandType(IC_RESULT(ic)), operandType(IC_LEFT(ic)));
  ...
  case '*':
    ...
    if (IS_OP_LITERAL(IC_RIGHT(ic))) {
      ...
      if (isEqualVal(OP_VALUE(IC_RIGHT(ic)), 1)) {
        /* '*' can have two unsigned chars as operands */
        /* and an unsigned int as result.              */
        if (compareType(operandType(IC_RESULT(ic)), operandType(IC_LEFT(ic))) ==
            1) {
          ic->op = '=';
          IC_RIGHT(ic) = IC_LEFT(ic);
          IC_LEFT(ic) = NULL;
  ...
  case CAST: {
    ...
    if (IS_OP_LITERAL(IC_RIGHT(ic))) {
      ...
      if (IS_GENPTR(ctype) && IS_PTR(otype)) {
        ...
        const char *name =
            IS_SYMOP(IC_RESULT(ic)) ? OP_SYMBOL(IC_RESULT(ic))->name : NULL;
        if (IS_FUNCPTR(otype))
      ...
      IC_RIGHT(ic) = operandFromValue(
          valCastLiteral(operandType(IC_LEFT(ic)), litval, litval));
      IC_LEFT(ic) = NULL;
      SET_ISADDR(IC_RESULT(ic), 0);
    }
    /* if casting to the same */
    if (compareType(operandType(IC_RESULT(ic)), operandType(IC_RIGHT(ic))) ==
        1) {
      ic->op = '=';
      IC_LEFT(ic) = NULL;
      SET_ISADDR(IC_RESULT(ic), 0);
  ...
  case '!':
    if (IS_OP_LITERAL(IC_LEFT(ic))) {
      ic->op = '=';
      IC_RIGHT(ic) = (isEqualVal(OP_VALUE(IC_LEFT(ic)), 0) ? operandFromLit(1)
                                                           : operandFromLit(0));
      IC_LEFT(ic) = NULL;
      SET_ISADDR(IC_RESULT(ic), 0);
    }
  ...
  case BITWISEAND:
    ...
    if (IS_OP_LITERAL(IC_RIGHT(ic))) {
      ...
      /* if yes turn it into assignment */
      if (IS_BOOLEAN(operandType(IC_RIGHT(
              ic)))) /* Special handling since _Bool is stored in 8 bits */
        goto boolcase;
  ...
  case '^':
    ...
    if (IS_OP_LITERAL(IC_RIGHT(ic)) && IS_BOOLEAN(operandType(IC_LEFT(ic))) &&
...
void updateSpillLocation(iCode *ic, int induction) {
  ...
  if (ASSIGN_ITEMP_TO_ITEMP(ic)) {
    if (!SPIL_LOC(IC_RIGHT(ic)) &&
      ...
      if (!OP_SYMBOL(IC_RIGHT(ic))->noSpilLoc && !IS_VOLATILE(setype) &&
          !IN_FARSPACE(SPEC_OCLS(setype)) &&
          !OTHERS_PARM(OP_SYMBOL(IC_RESULT(ic)))) {
    ...
    if (induction && OP_SYMBOL(IC_RIGHT(ic))->isreqv &&
        !OP_SYMBOL(IC_RESULT(ic))->noSpilLoc && !SPIL_LOC(IC_RESULT(ic))) {
...
/*-----------------------------------------------------------------*/
void setUsesDefs(operand *op, bitVect *bdefs, bitVect *idefs, bitVect **oud) {
...
/*-----------------------------------------------------------------*/
void ifxOptimize(iCode *ic, set *cseSet, int computeOnly, eBBlock *ebb,
                 int *change, ebbIndex *ebbi) {
  operand *pdop;
  symbol *label;
  ...
  if (!computeOnly) {
    ...
    if (pdop) {
      ...
      if (!ic->prev->prev || ic->prev->prev->op != '-' ||
          !IS_OP_LITERAL(IC_RIGHT(ic->prev->prev))) {
        sym_link *type = operandType(IC_RESULT(ic->prev));
        if (ic->prev->op != CAST || IS_BOOL(type) ||
            bitsForType(operandType(IC_RIGHT(ic->prev))) < bitsForType(type)) {
          if (!isOperandVolatile(ic->prev->op == '!' ? IC_LEFT(ic->prev)
                                                     : IC_RIGHT(ic->prev),
                                 FALSE)) {
            ...
            bitVectUnSetBit(OP_USES(IC_COND(ic)), ic->key);
            ReplaceOpWithCheaperOp(&IC_COND(ic), ic->prev->op == '!'
                                                     ? IC_LEFT(ic->prev)
                                                     : IC_RIGHT(ic->prev));
            (*change)++;
          }
...
int constFold(iCode *ic, set *cseSet) {
  ...
  /* deal with only + & - */
  if (ic->op != '+' && ic->op != '-' && ic->op != BITWISEAND)
    return 0;
  ...
  if (ic->op == BITWISEAND) /* Optimize out bitwise and of comparion results */
  {
    ...
    if (dic->op != EQ_OP && dic->op != NE_OP && dic->op != LE_OP &&
        dic->op != GE_OP && dic->op != '<' && dic->op != '>' && dic->op != '!')
      return 0;

    IC_RIGHT(ic) = (operandLitValueUll(IC_RIGHT(ic)) & 1) ? IC_LEFT(ic)
                                                          : operandFromLit(0);
...
static int boolCast(iCode *ic, set *cseSet) {
  iCode *dic = NULL;
  ...
  case GETABIT:
    break;
  case BITWISEAND:
    if (IS_BOOL(operandType(IC_LEFT(dic))) ||
...
/*-----------------------------------------------------------------*/
static void deleteGetPointers(set **cseSet, set **pss, operand *op,
                              eBBlock *ebb) {
  set *compItems = NULL;
  cseDef *cdp;
  operand *cop;
...
static void fixUpTypes(iCode *ic) {
  ...
  if (IS_PTR(t1) && compareType(t2 = operandType(IC_RESULT(ic)), t1) == 1) {
...
int cseBBlock(eBBlock *ebb, int computeOnly, ebbIndex *ebbi) {
  ...
  for (ic = ebb->sch; ic; ic = ic->next) {
    ...
    if ((ic->op == PCALL || ic->op == IPUSH || ic->op == IPOP ||
         ic->op == SEND) &&
        IS_SYMOP(IC_LEFT(ic))) {
    ...
    if (ic->op == JUMPTABLE) {
      if (IS_SYMOP(IC_JTCOND(ic))) {
        OP_USES(IC_JTCOND(ic)) = bitVectSetBit(OP_USES(IC_JTCOND(ic)), ic->key);
        setUsesDefs(IC_JTCOND(ic), ebb->defSet, ebb->outDefs, &ebb->usesDefs);
      }
      continue;
    }
    ...
    if (POINTER_GET(ic)) {
      if (!IS_PTR(operandType(IC_LEFT(ic)))) {
        setOperandType(IC_LEFT(ic), aggrToPtr(operandType(IC_LEFT(ic)), FALSE));
        IC_LEFT(ic)->aggr2ptr = 0;
        fixUpTypes(ic);
      } else if (IC_LEFT(ic)->aggr2ptr == 1) { /* band aid for kludge */
        setOperandType(IC_LEFT(ic), aggrToPtr(operandType(IC_LEFT(ic)), TRUE));
        IC_LEFT(ic)->aggr2ptr++;
        fixUpTypes(ic);
      }
    }
    ...
    if (POINTER_SET(ic)) {
      ...
      } else if (IC_RESULT(ic)->aggr2ptr == 1) { /* band aid for kludge */
        setOperandType(IC_RESULT(ic),
    ...
    if (!computeOnly && ic->op != ADDRESS_OF && IS_SYMOP(IC_LEFT(ic)) &&
    ...
    if (!computeOnly && POINTER_SET(ic) && IS_SYMOP(IC_RESULT(ic)) &&
    ...
    if (!(POINTER_GET(ic) &&
          (IS_BITFIELD(OP_SYMBOL(IC_RESULT(ic))->etype) ||
           ...
           IS_VOLATILE(operandType(IC_LEFT(ic))->next) ||
           bitVectBitValue(ebb->ndompset, IC_LEFT(ic)->key))) &&
        !ASSIGNMENT(ic) && IS_ITEMP(IC_RESULT(ic)) && !computeOnly) {
    ...
    if (ASSIGNMENT(ic) && IS_SYMOP(IC_RESULT(ic)) &&
        OTHERS_PARM(OP_SYMBOL(IC_RESULT(ic))) &&
        IS_PTR(operandType(IC_RESULT(ic)))) {
    ...
    if (POINTER_GET(ic) && !(IS_BITFIELD(OP_SYMBOL(IC_RESULT(ic))->etype) ||
                             isOperandVolatile(IC_LEFT(ic), TRUE))) {
      pdop = NULL;
    ...
    if (!(POINTER_SET(ic)) && IS_SYMOP(IC_RESULT(ic))) {
      ...
      if (isOperandGlobal(IC_RESULT(ic)) ||
        ...
        destructItemIf(&cseSet, freeLocalCseDef, ifAnyUnrestrictedGetPointer,
                       map->ptrType);
    ...
    if (POINTER_SET(defic) &&
      ...
      if (IS_SYMOP(IC_RESULT(ic))) {
        OP_USES(IC_RESULT(ic)) = bitVectSetBit(OP_USES(IC_RESULT(ic)), ic->key);
        setUsesDefs(IC_RESULT(ic), ebb->defSet, ebb->outDefs, &ebb->usesDefs);
      }
      ...
      if (!IS_PTR_RESTRICT(ptype)) {
        destructItemIf(&cseSet, freeLocalCseDef, ifDefGlobalAliasableByPtr,
                       DCL_TYPE(ptype));
        destructItemIf(&cseSet, freeLocalCseDef, ifFromGlobalAliasableByPtr,
                       DCL_TYPE(ptype));
      }
  ...
  for (expr = setFirstItem(ebb->inExprs); expr;
       expr = setNextItem(ebb->inExprs))
