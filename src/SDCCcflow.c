...
/*-----------------------------------------------------------------*/
static set *domSetFromVect(ebbIndex *ebbi, bitVect *domVect) {
  int i = 0;
  set *domSet = NULL;
...
/*                the next one :                                   */
/*-----------------------------------------------------------------*/
static void addSuccessor(eBBlock *thisBlock, eBBlock *succ) {
  ...
  thisBlock->succVect = bitVectSetBit(thisBlock->succVect, succ->bbnum);
...
/*-----------------------------------------------------------------*/
static void eBBPredecessors(ebbIndex *ebbi) {
...
/*-----------------------------------------------------------------*/
static void eBBSuccessors(ebbIndex *ebbi) {
  eBBlock **ebbs = ebbi->bbOrder;
  int count = ebbi->count;
  int i = 0;
  ...
  for (; i < count; i++) {
    ...
    if (ebbs[i + 1]) {
      ...
      if (ebbs[i]->ech) {
        ...
        if (!foundNoReturn && ebbs[i]->ech->op != GOTO &&
            ebbs[i]->ech->op != RETURN && ebbs[i]->ech->op != JUMPTABLE) {
          int j = i + 1;
    ...
    if ((ic = ebbs[i]->ech)) {
      ...
      if (ic->op == JUMPTABLE) {
        ...
        for (lbl = setFirstItem(IC_JTLABELS(ic)); lbl;
          ...
          addSuccessor(ebbs[i], eBBWithEntryLabel(ebbi, lbl));
      } else {
...
/*-----------------------------------------------------------------*/
static void computeDominance(ebbIndex *ebbi) {
  eBBlock **ebbs = ebbi->bbOrder;
  int count = ebbi->count;
  int i, j;
  ...
  for (i = 1; i < count; i++) {
    ...
    for (j = 0; j < count; j++) {
      ebbs[i]->domVect = bitVectSetBit(ebbs[i]->domVect, ebbs[j]->bbnum);
    }
  }
...
/*-----------------------------------------------------------------*/
eBBlock *immedDom(ebbIndex *ebbi, eBBlock *ebp) {
...
/*                     flowgraph                                   */
/*-----------------------------------------------------------------*/
static void computeDFOrdering(eBBlock *ebbp, int *count) {
...
/*-----------------------------------------------------------------*/
static void markNoPath(ebbIndex *ebbi) {
  eBBlock **ebbs = ebbi->bbOrder;
  int count = ebbi->count;
  int i;
