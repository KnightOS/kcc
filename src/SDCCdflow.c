...
DEFSETFUNC(ifKilledInBlock) {
  ...
  if (POINTER_GET(cdp->diCode) &&
      bitVectBitValue(src->ptrsSet, IC_LEFT(cdp->diCode)->key))
    return 1;

  /* if assignment to iTmep then if right is defined
     elsewhere kill this one */
  if (ASSIGNMENT(cdp->diCode) && !POINTER_SET(cdp->diCode) &&
      IS_ITEMP(IC_RESULT(cdp->diCode)) && IS_SYMOP(IC_RIGHT(cdp->diCode)) &&
  ...
  bitVectUnSetBit(outs = bitVectCopy(src->outDefs), cdp->diCode->key);
...
DEFSETFUNC(mergeInExprs) {
  ...
  dest->killedExprs =
      unionSets(dest->killedExprs, ebp->killedExprs, THROW_DEST);
  ...
  if (bitVectBitValue(dest->domVect, ebp->bbnum) && dest != ebp) {
    ...
    // if (dest != ebp)
    //  dest->inExprs = intersectSets (dest->inExprs, ebp->outExprs,
    //  THROW_DEST);
...
void computeDataFlow(ebbIndex *ebbi) {
  ...
  do {
    ...
    for (i = 0; i < count; i++) {
      ...
      for (pBlock = setFirstItem(pred); pBlock; pBlock = setNextItem(pred)) {
      ...
      if (optimize.global_cse) {
        change +=
            !isSetsEqualWith(ebbs[i]->outExprs, oldOutExprs, isCseDefEqual);
        change += !isSetsEqualWith(ebbs[i]->killedExprs, oldKilledExprs,
                                   isCseDefEqual);
      }
...
int usedBetweenPoints(operand *op, iCode *start, iCode *end) {
  ...
  for (; lic != end; lic = lic->next) {
    ...
    if (IS_PARM(op) && (lic->op == CALL || lic->op == PCALL)) {
      value *args;
      if (lic->op == CALL) {
    ...
    if (lic->op == IFX && IC_COND(lic)->key == op->key)
      return 1;

    if (lic->op == JUMPTABLE && IC_JTCOND(lic)->key == op->key)
      return 1;

    if (IC_RIGHT(lic) && IC_RIGHT(lic)->key == op->key)
      return 1;

    if (IC_LEFT(lic) && IC_LEFT(lic)->key == op->key)
      return 1;

    /* for a pointer assignment usage */
    if (POINTER_SET(lic) && op->key == IC_RESULT(lic)->key)
      return 1;
...
iCode *usedInRemaining(operand *op, iCode *ic) {
  ...
  for (; lic; lic = lic->next) {
    ...
    if (lic->op == CALL || lic->op == PCALL) {
      ...
      if ((IS_PARM(op) && isParameterToCall(args, op)) || isOperandGlobal(op))
        return lic;
    }

    if (ic->op == SEND && isOperandEqual(IC_LEFT(lic), op))
      return lic;
    ...
    if (IC_LEFT(lic) && isOperandEqual(IC_LEFT(lic), op))
      return lic;
