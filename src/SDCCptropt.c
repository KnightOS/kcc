...
static int pattern1(iCode *sic) {
  /* this is what we do. look for sequences like
     ...
     sic->next _SOME_POINTER_ := iTempY; sic->next->next either iTempZ :=
     @[iTempX]; sic->next->next->next or
     *(iTempX) := ..something.. sic->next->next->next if we find this then
     transform this to iTempX := _SOME_POINTER_; either iTempZ := @[iTempX]; or
     *(iTempX) := ..something..
     iTempY := _SOME_POINTER_ + nn ;   nn  = sizeof (pointed to object)
...
static int pattern2(iCode *sic) {
  /* this is what we do. look for sequences like
     ...
     iTempX := _SOME_POINTER_;
     iTempY := _SOME_POINTER_ + nn ;   nn  = sizeof (pointed to object)
     sic->next iTempK := iTempY; sic->next->next _SOME_POINTER_ := iTempK;
     sic->next->next->next either iTempZ := @[iTempX];
     sic->next->next->next->next or
     *(iTempX) := ..something.. sic->next->next->next->next if we find this then
     transform this to iTempX := _SOME_POINTER_; either iTempZ := @[iTempX]; or
     *(iTempX) := ..something..
     iTempY := _SOME_POINTER_ + nn ;   nn  = sizeof (pointed to object)
     iTempK := iTempY;
  ...
  if (!IS_ASSIGN_ICODE(sic->next->next->next))
    return 0;
  if (!isOperandEqual(IC_RIGHT(sic->next->next->next),
                      IC_RESULT(sic->next->next)))
    return 0;
...
/*--------------------------------------------------------------------*/
symbol *ptrBaseRematSym(symbol *ptrsym) {
  iCode *ric;
    ...
    if (!(SKIP_IC3(ic) || ic->op == IFX)) {
      ...
      /* Check for hazard #2 */
        ...
        if (ptrsym2->remat) {
          ...
          if (ptrsym2DclType == ptrsymDclType || ptrsym2DclType == GPOINTER ||
