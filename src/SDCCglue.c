...
set *ccpStr =
    NULL; /* char * const pointers with a string literal initializer */
...
char *aopLiteralGptr(const char *name, value *val) {
  ...
  if (IS_FUNCPTR(val->type))
    dbuf_tprintf(
        &dbuf, "!immedbyte",
        v | pointerTypeToGPByte(DCL_TYPE(val->type->next), val->name, name));
  else if (IS_PTR(val->type) && !IS_GENPTR(val->type))
    dbuf_tprintf(&dbuf, "!immedbyte",
                 pointerTypeToGPByte(DCL_TYPE(val->type), val->name, name));
  else
...
static void emitRegularMap(memmap *map, bool addPublics, bool arFlag) {
  ...
  for (sym = setFirstItem(map->syms); sym; sym = setNextItem(map->syms)) {
    ...
    if (arFlag && !IS_AGGREGATE(sym->type) &&
        !(sym->_isparm && !IS_REGPARM(sym->etype)) && !sym->allocreq &&
        sym->level)
      continue;
    ...
    if ((sym->level == 0 || (sym->_isparm && !IS_REGPARM(sym->etype) &&
                             !IS_STATIC(sym->localof->etype))) &&
        addPublics && !IS_STATIC(sym->etype) &&
        (IS_FUNC(sym->type) ? (sym->used || IFFUNC_HASBODY(sym->type))
                            : (!IS_EXTERN(sym->etype) || sym->ival)) &&
        !(IFFUNC_ISINLINE(sym->type) && !IS_STATIC(sym->etype) &&
          !IS_EXTERN(sym->etype))) {
    ...
    if (sym->ival && sym->level == 0) {
      if ((SPEC_OCLS(sym->etype) == xidata ||
          ...
          !SPEC_ABSA(sym->etype) && !SPEC_ADDRSPACE(sym->etype)) {
        sym_link *t;
        ast *ival = NULL;
        ...
        if (IS_AGGREGATE(tsym->type)) {
          ...
          if (getNelements(tsym->type, tsym->ival) > 1) {
            werrorfl(tsym->fileDef, tsym->lineDef, W_EXCESS_INITIALIZERS,
                     "scalar", tsym->name);
          }
          ival = newNode('=', newAst_VALUE(symbolVal(tsym)),
                         decorateType(resolveSymbols(list2expr(tsym->ival)),
                                      RESULT_TYPE_NONE));
        }
        if (ival) {
          // set ival's lineno to where the symbol was defined
          setAstFileLine(ival, filename = tsym->fileDef,
                         lineno = tsym->lineDef);
          // check if this is not a constant expression
          if (!constExprTree(ival)) {
            werrorfl(ival->filename, ival->lineno, E_CONST_EXPECTED,
                     "found expression");
        ...
        SPEC_OCLS(newSym->etype) =
            (SPEC_OCLS(sym->etype) == xidata) ? xinit : initializer;
        SNPRINTF(newSym->name, sizeof(newSym->name), "__xinit_%s", sym->name);
        SNPRINTF(newSym->rname, sizeof(newSym->rname), "__xinit_%s",
                 sym->rname);
        ...
        addSet((SPEC_OCLS(sym->etype) == xidata) ? &xinit->syms
                                                 : &initializer->syms,
               newSym);
        ...
        if (!SPEC_ABSA(sym->etype)) {
          ...
          for (ps = setFirstItem(statsg->syms); ps;
               ps = setNextItem(statsg->syms))
        ...
        if (IS_AGGREGATE(sym->type)) {
          ...
          if (getNelements(sym->type, sym->ival) > 1) {
            werrorfl(sym->fileDef, sym->lineDef, W_EXCESS_INITIALIZERS,
                     "scalar", sym->name);
          }
...
value *initValPointer(ast *expr) {
  ...
  if (IS_AST_OP(expr) && (expr->opval.op == '+' || expr->opval.op == '-') &&
      IS_CAST_OP(expr->left) && IS_ADDRESS_OF_OP(expr->left->right) &&
      IS_AST_LIT_VALUE(expr->right)) {
    return valForCastAggr(expr->left->right->left, expr->left->left->opval.lnk,
...
/*-----------------------------------------------------------------*/
value *initPointer(initList *ilist, sym_link *toType, int showError) {
  value *val;
  ast *expr;
  ...
  if (IS_AST_OP(expr) && (expr->opval.op == '+' || expr->opval.op == '-') &&
    ...
    return valForCastAggr(expr->left, expr->left->ftype, expr->right,
                          expr->opval.op);
  }
...
/*-----------------------------------------------------------------*/
int pointerTypeToGPByte(const int p_type, const char *iname,
                        const char *oname) {
  switch (p_type) {
  case IPOINTER:
  case POINTER:
    return GPTYPE_NEAR;
  case GPOINTER:
    werror(W_USING_GENERIC_POINTER, iname ? iname : "<null>",
           oname ? oname : "<null>");
    return -1;
  case FPOINTER:
    return GPTYPE_FAR;
...
/*-----------------------------------------------------------------*/
static void printIvalVal(struct dbuf_s *oBuf, value *val, int size,
                         bool newline) {
...
/*-----------------------------------------------------------------*/
static void _printPointerType(struct dbuf_s *oBuf, const char *name, int size) {
  if (size == 4) {
    if (port->little_endian)
      dbuf_printf(oBuf, "\t.byte %s, (%s >> 8), (%s >> 16), (%s >> 24)", name,
                  name, name, name);
    else
      dbuf_printf(oBuf, "\t.byte (%s >> 24), (%s >> 16), (%s >> 8), %s", name,
                  name, name, name);
  } else if (size == 3) {
    if (port->little_endian)
...
/*-----------------------------------------------------------------*/
static void printPointerType(struct dbuf_s *oBuf, const char *name) {
...
/*-----------------------------------------------------------------*/
static void printGPointerType(struct dbuf_s *oBuf, const char *iname,
                              const char *oname, int type) {
...
/*-----------------------------------------------------------------*/
void printIvalType(symbol *sym, sym_link *type, initList *ilist,
                   struct dbuf_s *oBuf) {
  value *val;
  ...
  if (!(val = list2val(ilist, FALSE))) {
    if (!!(val = initPointer(ilist, type, 0))) {
      int i, size = getSize(type), le = port->little_endian,
             top = (options.model == MODEL_FLAT24) ? 3 : 2;
      ;
      dbuf_printf(oBuf, "\t.byte ");
      for (i = (le ? 0 : size - 1); le ? (i < size) : (i > -1);
           i += (le ? 1 : -1)) {
        if (i == 0)
  ...
  if (val->type != type) {
    val = valCastLiteral(type, floatFromVal(val),
                         (TYPE_TARGET_ULONGLONG)ullFromVal(val));
  }
  ...
  case 1:
    if (!val)
      ...
      if (IS_UNSIGNED(val->type)) {
        dbuf_tprintf(oBuf, "\t!dbs\t; %u", aopLiteral(val, 0),
                     (unsigned int)ulVal);
      } else {
  ...
  case 4:
    ...
    break;
  case 8:
    printIvalVal(oBuf, val, 8,
                 true); // TODO: Print value as comment. Does dbuf_printf
                        // support long long even on MSVC?
    break;
  default:
...
/*-----------------------------------------------------------------*/
static unsigned int printIvalBitFields(symbol **sym, initList **ilist,
                                       struct dbuf_s *oBuf) {
  symbol *lsym = *sym;
  ...
  case 4:
    dbuf_tprintf(oBuf, "\t!db !constbyte, !constbyte, !constbyte, !constbyte\n",
                 (ival & 0xff), (ival >> 8) & 0xff, (ival >> 16) & 0xff,
                 (ival >> 24) & 0xff);
    bytes_written += 4;
    break;
  default:
...
/*-----------------------------------------------------------------*/
static void printIvalStruct(symbol *sym, sym_link *type, initList *ilist,
                            struct dbuf_s *oBuf) {
  symbol *sflds;
  initList *iloop = NULL;
  ...
  if (iloop)
    werrorfl(sym->fileDef, sym->lineDef, W_EXCESS_INITIALIZERS, "struct",
             sym->name);
}
...
/*-----------------------------------------------------------------*/
int printIvalChar(symbol *sym, sym_link *type, initList *ilist,
                  struct dbuf_s *oBuf, const char *s, bool check) {
  value *val;
  ...
  if (!s) {
    ...
    if (IS_ARRAY(val->type) && IS_CHAR(val->etype)) {
      ...
      if (check && DCL_ELEM(val->type) > size)
        werror(W_EXCESS_INITIALIZERS, "array of chars", sym->name,
               sym->lineDef);
...
/*-----------------------------------------------------------------*/
int printIvalChar16(symbol *sym, sym_link *type, initList *ilist,
                    struct dbuf_s *oBuf, const TYPE_TARGET_UINT *s,
                    bool check) {
  value *val;
  size_t size = DCL_ELEM(type);
  TYPE_TARGET_UINT *p;
  ...
  if (!s) {
    val = list2val(ilist, TRUE);
    /* if the value is a character string  */
    if (IS_ARRAY(val->type) && IS_INT(val->etype) && IS_UNSIGNED(val->etype) &&
        !IS_LONG(val->etype)) {
      if (!size) {
        /* we have not been given a size, but now we know it */
...
/*-----------------------------------------------------------------*/
int printIvalChar32(symbol *sym, sym_link *type, initList *ilist,
                    struct dbuf_s *oBuf, const TYPE_TARGET_ULONG *s,
                    bool check) {
  value *val;
  size_t size = DCL_ELEM(type);
  TYPE_TARGET_ULONG *p;
...
void printIvalArray(symbol *sym, sym_link *type, initList *ilist,
  ...
  if (ilist) {
    ...
    /* by a string                      */
    /* char *p = "abc";                 */
    if ((IS_CHAR(type->next) || IS_INT(type->next)) &&
      ...
      if (IS_CHAR(type->next) &&
          printIvalChar(sym, type, ilist, oBuf, SPEC_CVAL(sym->etype).v_char,
                        check))
        return;
      if (IS_INT(type->next) && IS_UNSIGNED(type->next)) {
        if (!IS_LONG(type->next) &&
            printIvalChar16(sym, type, ilist, oBuf,
                            SPEC_CVAL(sym->etype).v_char16, check))
          return;
        if (IS_LONG(type->next) &&
            printIvalChar32(sym, type, ilist, oBuf,
                            SPEC_CVAL(sym->etype).v_char32, check))
          return;
      }
    }
    /* char *p = {"abc"}; */
    if ((IS_CHAR(type->next) || IS_INT(type->next)) &&
        ilist->type == INIT_DEEP && ilist->init.deep &&
        ilist->init.deep->type == INIT_NODE) {
      val = list2val(ilist->init.deep, TRUE);
      if (!val) {
        werrorfl(ilist->init.deep->filename, ilist->init.deep->lineno,
                 E_INIT_STRUCT, sym->name);
        return;
      }
      if (!IS_LITERAL(val->etype)) {
        werrorfl(ilist->init.deep->filename, ilist->init.deep->lineno,
      ...
      if (IS_CHAR(type->next) &&
          printIvalChar(sym, type, ilist->init.deep, oBuf,
                        SPEC_CVAL(sym->etype).v_char, check))
        return;
      if (IS_INT(type->next) && IS_UNSIGNED(type->next)) {
        if (!IS_LONG(type->next) &&
            printIvalChar16(sym, type, ilist->init.deep, oBuf,
                            SPEC_CVAL(sym->etype).v_char16, check))
          return;
        if (IS_LONG(type->next) &&
            printIvalChar32(sym, type, ilist->init.deep, oBuf,
                            SPEC_CVAL(sym->etype).v_char32, check))
          return;
      }
    }
    ...
    for (iloop = ilist->init.deep; iloop; iloop = iloop->next) {
      if ((++size > DCL_ELEM(type)) && DCL_ELEM(type)) {
        werrorfl(sym->fileDef, sym->lineDef, W_EXCESS_INITIALIZERS, "array",
                 sym->name);
        break;
      }
...
/*--------------------------------------------------------------------*/
int printIvalCharPtr(symbol *sym, sym_link *type, value *val,
                     struct dbuf_s *oBuf) {
  int size = 0;
  char *p;
  ...
  if (val->name && strlen(val->name)) {
      ...
      if (IS_GENPTR(type) && GPTRSIZE > FARPTRSIZE && floatFromVal(val) != 0) {
        ...
        if (port->little_endian)
          dbuf_printf(oBuf, "\t.byte %s,%s,%s\n", aopLiteral(val, 0),
                      aopLiteral(val, 1), aopLiteralGptr(sym->name, val));
        else
          dbuf_printf(oBuf, "\t.byte %s,%s,%s\n",
                      aopLiteralGptr(sym->name, val), aopLiteral(val, 1),
                      aopLiteral(val, 0));
      } else {
        if (port->little_endian)
          dbuf_printf(oBuf, "\t.byte %s,%s,%s\n", aopLiteral(val, 0),
                      aopLiteral(val, 1), aopLiteral(val, 2));
        else
          dbuf_printf(oBuf, "\t.byte %s,%s,%s\n", aopLiteral(val, 2),
    ...
    case 4:
      if (IS_GENPTR(type) && GPTRSIZE > FARPTRSIZE && floatFromVal(val) != 0) {
        ...
        if (port->little_endian) {
          dbuf_printf(oBuf, "\t.byte %s,%s,%s", aopLiteral(val, 0),
                      aopLiteral(val, 1), aopLiteral(val, 2));
          ...
          dbuf_printf(oBuf, ",%s,%s,%s", aopLiteral(val, 2), aopLiteral(val, 1),
                      aopLiteral(val, 0));
        }
      } else {
        if (port->little_endian) {
          ...
          dbuf_printf(oBuf, "\t.byte %s,%s,%s,%s\n", aopLiteral(val, 3),
                      aopLiteral(val, 2), aopLiteral(val, 1),
                      aopLiteral(val, 0));
        }
      }
      break;
  ...
  if (!noInit && val->sym && val->sym->isstrlit &&
      !isinSet(statsg->syms, val->sym)) {
...
/*-----------------------------------------------------------------*/
void printIvalPtr(symbol *sym, sym_link *type, initList *ilist,
                  struct dbuf_s *oBuf) {
  value *val;
  int size;
  ...
  if (IS_LITERAL(val->etype)) {
    ...
    case 3:
      ...
      if (port->little_endian)
        dbuf_printf(oBuf, "\t.byte %s,%s", aopLiteral(val, 0),
                    aopLiteral(val, 1));
      else
        dbuf_printf(oBuf, "\t.byte %s,%s", aopLiteral(val, 1),
                    aopLiteral(val, 0));
      if (IS_GENPTR(val->type))
        dbuf_printf(oBuf, ",%s\n", aopLiteral(val, 2));
...
/*-----------------------------------------------------------------*/
void printIval(symbol *sym, sym_link *type, initList *ilist,
               struct dbuf_s *oBuf, bool check) {
  sym_link *itype;
      ...
      if (sym->ival) {
        ...
        if (IS_ARRAY(sym->type) &&
            (IS_CHAR(sym->type->next) && SPEC_CVAL(sym->etype).v_char ||
             IS_INT(sym->type->next) && !IS_LONG(sym->type->next) &&
                 SPEC_CVAL(sym->etype).v_char16 ||
             IS_INT(sym->type->next) && IS_LONG(sym->type->next) &&
                 SPEC_CVAL(sym->etype).v_char32)) {
...
/*-----------------------------------------------------------------*/
static void emitOverlay(struct dbuf_s *aBuf) {
  set *ovrset;
...
void glue(void) {
  ...
  if (port->extraAreas.genExtraAreaDeclaration) {
    port->extraAreas.genExtraAreaDeclaration(
        asmFile, mainf && IFFUNC_HASBODY(mainf->type));
  }
...
int isTargetKeyword(const char *s) {
  ...
  if (s[0] == '_' && s[1] == '_') {
    ...
    for (i = 0; port->keywords[i]; i++) {
      if (port->keywords[i][0] == '_' && strcmp(port->keywords[i], s + 1) == 0)
        return 1;
