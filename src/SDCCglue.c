  ...
  for (sym = setFirstItem(map->syms); sym; sym = setNextItem(map->syms)) {
    ...
    if (sym->ival && sym->level == 0) {
      if ((SPEC_OCLS(sym->etype) == xidata ||
        ...
        /* This results in unencessary calls to stringToSymbol() through
         * decorateType(), which make strings take twice as much  space as they
         * should */
...
/*-----------------------------------------------------------------*/
value *initPointer(initList *ilist, sym_link *toType) {
  value *val;
  ast *expr;
...
void printIvalType(symbol *sym, sym_link *type, initList *ilist,
  ...
  case 4:
    if (!val) {
      ...
      if (port->little_endian) {
        dbuf_printf(oBuf, "\t.byte %s,%s,%s,%s", aopLiteral(val, 0),
                    aopLiteral(val, 1), aopLiteral(val, 2), aopLiteral(val, 3));
      } else {
        dbuf_printf(oBuf, "\t.byte %s,%s,%s,%s", aopLiteral(val, 3),
  ...
  case 8:
    if (port->little_endian) {
      dbuf_printf(oBuf, "\t.byte %s,%s,%s,%s,%s,%s,%s,%s", aopLiteral(val, 0),
                  aopLiteral(val, 1), aopLiteral(val, 2), aopLiteral(val, 3),
                  aopLiteral(val, 4), aopLiteral(val, 5), aopLiteral(val, 6),
                  aopLiteral(val, 7));
    } else {
      dbuf_printf(oBuf, "\t.byte %s,%s,%s,%s,%s,%s,%s,%s", aopLiteral(val, 7),
                  aopLiteral(val, 6), aopLiteral(val, 5), aopLiteral(val, 4),
                  aopLiteral(val, 3), aopLiteral(val, 2), aopLiteral(val, 1),
                  aopLiteral(val, 0));
    }
    // TODO: Print value as comment. Does dbuf_printf support long long even on
    // MSVC?
    dbuf_printf(oBuf, "\n");
    break;
  default:
...
/*-----------------------------------------------------------------*/
static void printIvalBitFields(symbol **sym, initList **ilist,
                               struct dbuf_s *oBuf) {
  symbol *lsym = *sym;
