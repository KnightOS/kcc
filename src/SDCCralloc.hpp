...
// Compiler Construction - 22nd International Conference, CC 2013, Held as Part
// of the European Joint Conferences on Theory and Practice of Software, ETAPS
// 2013. Proceedings, Lecture Notes in Computer Science, volume 7791, pp. 1-20.
// Springer,
// 2013.
//
...
// float instruction_cost(const assignment &a, unsigned short int i, const G_t
// &G, const I_t &I); Which can range from simple, e.g. cost 1 for each byte
// accessed in a register, cost 4 for each byte accessed in memory to quite
// involved, e.g. the number of bytes of code the code generator would generate.
//
// 2) Call
// create_cfg(), thorup_tree_decomposition(), nicify(), alive_tree_dec(),
...
typedef std::vector<var_t>
    varset_t; // Faster than std::set,  std::tr1::unordered_set and
              // stx::btree_set here.

typedef boost::container::flat_map<int, float>
    icosts_t; // Faster than std::map and stx::btree_map here.

typedef std::vector<var_t> cfg_alive_t; // Faster than stx::btree_set here .
typedef boost::container::flat_set<var_t>
    cfg_dying_t; // Faster than stx::btree_set and std::set here.
...
struct assignment {
  ...
  std::vector<reg_t>
      global;       // Entries: global[var] = reg (-1 if no reg assigned)
  icosts_t i_costs; // Costs for all instructions in bag (needed to avoid double
                    // counting costs at join nodes)
  i_assignment_t i_assignment; // Assignment at the instruction currently being
                               // added in an introduce node;
...
// typedef std::vector<assignment> assignment_list_t; // Probably faster, but
// would require some code reorganization.

struct tree_dec_node {
  std::set<unsigned int> bag;
  std::set<var_t> alive;
  assignment_list_t assignments;
  unsigned weight; // The weight is the number of nodes at which intermediate
                   // results need to be remembered. In general, to minimize
                   // memory consumption, at join nodes the child with maximum
                   // weight should be processed first.
};

typedef boost::container::flat_multimap<int, var_t>
    operand_map_t; // Faster than std::multimap<int, var_t> and
                   // stx::btree_multimap<int, var_t> here.
...
cfg_node::cfg_node(void) { default_constructor_of_cfg_node_called = true; }
#endif
...
// For early removel of assignments that cannot be extended to valid
// assignments. Port-specific.
template <class G_t, class I_t>
...
// Avoid overwriting operands that are still needed by the result.
// Port-specific.
template <class I_t>
...
// generated. Port-specific.
static void extra_ic_generated(iCode *ic);

inline void
add_operand_to_cfg_node(cfg_node &n, operand *o,
                        std::map<std::pair<int, reg_t>, var_t> &sym_to_index) {
  reg_t k;
  if (o && IS_SYMOP(o) &&
      sym_to_index.find(std::pair<int, reg_t>(OP_SYMBOL_CONST(o)->key, 0)) !=
          sym_to_index.end()) {
    if (n.operands.find(OP_SYMBOL_CONST(o)->key) == n.operands.end())
      for (k = 0; k < OP_SYMBOL_CONST(o)->nRegs; k++)
        n.operands.insert(std::pair<int, var_t>(
            OP_SYMBOL_CONST(o)->key,
            sym_to_index[std::pair<int, reg_t>(OP_SYMBOL_CONST(o)->key, k)]));
  }
}
...
static inline int component_size_impl(const cfg_t &cfg,
                                      const std::vector<bool> &life, var_t v,
                                      int i, std::vector<bool> &visited) {
  ...
  for (boost::tie(in, in_end) = boost::in_edges(i, cfg); in != in_end; ++in)
    if (life[boost::source(*in, cfg)] && !visited[boost::source(*in, cfg)])
      size +=
          component_size_impl(cfg, life, v, boost::source(*in, cfg), visited);

  for (boost::tie(out, out_end) = boost::out_edges(i, cfg); out != out_end;
       ++out)
    if (life[boost::target(*out, cfg)] && !visited[boost::target(*out, cfg)])
      size +=
          component_size_impl(cfg, life, v, boost::target(*out, cfg), visited);

  return (size);
}

static inline int component_size(const cfg_t &cfg,
                                 const std::vector<bool> &life, var_t v,
                                 int i) {
...
static bool liverange_connected(const cfg_t &cfg, var_t v) {
  ...
  for (int i = 0; i < boost::num_vertices(cfg); i++)
    if (std::find(cfg[i].alive.begin(), cfg[i].alive.end(), v) !=
        cfg[i].alive.end()) {
      life[i] = true;
      num_life++;
      last_life = i;
...
static iCode *create_cfg(cfg_t &cfg, con_t &con, ebbIndex *ebbi) {
  ...
  start_ic = iCodeLabelOptimize(iCodeFromeBBlock(ebbs, ebbi->count));
  {
    ...
    for (ic = start_ic, i = 0, j = 0; ic; ic = ic->next, i++) {
      if (currFunc)
        currFunc->funcDivFlagSafe &=
            !(ic->op == INLINEASM || ic->op == '/' || ic->op == '%' ||
              ic->op == PCALL ||
              ic->op == CALL && (IS_OP_LITERAL(IC_LEFT(ic)) ||
                                 !OP_SYMBOL(IC_LEFT(ic))->funcDivFlagSafe) ||
              ic->op == RIGHT_OP &&
                  IS_OP_LITERAL(IC_RIGHT(
                      ic))); // Right shift might be implemented using division.
...
#ifdef DEBUG_SEGV
      wassertl(default_constructor_of_cfg_node_called,
      ...
      if (ic->op == SEND && ic->builtinSEND) // Ensure that only the very first
                                             // send iCode is active.
      {
      ...
      for (int j2 = 0; j2 <= operandKey; j2++) {
        if (bitVectBitValue(ic->rlive, j2)) {
          ...
          if (sym_to_index.find(std::pair<int, reg_t>(j2, 0)) !=
              sym_to_index.end())
            continue;

          // Other parts of the allocator may rely on the variables
          // corresponding to bytes from the same sdcc variable to have
          // subsequent numbers.
  ...
  for (ic = start_ic; ic; ic = ic->next) {
    wassertl(key_to_index[ic->key] < boost::num_vertices(cfg),
             "Node not in CFG.");

    if (ic->op != GOTO && ic->op != RETURN && ic->op != JUMPTABLE && ic->next) {
      wassertl(key_to_index[ic->next->key] < boost::num_vertices(cfg),
               "Next node not in CFG.");
      boost::add_edge(key_to_index[ic->key], key_to_index[ic->next->key], cfg);
    }

    if (ic->op == GOTO) {
      wassertl(key_to_index[eBBWithEntryLabel(ebbi, ic->label)->sch->key] <
                   boost::num_vertices(cfg),
               "GOTO target not in CFG.");
      boost::add_edge(
          key_to_index[ic->key],
          key_to_index[eBBWithEntryLabel(ebbi, ic->label)->sch->key], cfg);
    } else if (ic->op == RETURN) {
      wassertl(key_to_index[eBBWithEntryLabel(ebbi, returnLabel)->sch->key] <
                   boost::num_vertices(cfg),
               "RETURN target not in CFG.");
      boost::add_edge(
          key_to_index[ic->key],
          key_to_index[eBBWithEntryLabel(ebbi, returnLabel)->sch->key], cfg);
    } else if (ic->op == IFX) {
      wassertl(key_to_index[eBBWithEntryLabel(ebbi, IC_TRUE(ic) ? IC_TRUE(ic)
                                                                : IC_FALSE(ic))
                                ->sch->key] < boost::num_vertices(cfg),
               "IFX target not in CFG.");
      boost::add_edge(
          key_to_index[ic->key],
          key_to_index[eBBWithEntryLabel(ebbi, IC_TRUE(ic) ? IC_TRUE(ic)
                                                           : IC_FALSE(ic))
                           ->sch->key],
          cfg);
      ...
      for (symbol *lbl = (symbol *)(setFirstItem(IC_JTLABELS(ic))); lbl;
        ...
        wassertl(key_to_index[eBBWithEntryLabel(ebbi, lbl)->sch->key] <
                     boost::num_vertices(cfg),
                 "GOTO target not in CFG.");
        boost::add_edge(key_to_index[ic->key],
                        key_to_index[eBBWithEntryLabel(ebbi, lbl)->sch->key],
                        cfg);
      }
    ...
    for (int i = 0; i <= operandKey; i++) {
      ...
      if (bitVectBitValue(ic->rlive, i)) {
        ...
        for (reg_t k = 0; k < isym->nRegs; k++) {
          ...
          wassert(sym_to_index.find(std::pair<int, int>(i, k)) !=
                  sym_to_index.end());
          wassertl(key_to_index[ic->key] < boost::num_vertices(cfg),
                   "Node not in CFG.");
          cfg[key_to_index[ic->key]].alive.push_back(
              sym_to_index[std::pair<int, int>(i, k)]);
        }

        // TODO: Move this to a place where it also works when using the old
        // allocator!
        isym->block = btree_lowest_common_ancestor(isym->block, ic->block);
        // If this symbol has a spill location, ensure the spill location is
        // also allocated in a compatible block
        if (SYM_SPIL_LOC(isym))
          SYM_SPIL_LOC(isym)->block = btree_lowest_common_ancestor(
              SYM_SPIL_LOC(isym)->block, isym->block);
      }
    }

    if (ic->op == IFX)
      add_operand_to_cfg_node(cfg[key_to_index[ic->key]], IC_COND(ic),
                              sym_to_index);
    else if (ic->op == JUMPTABLE)
      add_operand_to_cfg_node(cfg[key_to_index[ic->key]], IC_JTCOND(ic),
                              sym_to_index);
    else {
      add_operand_to_cfg_node(cfg[key_to_index[ic->key]], IC_RESULT(ic),
                              sym_to_index);
      add_operand_to_cfg_node(cfg[key_to_index[ic->key]], IC_LEFT(ic),
                              sym_to_index);
      add_operand_to_cfg_node(cfg[key_to_index[ic->key]], IC_RIGHT(ic),
                              sym_to_index);
    }

    // TODO: Extend live-ranges of returns of built-in function calls back to
    // first SEND.
  ...
  for (var_t i = (var_t)boost::num_vertices(con) - 1; i >= 0; i--)
    if (!liverange_connected(cfg, i)) {
      // Non-connected CFGs are created by at least GCSE and lospre. We now have
      // a live-range splitter that fixes them, so this should no longer be
      // necessary, but we leave this code here for now, so in case one gets
      // through, we can still generate correct code.
      std::cerr
          << "Warning: Non-connected liverange found and extended to connected "
      ...
      boost::copy_graph(cfg, cfg2,
                        boost::vertex_copy(forget_properties())
                            .edge_copy(forget_properties()));
      std::vector<boost::graph_traits<cfg_t>::vertices_size_type> component(
          num_vertices(cfg2));
      boost::connected_components(cfg2, &component[0]);

      for (boost::graph_traits<cfg_t>::vertices_size_type j = 0;
           j < boost::num_vertices(cfg) - 1; j++) {
        if (std::find(cfg[j].alive.begin(), cfg[j].alive.end(), i) ==
            cfg[j].alive.end())
          continue;

        for (boost::graph_traits<cfg_t>::vertices_size_type k = 0;
             k < boost::num_vertices(cfg) - 1; k++)
          if (component[j] == component[k] &&
              std::find(cfg[k].alive.begin(), cfg[k].alive.end(), i) ==
                  cfg[k].alive.end())
  ...
  for (boost::graph_traits<cfg_t>::vertices_size_type i = 0;
       i < num_vertices(cfg); i++) {
    ...
    for (boost::tie(j, j_end) = adjacent_vertices(i, cfg); j != j_end; ++j) {
      ...
      for (v = cfg[*j].alive.begin(), v_end = cfg[*j].alive.end(); v != v_end;
           ++v) {
        const symbol *const vsym =
            (symbol *)(hTabItemWithKey(liveRanges, con[*v].v));
        ...
        if (!(POINTER_SET(cfg[*j].ic) ||
              cfg[*j].ic->op == SET_VALUE_AT_ADDRESS) &&
            (!left || !IS_SYMOP(left) ||
  ...
  for (boost::graph_traits<cfg_t>::vertices_size_type i = 0;
    ...
    for (v = cfg[i].alive.begin(), v_end = cfg[i].alive.end(); v != v_end;
         ++v) {
      cfg_alive_t::const_iterator v2, v2_end;

      // Conflict between operands are handled by
      // add_operand_conflicts_in_node().
      if (cfg[i].dying.find(*v) != cfg[i].dying.end())
        continue;
      if (ic->op != IFX && ic->op != JUMPTABLE && IC_RESULT(ic) &&
          IS_SYMOP(IC_RESULT(ic))) {
        operand_map_t::const_iterator oi, oi_end;
        for (boost::tie(oi, oi_end) = cfg[i].operands.equal_range(
                 OP_SYMBOL_CONST(IC_RESULT(ic))->key);
             oi != oi_end; ++oi)
          if (oi->second == *v)
            goto next_var;
      }
      ...
      // TODO: Check if we can use v, ++v2 instead of cfg[i].alive.begin() to
      // speed things up.
      for (v2 = cfg[i].alive.begin(), v2_end = cfg[i].alive.end(); v2 != v2_end;
           ++v2) {
        if (*v == *v2)
          continue;
...
template <class G_t>
void assignments_introduce_instruction(assignment_list_t &alist,
                                       unsigned short int i, const G_t &G) {
  assignment_list_t::iterator ai, ai_end;

#if !defined(_MSC_VER) // Efficient code - reduces total SDCC runtime by
                       // about 5.5% vs. code below, but doesn't work with
                       // MSVC++ (at least up to MSVC++ 2015)
  struct inserter_t {
    explicit inserter_t(const std::vector<reg_t> &g, i_assignment_t &a)
        : global(g), ia(a) {}
    ...
    inserter_t &operator*() { return (*this); }
    inserter_t &operator++() { return (*this); }
  ...
  for (ai = alist.begin(), ai_end = alist.end(); ai != ai_end; ++ai) {
    ...
    std::set_intersection(ai->local.begin(), ai->local.end(),
                          G[i].alive.begin(), G[i].alive.end(),
                          inserter_t(ai->global, ia));
  ...
  for (ai = alist.begin(), ai_end = alist.end(); ai != ai_end; ++ai) {
    ...
    std::set_intersection(ai->local.begin(), ai->local.end(),
                          G[i].alive.begin(), G[i].alive.end(),
                          std::inserter(i_variables, i_variables.end()));
...
template <class G_t, class I_t>
static void assignments_introduce_variable(assignment_list_t &alist,
                                           unsigned short int i, short int v,
                                           const G_t &G, const I_t &I) {
    ...
    for (reg_t r = 0; r < port->num_regs; r++) {
      if (!assignment_conflict(*ai, I, v, r)) {
        if (!a_initialized) {
          ...
          a.marked = false;
          varset_t::iterator i =
              std::lower_bound(a.local.begin(), a.local.end(), v);
...
template <class I_t>
float compability_cost(const assignment &a, const assignment &ac,
                       const I_t &I) {
  typedef
...
#if 0 // This improves the quality of assignments, but it has a big runtime
      // overhead for some cases.
      adjacency_iter_t j, j_end;
...
// Ensure that we never get more than options.max_allocs_per_node assignments at
// a single node of the tree decomposition. Tries to drop the worst ones first
// (but never drop the empty assignment, as it's the only one guaranteed to be
// always valid).
template <class G_t, class I_t>
static void drop_worst_assignments(assignment_list_t &alist,
                                   unsigned short int i, const G_t &G,
                                   const I_t &I, const assignment &ac,
                                   bool *const assignment_optimal) {
  unsigned int n;
  size_t alist_size;
  assignment_list_t::iterator ai, an;

  if ((alist_size = alist.size()) * port->num_regs <=
          static_cast<size_t>(options.max_allocs_per_node) ||
      alist_size <= 1)
    return;
  ...
  *assignment_optimal = false;
  ...
  std::cout << "Too many assignments here (" << i << "):" << alist_size << " > "
            << options.max_allocs_per_node / port->num_regs
            << ". Dropping some.\n";
  std::cout.flush();
#endif
...
template <class T_t, class G_t, class I_t>
static void tree_dec_ralloc_introduce(
    T_t &T, typename boost::graph_traits<T_t>::vertex_descriptor t,
    const G_t &G, const I_t &I, const assignment &ac,
    bool *const assignment_optimal) {
  typedef
  ...
  std::set_difference(T[t].alive.begin(), T[t].alive.end(), T[*c].alive.begin(),
                      T[*c].alive.end(),
                      std::inserter(new_vars, new_vars.end()));

  std::set<unsigned short> new_inst;
  std::set_difference(T[t].bag.begin(), T[t].bag.end(), T[*c].bag.begin(),
                      T[*c].bag.end(), std::inserter(new_inst, new_inst.end()));
  ...
  for (ai = alist.begin(); ai != alist.end();) {
    if ((ai->s += (ai->i_costs[i] = instruction_cost(*ai, i, G, I))) ==
  ...
  // Free memory in the std::set<var_t, boost::pool_allocator<var_t> > that live
  // in the assignments in the list.
  // boost::singleton_pool<boost::fast_pool_allocator_tag,
  // sizeof(var_t)>::release_memory();
...
static bool assignments_locally_same(const assignment &a1,
                                     const assignment &a2) {
...
tree_dec_ralloc_forget(T_t &T,
  ...
  std::set_difference(T[*c].bag.begin(), T[*c].bag.end(), T[t].bag.begin(),
                      T[t].bag.end(), std::inserter(old_inst, old_inst.end()));
  wassert(old_inst.size() == 1);
  unsigned short int i = *(old_inst.begin());

  varset_t old_vars;
  std::set_difference(T[*c].alive.begin(), T[*c].alive.end(),
                      T[t].alive.begin(), T[t].alive.end(),
                      std::inserter(old_vars, old_vars.end()));
  ...
  for (ai = alist.begin(); ai != alist.end(); ++ai) {
    newlocal.clear();
    std::set_difference(ai->local.begin(), ai->local.end(), old_vars.begin(),
                        old_vars.end(),
...
tree_dec_ralloc_join(T_t &T,
  ...
  for (ai = alist.begin(), ai2 = alist2.begin();
       ai != alist.end() && ai2 != alist2.end();) {
    if (assignments_locally_same(*ai, *ai2)) {
      ...
      for (size_t i = 0; i < ai->global.size(); i++)
        ai->global[i] =
            ((ai->global[i] != -1) ? ai->global[i] : ai2->global[i]);
      ++ai;
      ++ai2;
    } else if (*ai < *ai2)
      ai = alist.erase(ai);
...
#ifdef DEBUG_RALLOC_DEC_ASS
  for (std::list<assignment>::iterator ai = alist.begin(); ai != alist.end();
       ++ai) {
    print_assignment(*ai);
    std::cout << "\n";
  }
...
template <class T_t>
void get_best_local_assignment(
    assignment &a, typename boost::graph_traits<T_t>::vertex_descriptor t,
    const T_t &T) {
...
// Handle nodes in the tree decomposition, by detecting their type and calling
// the appropriate function. Recurses.
template <class T_t, class G_t, class I_t>
static void
tree_dec_ralloc_nodes(T_t &T,
                      typename boost::graph_traits<T_t>::vertex_descriptor t,
                      const G_t &G, const I_t &I, const assignment &ac,
                      bool *const assignment_optimal) {
  typedef
  ...
  case 2:
    ...
    if (T[c0].weight <
        T[c1].weight) // Minimize memory consumption needed for keeping
                      // intermediate results. As a side effect, this also helps
                      // the ac mechanism in the heuristic.
      std::swap(c0, c1);
...
template <class T_t>
static std::pair<typename boost::graph_traits<T_t>::vertex_descriptor, size_t>
find_best_root(const T_t &T,
               typename boost::graph_traits<T_t>::vertex_descriptor t,
               size_t t_s,
               typename boost::graph_traits<T_t>::vertex_descriptor t_old,
               size_t t_old_s) {
  typedef
      typename boost::graph_traits<T_t>::adjacency_iterator adjacency_iter_t;
  adjacency_iter_t c, c_end;
  ...
  case 0:
    return (
        t_s > t_old_s
            ? std::pair<typename boost::graph_traits<T_t>::vertex_descriptor,
                        size_t>(t, t_s)
            : std::pair<typename boost::graph_traits<T_t>::vertex_descriptor,
                        size_t>(t_old, t_old_s));
  case 1:
    return (find_best_root(T, *c, T[*c].alive.size() ? T[*c].alive.size() : t_s,
                           t_old, t_old_s));
  case 2:
    c0 = *c++;
    c1 = *c;
    boost::tie(t0, t0_s) = find_best_root(
        T, c0, T[c0].alive.size() ? T[c0].alive.size() : t_s, t_old, t_old_s);
    return (find_best_root(T, c1, T[c1].alive.size() ? T[c1].alive.size() : t_s,
                           t0_s > t_old_s ? t0 : t_old,
                           t0_s > t_old_s ? t0_s : t_old_s));
    break;
  default:
    std::cerr << "Not nice.\n";
    break;
  ...
  return (
      std::pair<typename boost::graph_traits<T_t>::vertex_descriptor, size_t>(
          t_old, t_old_s));
}

// Change the root to t.
template <class T_t>
static void re_root(T_t &T,
                    typename boost::graph_traits<T_t>::vertex_descriptor t) {
  ...
  for (boost::tie(e, e_end) = boost::in_edges(s1, T); e != e_end;
       boost::tie(e, e_end) = boost::in_edges(s1, T)) {
    s2 = boost::source(*e, T);
...
template <class T_t> static void good_re_root(T_t &T) {
  typename boost::graph_traits<T_t>::vertex_descriptor t;
  ...
  for (boost::tie(c, c_end) = boost::adjacent_vertices(t, T);
       c != c_end && !T[*c].alive.size();)
...
static void dump_con(const con_t &con) {
  ...
  std::ofstream dump_file(
      (std::string(dstFileName) + ".dumpralloccon" + currFunc->rname + ".dot")
          .c_str());

  std::string *name = new std::string[num_vertices(con)];
  for (var_t i = 0; static_cast<boost::graph_traits<cfg_t>::vertices_size_type>(
                        i) < boost::num_vertices(con);
       i++) {
...
static void dump_cfg(const cfg_t &cfg) {
  ...
  std::ofstream dump_file(
      (std::string(dstFileName) + ".dumpralloccfg" + currFunc->rname + ".dot")
          .c_str());
...
static void dump_tree_decomposition(const tree_dec_t &tree_dec) {
  ...
  std::ofstream dump_file(
      (std::string(dstFileName) + ".dumprallocdec" + currFunc->rname + ".dot")
          .c_str());
  ...
  boost::write_graphviz(
      dump_file, tree_dec, boost::make_label_writer(name),
      boost::default_writer(),
