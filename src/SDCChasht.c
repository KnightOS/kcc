...
void hTabAddItemLong(hTab **htab, int key, void *pkey, void *item) {
  ...
  if (key > (*htab)->size) {
    int i;
    (*htab)->table =
        Safe_realloc((*htab)->table, (key * 2 + 2) * sizeof(hashtItem *));
...
/*-----------------------------------------------------------------*/
void hTabDeleteItem(hTab **htab, int key, const void *item,
...
static const hashtItem *_findItem(hTab *htab, int key, void *item,
...
static const hashtItem *_findByKey(hTab *htab, int key, const void *pkey,
...
void *hTabFindByKey(hTab *h, int key, const void *pkey,
                    int (*compare)(const void *, const void *)) {
...
int hTabDeleteByKey(hTab **h, int key, const void *pkey,
                    int (*compare)(const void *, const void *)) {
  hashtItem *htip, **htipp;
  bool found = FALSE;
  ...
  htip = (*h)->table[key];
  for (; htip; htip = htip->next) {
    if ((compare && compare(pkey, htip->pkey)) || pkey == htip->pkey) {
      *htipp = htip->next;
      found = TRUE;
      break;
    }
...
/*-----------------------------------------------------------------*/
int hTabIsInTable(hTab *htab, int key, void *item,
                  int (*compareFunc)(void *, void *)) {
...
/*-----------------------------------------------------------------*/
int isHtabsEqual(hTab *htab1, hTab *htab2, int (*compareFunc)(void *, void *)) {
  void *item;
  int key;
...
/*-----------------------------------------------------------------*/
int hTabMaxKey(hTab *htab) { return (htab ? htab->maxKey : 0); }
...
*/
static int _compare(const void *s1, const void *s2) { return !strcmp(s1, s2); }
