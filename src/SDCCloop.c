...
/*-----------------------------------------------------------------*/
static induction *newInduction(operand *sym, unsigned int op, long constVal,
                               iCode *ic, operand *asym) {
  induction *ip;
...
/*-----------------------------------------------------------------*/
static int assignmentsToSym(set *sset, operand *sym) {
  eBBlock *ebp;
  int assigns = 0;
...
/*-----------------------------------------------------------------*/
static int isOperandInvariant(operand *op, region *theLoop, set *lInvars) {
  int opin = 0;
  ...
  if (op) {
    if (IS_OP_LITERAL(op))
      ...
      opin = 1;
    else if (IS_SYMOP(op) && !IS_OP_GLOBAL(op) && !IS_OP_VOLATILE(op) &&
...
static int loopInvariants(region *theLoop, ebbIndex *ebbi) {
  ...
  for (lBlock = setFirstItem(theLoop->regBlocks); lBlock;
    ...
    for (ic = lBlock->sch; ic; ic = ic->next) {
      ...
      if (IC_RESULT(ic) &&
          (isOperandVolatile(IC_RESULT(ic), TRUE) || IS_OP_PARM(IC_RESULT(ic))))
        continue;
      ...
      if (ic->op == ADDRESS_OF && IS_SYMOP(IC_LEFT(ic)) &&
        ...
        for (sBlock = setFirstItem(lSet); sBlock; sBlock = setNextItem(lSet)) {
          ...
          if (sBlock == lBlock) {
            if (!ifDiCodeIs(sBlock->outExprs, ic))
              break;
          } else if (bitVectBitsInCommon(sBlock->defSet,
          ...
          for (ic2 = sBlock->sch; ic2; ic2 = ic2->next) {
            if (ic2->op == IFX) {
              ...
              if (IC_RIGHT(ic2) && isOperandEqual(IC_RESULT(ic), IC_RIGHT(ic2)))
                used = 1;
              if ((ic != ic2) &&
                  (isOperandEqual(IC_RESULT(ic), IC_RESULT(ic2))))
                break;
...
/*-----------------------------------------------------------------*/
static int addressTaken(set *sset, operand *sym) {
  set *loop;
  eBBlock *ebp;
  set *loop2;
...
/*-----------------------------------------------------------------*/
static iCode *findDefInRegion(set *regBlocks, operand *defOp, eBBlock **owner) {
  eBBlock *lBlock;
...
/*                    loop exits                                   */
/*-----------------------------------------------------------------*/
static void addPostLoopBlock(region *loopReg, ebbIndex *ebbi, iCode *ic) {
  bitVect *loopSuccs;
  int i;
  ...
  for (i = 0; i < loopSuccs->size; i++) {
    ...
    if (strncmp(LOOPEXITLBL, eblock->entryLabel->name,
      ...
      ebbi->bbOrder =
          Safe_realloc(ebbi->bbOrder, (ebbi->count + 1) * sizeof(eBBlock *));
      ...
      ebbi->dfOrder =
          Safe_realloc(ebbi->dfOrder, (ebbi->count + 1) * sizeof(eBBlock *));
      ...
      for (ebpi = setFirstItem(loopReg->exits); ebpi;
      ...
      oldPredList =
          subtractFromSet(eblock->predList, loopReg->exits, THROW_NONE);
      for (ebpi = setFirstItem(oldPredList); ebpi;
      ...
      /* set the label */
      postLoopBlk->sch = postLoopBlk->ech =
          newiCodeLabelGoto(LABEL, postLoopBlk->entryLabel);
    }
    ...
    postLoopBlk->sch->filename = postLoopBlk->ech->filename =
        eblock->sch->filename;
...
/*-----------------------------------------------------------------*/
static set *basicInduction(region *loopReg, ebbIndex *ebbi) {
  eBBlock *lBlock;
  set *indVars = NULL;
  ...
  for (lBlock = setFirstItem(loopReg->regBlocks); lBlock;
    ...
    for (ic = lBlock->sch; ic; ic = ic->next) {
      ...
      if (!(dic = findDefInRegion(setFromSet(loopReg->regBlocks), IC_RIGHT(ic),
                                  &owner)))
        continue;
      ...
      if (!isOperandEqual(IC_RESULT(ic), aSym) &&
        ...
        if (!(ddic = findDefInRegion(setFromSet(loopReg->regBlocks), aSym,
                                     &owner)))
          continue;
...
/*-----------------------------------------------------------------*/
static int loopInduction(region *loopReg, ebbIndex *ebbi) {
  int change = 0;
  ...
  for (lBlock = setFirstItem(loopReg->regBlocks), lastBlock = lBlock;
    ...
    for (ic = lBlock->sch; ic; ic = ic->next) {
      ...
      indIc = newiCode(ic->op, operandFromOperand(aSym),
      ...
      indIc = newiCode(ip->op, operandFromOperand(IC_RESULT(ic)),
...
static DEFSETFUNC(mergeRegions) {
  ...
  for (lp = setFirstItem(allRegion); lp; lp = setNextItem(allRegion)) {
    ...
    if (lp->entry == theLoop->entry) {
      theLoop->regBlocks =
          unionSets(theLoop->regBlocks, lp->regBlocks, THROW_DEST);
      lp->merged = 1;
    }
  }
...
/*-----------------------------------------------------------------*/
hTab *createLoopRegions(ebbIndex *ebbi) {
  ...
  for (lp = setFirstItem(allRegion); lp; lp = setNextItem(allRegion)) {
    applyToSet(lp->regBlocks, addToExitsMarkDepth, lp->regBlocks, &lp->exits,
