...
#define IS_AUTOSYM(op)                                                         \
  (IS_ITEMP(op) || (IS_SYMOP(op) && IS_AUTO(OP_SYMBOL(op)) && !IS_PARM(op)))
...
/*-----------------------------------------------------------------*/
static void sequenceiCode(eBBlock **ebbs, int count) {
  int i;
...
/* setFromRange - sets the from range of a given operand           */
/*-----------------------------------------------------------------*/
static void setLiveFrom(symbol *sym, int from) {
...
/* setToRange - set the range to for an operand                    */
/*-----------------------------------------------------------------*/
static void setLiveTo(symbol *sym, int to) {
...
/*-----------------------------------------------------------------*/
static void markLiveRanges(eBBlock **ebbs, int count) {
  int i, key;
  symbol *sym;
...
/*-----------------------------------------------------------------*/
static void markAlive(iCode *sic, iCode *eic, int key) {
  iCode *dic;
...
/*-----------------------------------------------------------------*/
static int findNextUseSym(eBBlock *ebp, iCode *ic, symbol *sym) {
  int retval = 0;
  iCode *uic;
  eBBlock *succ;
...
/*               Return 0 iff there is no next use.                */
/*-----------------------------------------------------------------*/
static int findNextUse(eBBlock *ebp, iCode *ic, operand *op) {
  if (op->isaddr)
    OP_SYMBOL(op)->isptr = 1;
...
/*-----------------------------------------------------------------*/
static void unvisitBlocks(eBBlock **ebbs, int count) {
  int i;
...
/*------------------------------------------------------------------*/
static void markWholeLoop(eBBlock *ebp, int key) {
  eBBlock *ebpi;
  ...
  for (ebpi = setFirstItem(ebp->predList); ebpi;
  ...
  for (ebpi = setFirstItem(ebp->succList); ebpi;
...
/*------------------------------------------------------------------*/
static bool findPrevUseSym(eBBlock *ebp, iCode *ic, symbol *sym) {
  eBBlock *pred;
  iCode *uic;
...
/*-----------------------------------------------------------------*/
static void incUsed(iCode *ic, operand *op) {
  if (ic->depth)
...
/*-----------------------------------------------------------------*/
static void rliveClear(eBBlock **ebbs, int count) {
  int i;
...
/*-----------------------------------------------------------------*/
static void computeClash(eBBlock **ebbs, int count) {
  int i;
        ...
        for (key2 = key1 + 1; key2 < ic->rlive->size; key2++) {
          ...
          if ((ic->op != JUMPTABLE) && (ic->op != IFX) &&
            ...
            if (OP_SYMBOL(IC_RESULT(ic))->key == key1 &&
                sym1->liveFrom == ic->seq && sym2->liveTo == ic->seq) {
              if (IS_SYMOP(IC_LEFT(ic)))
            ...
            if (OP_SYMBOL(IC_RESULT(ic))->key == key2 &&
                sym2->liveFrom == ic->seq && sym1->liveTo == ic->seq) {
              if (IS_SYMOP(IC_LEFT(ic)))
...
bool allDefsOutOfRange(bitVect *defs, int fseq, int toseq) {
  ...
  for (i = 0; i < defs->size; i++) {
    ...
    if (bitVectBitValue(defs, i) && (ic = hTabItemWithKey(iCodehTab, i)) &&
    ...
    case IFX:
      visit(visited,
            hTabItemWithKey(labelDef,
                            (IC_TRUE(ic) ? IC_TRUE(ic) : IC_FALSE(ic))->key),
            key);
      ic = ic->next;
      break;
    default:
      ic = ic->next;
      if (!POINTER_SET(ic) && IC_RESULT(ic) && IS_SYMOP(IC_RESULT(ic)) &&
          OP_SYMBOL_CONST(IC_RESULT(ic))->key == key) {
        addSet(visited, ic);
        return;
      }
    }
...
int separateLiveRanges(iCode *sic, ebbIndex *ebbi) {
  ...
  for (iCode *ic = sic; ic; ic = ic->next) {
    if (ic->op == IFX || ic->op == GOTO || ic->op == JUMPTABLE ||
        ...
        isinSet(candidates, OP_SYMBOL(IC_RESULT(ic))))
      continue;
  ...
  for (sym = setFirstItem(candidates); sym; sym = setNextItem(candidates)) {
    ...
    do {
      ...
      if (!bitVectBitValue(((iCode *)(setFirstItem(defs)))->rlive, sym->key)) {
        werror(W_INTERNAL_ERROR, __FILE__, __LINE__,
               "Variable is not alive at one of its definitions");
        break;
      }
      ...
      if (newdefs && defs) {
        operand *tmpop = newiTempOperand(
            operandType(IC_RESULT((iCode *)(setFirstItem(newdefs)))), TRUE);
        ...
        for (iCode *ic = setFirstItem(visited); ic; ic = setNextItem(visited)) {
          ...
          if (IC_RIGHT(ic) && IS_ITEMP(IC_RIGHT(ic)) &&
              OP_SYMBOL(IC_RIGHT(ic)) == sym)
            IC_RIGHT(ic) = operandFromOperand(tmpop);
          if (IC_RESULT(ic) && IS_ITEMP(IC_RESULT(ic)) &&
              OP_SYMBOL(IC_RESULT(ic)) == sym && !POINTER_SET(ic) && ic->next &&
              !isinSet(visited, ic->next))
            continue;
          if (IC_RESULT(ic) && IS_ITEMP(IC_RESULT(ic)) &&
