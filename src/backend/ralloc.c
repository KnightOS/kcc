...
static reg_info _gbz80_regs[] = {
    {REG_GPR, A_IDX, "a", 1}, {REG_GPR, C_IDX, "c", 1},
    {REG_GPR, B_IDX, "b", 1}, {REG_GPR, E_IDX, "e", 1},
    {REG_GPR, D_IDX, "d", 1}, {REG_GPR, L_IDX, "l", 1},
    {REG_GPR, H_IDX, "h", 1}, {REG_CND, CND_IDX, "c", 1}};

static reg_info _z80_regs[] = {
    {REG_GPR, A_IDX, "a", 1},     {REG_GPR, C_IDX, "c", 1},
    {REG_GPR, B_IDX, "b", 1},     {REG_GPR, E_IDX, "e", 1},
    {REG_GPR, D_IDX, "d", 1},     {REG_GPR, L_IDX, "l", 1},
    {REG_GPR, H_IDX, "h", 1},     {REG_GPR, IYL_IDX, "iyl", 1},
    {REG_GPR, IYH_IDX, "iyh", 1}, {REG_CND, CND_IDX, "c", 1}};
 ...
 */
static reg_info *allocReg(short type) {
  int i;
...
/*-----------------------------------------------------------------*/
static void useReg(reg_info *reg) { reg->isFree = 0; }
#endif
...
/*-----------------------------------------------------------------*/
static int allLRs(symbol *sym, eBBlock *ebp, iCode *ic) { return 1; }
#endif
...
DEFSETFUNC(isFree) {
  ...
  if (sym->isFree && noOverLap(sym->usl.itmpStack, fsym) &&
...
void z80SpillThis(symbol *sym) {
  ...
  if (!(sym->remat || sym->usl.spillLoc) ||
      (sym->usl.spillLoc &&
       !sym->usl.spillLoc
            ->onStack)) // z80 port currently only supports on-stack spill
                        // locations in code generation.
    createStackSpil(sym);
  else
 ...
 */
reg_info *getRegGpr(iCode *ic, eBBlock *ebp, symbol *sym) {
  reg_info *reg;
  int j;
 ...
 */
static bool symHasReg(symbol *sym, const reg_info *reg) {
  int i;
...
static void deassignLRs(iCode *ic, eBBlock *ebp) {
  ...
  for (sym = hTabFirstItem(liveRanges, &k); sym;
    ...
    /* special case check if this is an IFX &
       ...
       of the symbol */
    if (ic->op == IFX && ic->prev && ic->prev->op == IPOP &&
        !ic->prev->parmPush && !OP_SYMBOL(IC_LEFT(ic->prev))->isspilt)
    ...
    if (sym->nRegs) {
      ...
      if (options.oldralloc && IC_RESULT(ic) &&
          !(SKIP_IC2(ic) || ic->op == JUMPTABLE || ic->op == IFX ||
            ic->op == IPUSH || ic->op == IPOP || ic->op == RETURN) &&
...
static int positionRegs(symbol *result, symbol *opsym) {
  ...
  D(D_ALLOC,
    ("positionRegs: on result %p opsym %p line %u\n", result, opsym, lineno));
...
/*------------------------------------------------------------------*/
static void verifyRegsAssigned(operand *op, iCode *ic) {
  symbol *sym;
...
static void serialRegAssign(eBBlock **ebbs, int count) {
  ...
  for (i = 0; i < count; i++) {
    ...
    for (ic = ebbs[i]->sch; ic; ic = ic->next) {
      ...
      if (SKIP_IC2(ic) || ic->op == JUMPTABLE || ic->op == IFX ||
      ...
      if (IC_RESULT(ic)) {
        ...
        if (willCS) {
          if (sym->usl.spillLoc) {
            symbol *leastUsed =
                leastUsedLR(liveRangesWith(spillable, allLRs, ebbs[i], ic));
            ...
            /* if none of the liveRanges have a spillLocation then better
               to spill this one than anything else already assigned to
               registers */
            if (liveRangesWith(spillable, noSpilLoc, ebbs[i], ic)) {
              /* if this is local to this block then we might find a block spil
               */
              if (!(sym->liveFrom >= ebbs[i]->fSeq &&
        ...
        if ((sym->nRegs == 2) && tryAllocatingRegPair(sym)) {
          ...
          if (sym->nRegs == 2 && sym->regs[0] == sym->regs[1] + 1 &&
              sym->regs[0] != &regsZ80[2]) {
            freeReg(sym->regs[0]);
            freeReg(sym->regs[1]);
...
static void fillGaps() {
  ...
  for (sym = hTabFirstItem(liveRanges, &key); sym;
    ...
    for (i = 0; i < sym->clashes->size; i++) {
      ...
      if (bitVectBitValue(sym->clashes, i) ==
              0 || /* those that clash with this */
          bitVectBitValue(_G.totRegAssigned, i) ==
              0) /* and are still assigned to registers */
        continue;
    ...
    for (i = 0; i < sym->defs->size; i++) {
      if (bitVectBitValue(sym->defs, i)) {
        ...
        if (SKIP_IC(ic))
          continue;
        assert(isSymbolEqual(sym,
                             OP_SYMBOL(IC_RESULT(ic)))); /* just making sure */
...
/*-----------------------------------------------------------------*/
bitVect *rUmaskForOp(const operand *op) {
  bitVect *rumask;
  symbol *sym;
  int j;
  ...
  rumask = newBitVect(_G.nRegs + (2));

  for (j = 0; j < sym->nRegs; j++) {
    if (!(sym->regs[j]) || sym->regs[j]->rIdx < 0 ||
        sym->regs[j]->rIdx > CND_IDX) {
...
bitVect *z80_rUmaskForOp(const operand *op) { return rUmaskForOp(op); }

/** Returns bit vector of registers used in iCode.
 */
bitVect *regsUsedIniCode(iCode *ic) {
  bitVect *rmask = newBitVect(_G.nRegs + (2));
...
static void createRegMask(eBBlock **ebbs, int count) {
  ...
  for (i = 0; i < count; i++) {
    ...
    for (ic = ebbs[i]->sch; ic; ic = ic->next) {
      ...
      ic->rMask = newBitVect(_G.nRegs + 1 + (2));
...
static void regTypeNum(void) {
  ...
  for (sym = hTabFirstItem(liveRanges, &k); sym;
    ...
    if ((sym->liveTo - sym->liveFrom) == 0 && getSize(sym->type) <= 4)
      continue;
    else if ((sym->liveTo - sym->liveFrom) == 0 &&
             bitVectnBitsOn(sym->defs) <= 1) {
...
static int packRegsForAssign(iCode *ic, eBBlock *ebp) {
  ...
  if (!IS_ITEMP(IC_RIGHT(ic)) || OP_SYMBOL(IC_RIGHT(ic))->isind ||
      OP_LIVETO(IC_RIGHT(ic)) > ic->seq)
    return 0;

  /* Avoid having multiple named address spaces in one iCode. */
  if (IS_SYMOP(IC_RESULT(ic)) &&
      SPEC_ADDRSPACE(OP_SYMBOL(IC_RESULT(ic))->etype))
    return 0;
  ...
  for (dic = ic->prev; dic; dic = dic->prev) {
    ...
    if (dic->op == CALL || dic->op == PCALL || dic->op == INLINEASM ||
        dic->op == CRITICAL || dic->op == ENDCRITICAL) {
      dic = NULL;
      break;
    }
    ...
    if (dic->op == IFX) {
      if (IS_SYMOP(IC_COND(dic)) && (IC_COND(dic)->key == IC_RESULT(ic)->key ||
      ...
      if (IS_SYMOP(IC_LEFT(dic)) && (IC_LEFT(dic)->key == IC_RESULT(ic)->key ||
  ...
  if (OP_SYMBOL(IC_RESULT(ic))->onStack || OP_SYMBOL(IC_RESULT(ic))->iaccess) {
    ...
    if (!((IC_LEFT(dic) && IC_RESULT(ic)->key == IC_LEFT(dic)->key) ||
          (IC_RIGHT(dic) && IC_RESULT(ic)->key == IC_RIGHT(dic)->key)))
      return 0;
  }

pack:
  /* Keep assignment if it is an sfr write  - not all of code generation can
   * deal with result in sfr */
  if (IC_RESULT(ic) && IS_TRUE_SYMOP(IC_RESULT(ic)) &&
      SPEC_OCLS(OP_SYMBOL(IC_RESULT(ic))->etype) &&
...
static int packRegsForSupport(iCode *ic, eBBlock *ebp) {
  ...
  /* do the same for the right operand */
right:
  if (!change && IS_ITEMP(IC_RIGHT(ic)) &&
...
static iCode *packRegsForOneuse(iCode *ic, operand *op, eBBlock *ebp) {
  ...
  if (dic->op == CALL || dic->op == PCALL) {
    if (ic->op != SEND && ic->op != RETURN && !POINTER_SET(ic) &&
        !POINTER_GET(ic)) {
  ...
  if (isOperandInFarSpace(IC_LEFT(dic)) || isOperandInFarSpace(IC_RIGHT(dic)) ||
      IS_OP_RUONLY(IC_LEFT(ic)) || IS_OP_RUONLY(IC_RIGHT(ic))) {
    return NULL;
  }
  ...
  for (dic = dic->next; dic && dic != ic; dic = dic->next) {
    ...
    if (isOperandInFarSpace(IC_LEFT(dic)) ||
        ...
        isOperandInFarSpace(IC_RESULT(dic)) || IS_OP_RUONLY(IC_LEFT(dic)) ||
        IS_OP_RUONLY(IC_RIGHT(dic)) || IS_OP_RUONLY(IC_RESULT(dic))) {
      return NULL;
    }
  }
...
static iCode *packRegsForHLUse3(iCode *lic, operand *op, eBBlock *ebp) {
  int i, key;
  symbol *sym;
  iCode *ic, *dic;
  bool isFirst = TRUE;
  ...
  D(D_PACK_HLUSE3,
    ("Checking HL on %p lic key %u first def %u line %u:\n", OP_SYMBOL(op),
     lic->key, bitVectFirstBit(OP_DEFS(op)), lic->lineno));
  if (D_PACK_HLUSE3)
    piCode(lic, NULL);
  ...
  if (!options.oldralloc ? getSize(operandType(op)) != 2
  ...
  for (; ic && ic->seq <= OP_SYMBOL(op)->liveTo;
    ...
    if (IC_RESULT(ic) && IS_SYMOP(IC_RESULT(ic)) &&
        (isOperandInDirSpace(IC_RESULT(ic)) || exstk))
      return NULL;
    ...
    if ((ic->op == '=' && !POINTER_SET(ic)) || ic->op == UNARYMINUS ||
        ic->op == RETURN || ic->op == RIGHT_OP ||
        (ic->op == '-' && getSize(operandType(IC_RESULT(ic))) == 1) ||
        ic->op == BITWISEAND || ic->op == '|' || ic->op == '>' ||
        ic->op == '<' || ic->op == EQ_OP ||
        (ic->op == '+' && getSize(operandType(IC_RESULT(ic))) == 1))
    ...
    /* By default give up */
    D(D_PACK_HLUSE3,
      ("packRegsForHLUse3 giving up at icode %u\n", (unsigned)(ic->op)));
    return NULL;
  }
...
static iCode *packRegsForIYUse(iCode *lic, operand *op, eBBlock *ebp) {
  int i, key;
  symbol *sym;
  iCode *ic, *dic;
  bitVect *uses;
 ...
 */
static bool opPreservesA(iCode *uic) {
  if (uic->op == IFX) {
 ...
 */
static bool opCanUseA(iCode *uic) {
  if (uic->op == IFX) {
    /* If we've gotten this far then the thing to compare must be
       small enough and must be in A.
     */
 ...
 */
static bool opIgnoresA(iCode *ic, iCode *uic) {
  /* A increment of an iTemp by a constant is OK. */
  if (uic->op == '+' && IS_ITEMP(IC_LEFT(uic)) && IS_ITEMP(IC_RESULT(uic)) &&
      IS_OP_LITERAL(IC_RIGHT(uic))) {
...
static void packRegsForAccUse2(iCode *ic) {
  ...
  D(D_ACCUSE2,
    ("packRegsForAccUse2: running on ic %p line %u\n", ic, ic->lineno));
  if (D_ACCUSE2)
    piCode(ic, NULL);

  /* Filter out all but those 'good' commands */
  if (!POINTER_GET(ic) && ic->op != '+' && ic->op != '-' &&
      !IS_BITWISE_OP(ic) && ic->op != '=' && ic->op != EQ_OP && ic->op != '<' &&
      ic->op != '>' && ic->op != CAST && ic->op != GETHBIT &&
...

  {
    ...
    do {
      ...
      /*else if (scan->next == NULL && bitVectnBitsOn (uses) == 1 && next !=
         NULL)
         {
         if (next->prev == NULL)
         {
         ...
         {
         D (D_ACCUSE2, ("  + Dropping as last in list and next doesn't start a
         block\n")); return;
         }
        ...
        if (opIgnoresA(ic, scan->next)) {
          ...
          D(D_ACCUSE2, ("  + Dropping as parts are not consecuitive and "
                        "intermediate might use A\n"));
          return;
        }
      }
...
static void packRegisters(eBBlock *ebp) {
  ...
  for (ic = ebp->sch; ic; ic = ic->next) {
    ...
    if (ic->op == ADDRESS_OF && IS_ITEMP(IC_RESULT(ic)) &&
        bitVectnBitsOn(OP_DEFS(IC_RESULT(ic))) == 1 &&
        !IS_PARM(IC_RESULT(
            ic)) /* The receiving of the paramter isnot accounted for in DEFS */
    ...
    /* if straight assignment then carry remat flag if this is the
       only definition */
    if (ic->op == '=' && !POINTER_SET(ic) && IS_SYMOP(IC_RIGHT(ic)) &&
        OP_SYMBOL(IC_RIGHT(ic))->remat && !isOperandGlobal(IC_RESULT(ic)) &&
        bitVectnBitsOn(OP_SYMBOL(IC_RESULT(ic))->defs) == 1 &&
        !IS_PARM(IC_RESULT(ic)) && /* The receiving of the paramter isnot
                                      accounted for in DEFS */
    ...
    if ((ic->op == '+' || ic->op == '-') &&
        (IS_SYMOP(IC_LEFT(ic)) && IS_ITEMP(IC_RESULT(ic)) &&
         IS_OP_LITERAL(IC_RIGHT(ic))) &&
    ...
    if (!DISABLE_PACK_ONE_USE && POINTER_SET(ic) && IS_SYMOP(IC_RESULT(ic)) &&
        ...
        !OP_SYMBOL(IC_RESULT(ic))->remat && !IS_OP_RUONLY(IC_RIGHT(ic)) &&
    ...
    if (!DISABLE_PACK_ONE_USE && POINTER_GET(ic) && IS_SYMOP(IC_LEFT(ic)) &&
        ...
        !OP_SYMBOL(IC_LEFT(ic))->remat && !IS_OP_RUONLY(IC_RESULT(ic)) &&
    ...
    if ((options.oldralloc || !OPTRALLOC_HL) && !DISABLE_PACK_HL &&
        IS_ITEMP(IC_RESULT(ic)))
      if (!IY_RESERVED)
        packRegsForHLUse3(ic, IC_RESULT(ic), ebp);

    if ((options.oldralloc || !OPTRALLOC_IY) && !DISABLE_PACK_IY &&
        !IY_RESERVED && IS_ITEMP(IC_RESULT(ic)))
 ...
 */
static iCode *joinPushes(iCode *lic) {
  iCode *ic, *uic, *fic;
  ...
  for (ic = lic; ic; ic = ic->next) {
      ...
      continue;
    {
      ...
      if (IFFUNC_ISSMALLC(ftype)) /* SmallC calling convention pushes 8-bit
                                     values as 16 bit */
        continue;
    }
    ...
    /* Both characters? */
    if (getSize(operandType(IC_LEFT(ic))) != 1 ||
        getSize(operandType(IC_LEFT(uic))) != 1) {
      continue;
    }
...
/** Serially allocate registers to the variables.
    ...
    packing.
    In the new register allocator it only serves to mark variables for the new
   register allocator.
 */
static void serialRegMark(eBBlock **ebbs, int count) {
  int i;
  short int max_alloc_bytes =
      SHRT_MAX; // Byte limit. Set this to a low value to pass only few
                // variables to the register allocator. This can be useful for
                // debugging.
    ...
    for (ic = ebbs[i]->sch; ic; ic = ic->next) {
      ...
      if (IC_RESULT(ic)) {
        ...
        /* if it does not need or is spilt
           ...
           or will not live beyond this instructions */
        if (!sym->nRegs || sym->isspilt ||
            bitVectBitValue(_G.regAssigned, sym->key) || sym->for_newralloc ||
            (sym->liveTo <= ic->seq &&
             (sym->nRegs <= 4 || ic->op != CALL && ic->op != PCALL))) {
          D(D_ALLOC, ("serialRegAssign: won't live long enough.\n"));
          continue;
        }
        ...
        if (sym->usl.spillLoc && !sym->usl.spillLoc->_isparm &&
            !USE_OLDSALLOC) // I have no idea where these spill locations come
                            // from. Sometime two symbols even have the same
                            // spill location, whic tends to mess up stack
                            // allocation. Those that come from previous
                            // iterations in this loop would be okay, but those
                            // from outside are a problem.
        {
          sym->usl.spillLoc = 0;
          sym->isspilt = false;
        }
        ...
        if (sym->nRegs >
            4) /* TODO. Change this once we can allocate bigger variables (but
                  still spill when its a big return value). Also change in
                  ralloc2.cc, operand_on-stack in that case*/
        {
...
/*-----------------------------------------------------------------*/
void z80_oldralloc(ebbIndex *ebbi) {
...
/*-----------------------------------------------------------------*/
void z80_assignRegisters(ebbIndex *ebbi) {
#ifdef OLDRALLOC
  if (options.oldralloc)
    z80_oldralloc(ebbi);
  else
