...
static OPTION _z80_options[] = {
    {0, OPTION_CALLEE_SAVES_BC, &z80_opts.calleeSavesBC,
     "Force a called function to always save BC"},
    {0, OPTION_CODE_SEG, &options.code_seg,
...
static int do_pragma(int id, const char *name, const char *cp) {
...
static int _process_pragma(const char *s) {
  return process_pragma_tbl(pragma_tbl, s);
}

static bool _parseOptions(int *pargc, char **argv, int *i) { return FALSE; }
...
static void _setValues(void) {
  ...
  if (options.nostdlib == FALSE) {
    ...
    for (s = setFirstItem(libDirsSet); s != NULL; s = setNextItem(libDirsSet)) {
      ...
      path = buildCmdLine2("%s" DIR_SEPARATOR_STRING
                           "{port}" DIR_SEPARATOR_STRING "crt0{objext}",
                           s);
  ...
  dbuf_init(&dbuf, 128);
  dbuf_printf(&dbuf, "-b_CODE=0x%04X -b_DATA=0x%04X", options.code_loc,
              options.data_loc);
  setMainValue("z80bases", dbuf_c_str(&dbuf));
  dbuf_destroy(&dbuf);

  /* For the old register allocator (with the new one we decide to omit the
   * frame pointer for each function individually) */
...
static void _setDefaultOptions(void) {
  /* SirCmpwn NOTE: We may want to change these defaults at some point,
   * particular with respect to code locations */
  options.nopeep = 0;
  options.stackAuto = 1;
...
static const char *_getRegName(const struct reg_info *reg) {
  if (reg) {
    return reg->name;
  }
  /*  assert (0); */
...
static const char *_z80LinkCmd[] = {"scas", "-o", "$1", "$1.o", "$l", NULL};

/* $3 is replaced by assembler.debug_opts resp. port->assembler.plain_opts */
static const char *_z80AsmCmd[] = {"scas",   "-c", "-o", "$2",
                                   "$1.asm", "$l", NULL};

static const char *const _crt[] = {
    "knightos-crt0.o",
    NULL,
};

static const char *const _libs_z80[] = {
    "z80",
    NULL,
};
...
/* Globals */
PORT z80_port = {
    3 /* SirCmpwn NOTE: This was TARGET_ID_Z80, which isn't in use in theory. We
         should eventually get rid of port targets. */
    ,
    "z80",
    ...
    {0x00, 0x40, 0x60, 0x80}, /* far, near, xstack, code */
    {
        ...
        "INITIALIZER", /* name of segment for copies of initialized variables in
                          code space */
        NULL,
        NULL,
        1, /* CODE  is read-only */
    ...
    9, /* Number of registers handled in the tree-decomposition-based register
          allocator in SDCCralloc.hpp */
    PORT_MAGIC};
