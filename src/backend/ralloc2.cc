  ...
  if (o && IS_SYMOP(o)) {
    ...
    if (oi != oi_end) {
      ...
      if (a.local.find(v) != a.local.end()) {
        ...
        // than bc, de:
        if ((size == 2 || size == 4) &&
            (byteregs[1] != byteregs[0] + 1 || byteregs[0] != REG_C &&
                                                   byteregs[0] != REG_E &&
                                                   byteregs[0] != REG_L))
          c += 2.0f;
...
static bool Ainst_ok(const assignment &a, unsigned short int i, const G_t &G,
  ...
  // std::cout << "Ainst_ok: A = (" << ia.registers[REG_A][0] << ", " <<
  // ia.registers[REG_A][1] << "), inst " << i << ", " << ic->key << "\n";

  // Code generator cannot handle variables that are only partially in A.
  if (I[ia.registers[REG_A][1]].size > 1 ||
      ia.registers[REG_A][0] >= 0 && I[ia.registers[REG_A][0]].size > 1)
    return (false);
  ...
  if (ic->op == BITWISEAND && ifxForOp(IC_RESULT(ic), ic) &&
      (IS_OP_LITERAL(left) && (!(IS_GB && IS_TRUE_SYMOP(right) ||
                                 exstk && operand_on_stack(right, a, i, G)) ||
                               operand_in_reg(right, ia, i, G) &&
...
nobit:
  ...
  if (ic->op == '=' && POINTER_SET(ic))
    return (dying.find(ia.registers[REG_A][1]) != dying.end() ||
        ...
        (operand_in_reg(left, REG_A, ia, i, G) ||
         operand_in_reg(left, REG_B, ia, i, G) &&
             (getSize(operandType(left)) < 2 ||
              operand_in_reg(left, REG_C, ia, i, G) &&
                  I[ia.registers[REG_C][1]].byte == 0) ||
         operand_in_reg(left, REG_D, ia, i, G) &&
             (getSize(operandType(left)) < 2 ||
              operand_in_reg(left, REG_E, ia, i, G) &&
  ...
  if (input_in_A &&
      (result_in_A || dying.find(ia.registers[REG_A][1]) != dying.end() ||
       dying.find(ia.registers[REG_A][0]) != dying.end())) {
...
static bool HLinst_ok(const assignment &a, unsigned short int i, const G_t &G,
  ...
  if (exstk &&
      ...
      (result && IS_SYMOP(result) && getSize(operandType(result)) >= 2 ||
       !result_only_HL)) // Todo: Make this more accurate to get better code
  ...
  if (ic->op == IPUSH && input_in_H &&
      (getSize(operandType(IC_LEFT(ic))) <= 2 ||
       ia.registers[REG_L][1] > 0 && I[ia.registers[REG_L][1]].byte == 2 &&
           ia.registers[REG_H][1] > 0 && I[ia.registers[REG_H][1]].byte == 3))
    return (true);
  ...
  if (ic->op == IPUSH && getSize(operandType(left)) == 2 &&
      (ia.registers[REG_C][1] < 0 && ia.registers[REG_B][1] < 0 ||
       ia.registers[REG_E][1] < 0 &&
           ia.registers[REG_D][1] < 0)) // Can use pair other than HL.
    return (true);
  if (ic->op == IPUSH && getSize(operandType(left)) <= 2 &&
      (operand_in_reg(left, REG_C, ia, i, G) &&
           I[ia.registers[REG_C][1]].byte == 0 &&
  ...
  if (POINTER_GET(ic) && getSize(operandType(IC_RESULT(ic))) == 1 &&
      ...
      (operand_in_reg(right, REG_C, ia, i, G) &&
           I[ia.registers[REG_C][1]].byte == 0 &&
           operand_in_reg(right, REG_B, ia, i, G) || // Uses ld a, (bc)
       operand_in_reg(right, REG_E, ia, i, G) &&
           I[ia.registers[REG_E][1]].byte == 0 &&
           operand_in_reg(right, REG_D, ia, i, G) || // Uses ld a, (de)
       operand_in_reg(right, REG_IYL, ia, i, G) &&
           I[ia.registers[REG_IYL][1]].byte == 0 &&
           operand_in_reg(right, REG_IYH, ia, i, G))) // Uses ld r, 0 (iy)
    return (true);
...
static bool IYinst_ok(const assignment &a, unsigned short int i, const G_t &G,
  ...
  /*if(ic->key == 40)
     ...
     ")\nIYL = (" << ia.registers[REG_IYL][0] << ", " <<
     ia.registers[REG_IYL][1] << "), IYH = (" << ia.registers[REG_IYH][0] << ",
     " << ia.registers[REG_IYH][1] << ")inst " << i << ", " << ic->key <<
     "\n";*/
...
static bool assignment_hopeless(const assignment &a, unsigned short int i,
  ...
  // Code generator cannot handle variables that are only partially in IY.
  if (OPTRALLOC_IY && (ia.registers[REG_IYL][1] >= 0 &&
                           (I[ia.registers[REG_IYL][1]].size != 2 ||
                            I[ia.registers[REG_IYL][1]].byte != 0) ||
                       ia.registers[REG_IYH][1] >= 0 &&
                           (I[ia.registers[REG_IYH][1]].size != 2 ||
                            I[ia.registers[REG_IYH][1]].byte != 1) ||
                       ia.registers[REG_IYL][0] >= 0 &&
                           (I[ia.registers[REG_IYL][0]].size != 2 ||
                            I[ia.registers[REG_IYL][0]].byte != 0) ||
                       ia.registers[REG_IYH][0] >= 0 &&
                           (I[ia.registers[REG_IYH][0]].size != 2 ||
                            I[ia.registers[REG_IYH][0]].byte != 1)))
    return (true);
  ...
  for (v = a.local.begin(), v_end = a.local.end(); v != v_end; ++v) {
    ...
    if ((I[*v].byte % 2) &&
        (a.global[*v] == REG_L || a.global[*v] == REG_E ||
         a.global[*v] == REG_C)) // Try not to reverse bytes.
      c += 8.0f;
    if (!(I[*v].byte % 2) && I[*v].size > 1 &&
        (a.global[*v] == REG_H || a.global[*v] == REG_D ||
         a.global[*v] == REG_B)) // Try not to reverse bytes.
      c += 8.0f;
...
static bool tree_dec_ralloc(T_t &T, const G_t &G, const I_t &I) {
  ...
  for (unsigned int v = 0; v < boost::num_vertices(I); v++) {
    ...
    if (winner.global[v] >= 0) {
      ...
      sym->accuse = 0;
      sym->nRegs = I[v].size;
      // spillThis(sym); Leave it to regFix, which can do some spillocation
      // compaction. Todo: Use Thorup instead.
      sym->isspilt = false;
    }
  }
