...
// #define DEBUG_RALLOC_DEC // Uncomment to get debug messages while doing
// register allocation on the tree decomposition. #define DEBUG_RALLOC_DEC_ASS
// // Uncomment to get debug messages about assignments while doing register
// allocation on the tree decomposition (much more verbose than the one above).
...
float default_operand_cost(const operand *o, const assignment &a,
                           unsigned short int i, const G_t &G, const I_t &I) {
  float c = 0.0f;

  operand_map_t::const_iterator oi, oi_end;

  var_t byteregs[4]; // Todo: Change this when sdcc supports variables larger
                     // than 4 bytes in registers.
  ...
  if (o && IS_SYMOP(o)) {
    ...
    if (oi != oi_end) {
      ...
      if (std::binary_search(a.local.begin(), a.local.end(), v)) {
        ...
        while (++oi != oi_end) {
          v = oi->second;
          c += (std::binary_search(a.local.begin(), a.local.end(), v)
                    ? 1.0f
        ...
        // Todo: Extend this once the register allcoator can use registers other
        // than bc, de:
        if ((size == 2 || size == 4) &&
            (byteregs[1] != byteregs[0] + 1 ||
             (byteregs[0] != REG_C && byteregs[0] != REG_E &&
              byteregs[0] != REG_L)))
          c += 2.0f;
        if (size == 4 && (byteregs[3] != byteregs[2] + 1 ||
                          (byteregs[2] != REG_C && byteregs[2] != REG_E &&
                           byteregs[0] != REG_L)))
          c += 2.0f;
        ...
        if (byteregs[0] == REG_A)
          ...
          c -= 0.1f;
        else if ((OPTRALLOC_IY && byteregs[0] == REG_IYL) ||
                 byteregs[0] == REG_IYH)
          c += 0.1f;
      }
      // Spilt.
...
template <class G_t, class I_t>
static bool operand_sane(const operand *o, const assignment &a,
                         unsigned short int i, const G_t &G, const I_t &I) {
  if (!o || !IS_SYMOP(o))
    return (true);
...
template <class G_t, class I_t>
static float default_instruction_cost(const assignment &a, unsigned short int i,
...
template <class G_t, class I_t>
static bool inst_sane(const assignment &a, unsigned short int i, const G_t &G,
                      const I_t &I) {
  const iCode *ic = G[i].ic;

  // for a sequence of built-in SENDs, all of the SENDs must be sane
  if (ic->op == SEND && ic->builtinSEND && ic->next->op == SEND &&
      !inst_sane(a, *(adjacent_vertices(i, G).first), G, I))
    return (false);
...
template <class G_t, class I_t>
static float assign_cost(const assignment &a, unsigned short int i,
                         const G_t &G, const I_t &I) {
  float c = 0.0f;
  ...
  reg_t byteregs[4] = {
      -1, -1, -1, -1}; // Todo: Change this when sdcc supports variables larger
                       // than 4 bytes in register allocation for z80.
  ...
  boost::tie(oi, oi_end) =
      G[i].operands.equal_range(OP_SYMBOL_CONST(right)->key);
  if (oi != oi_end) {
    var_t v = oi->second;
  ...
  boost::tie(oi, oi_end) =
      G[i].operands.equal_range(OP_SYMBOL_CONST(result)->key);
  if (oi != oi_end) {
    var_t v = oi->second;
...
template <class G_t, class I_t>
static float return_cost(const assignment &a, unsigned short int i,
                         const G_t &G, const I_t &I) {
  float c = 0.0f;
  ...
  reg_t byteregs[4] = {-1, -1, -1, -1}; // Todo: Change this when sdcc supports
                                        // variables larger than 4 bytes.
  ...
  boost::tie(oi, oi_end) =
      G[i].operands.equal_range(OP_SYMBOL_CONST(left)->key);
  if (oi != oi_end) {
    var_t v = oi->second;
...
template <class G_t, class I_t>
static float call_cost(const assignment &a, unsigned short int i, const G_t &G,
                       const I_t &I) {
  float c = 0.0f;
...
template <class G_t, class I_t>
static float ifx_cost(const assignment &a, unsigned short int i, const G_t &G,
                      const I_t &I) {
  const iCode *ic = G[i].ic;
...
template <class G_t, class I_t>
static float jumptab_cost(const assignment &a, unsigned short int i,
                          const G_t &G, const I_t &I) {
  const iCode *ic = G[i].ic;
...
static void add_operand_conflicts_in_node(const cfg_node &n, I_t &I) {
  ...
  if (!(ic->op == UNARYMINUS || ic->op == '+' || ic->op == '-' ||
    ...
    return; // Code generation can always handle all other operations. Todo:
            // Handle ^, |, BITWISEAND and float UNARYMINUS there as well.
...
template <class G_t>
static bool operand_in_reg(const operand *o, const i_assignment_t &ia,
                           unsigned short int i, const G_t &G) {
  if (!o || !IS_SYMOP(o))
    return (false);
...
// Return true, iff the operand is placed in a reg.
template <class G_t>
static bool operand_byte_in_reg(const operand *o, int offset, reg_t r,
                                const assignment &a, unsigned short int i,
                                const G_t &G) {
  if (!o || !IS_SYMOP(o))
    return (false);
...
template <class G_t>
bool operand_on_stack(const operand *o, const assignment &a,
                      unsigned short int i, const G_t &G) {
  if (!o || !IS_SYMOP(o))
    return (false);
...
template <class G_t>
static bool operand_is_pair(const operand *o, const assignment &a,
                            unsigned short int i, const G_t &G) {
  if (!o || !IS_SYMOP(o))
    return (false);
  ...
  if (a.global[oi->second] != REG_C && a.global[oi->second] != REG_E &&
      a.global[oi->second] != REG_L && a.global[oi->second] != REG_IYL)
    return (false);
...
template <class G_t, class I_t>
static bool Ainst_ok(const assignment &a, unsigned short int i, const G_t &G,
                     const I_t &I) {
  const iCode *ic = G[i].ic;
  ...
  bool exstk =
      (should_omit_frame_ptr || (currFunc && currFunc->stack > 127) || IS_GB);

  // std::cout << "Ainst_ok at " << G[i].ic->key << ": A = (" <<
  // ia.registers[REG_A][0] << ", " << ia.registers[REG_A][1] << "), inst " << i
  // << ", " << ic->key << "\n";
  ...
  case JUMPTABLE:
    ...
    break;
  default:
    input_in_A = operand_in_reg(left, REG_A, ia, i, G) ||
                 operand_in_reg(right, REG_A, ia, i, G);
    break;
  }
  ...
  if (ic->op == '^' || ic->op == BITWISEAND || ic->op == '|' || ic->op == '~' ||
      ic->op == LEFT_OP) // Codegen can handle it all.
    return (true);

  if (ic->op == RIGHT_OP && getSize(operandType(result)) == 1 &&
      IS_OP_LITERAL(right))
    return (true);
  ...
  if ((ic->op == EQ_OP || ic->op == '<' &&
      ...
      getSize(operandType(IC_LEFT(ic))) == 1 && ifxForOp(IC_RESULT(ic), ic) &&
      operand_in_reg(left, REG_A, ia, i, G) &&
      (IS_OP_LITERAL(right) || operand_in_reg(right, REG_C, ia, i, G) ||
       operand_in_reg(right, REG_B, ia, i, G) ||
       operand_in_reg(right, REG_E, ia, i, G) ||
       operand_in_reg(right, REG_D, ia, i, G) ||
       operand_in_reg(right, REG_H, ia, i, G) ||
       operand_in_reg(right, REG_L, ia, i, G)))
    return (true);

  const cfg_dying_t &dying = G[i].dying;
  const bool dying_A = result_in_A ||
                       dying.find(ia.registers[REG_A][1]) != dying.end() ||
                       dying.find(ia.registers[REG_A][0]) != dying.end();

  if ((ic->op == '+' ||
       ic->op == '-' && !operand_in_reg(right, REG_A, ia, i, G) ||
       ic->op == UNARYMINUS && !IS_GB) &&
  ...
  if ((ic->op == '+' ||
    ...
    if ((operand_byte_in_reg(left, 0, REG_A, a, i, G) ||
         !operand_in_reg(left, REG_A, ia, i, G)) &&
        (operand_byte_in_reg(right, 0, REG_A, a, i, G) ||
         !operand_in_reg(right, REG_A, ia, i, G)) &&
        (operand_byte_in_reg(result, getSize(operandType(IC_RESULT(ic))) - 1,
                             REG_A, a, i, G) ||
         !result_in_A))
      return (true);
  }
  ...
  if (ic->op == IFX && dying_A &&
      (getSize(operandType(left)) >= 1 &&
           operand_byte_in_reg(left, 0, REG_A, a, i, G) ||
       getSize(operandType(left)) >= 2 && !IS_FLOAT(operandType(left)) &&
           operand_byte_in_reg(left, 1, REG_A, a, i, G)))
    return (true);

  // Can test register via inc / dec.
  if (ic->op == IFX && getSize(operandType(left)) == 1 &&
      (operand_byte_in_reg(left, 0, REG_B, a, i, G) ||
       operand_byte_in_reg(left, 0, REG_C, a, i, G) ||
       operand_byte_in_reg(left, 0, REG_D, a, i, G) ||
       operand_byte_in_reg(left, 0, REG_E, a, i, G) ||
       operand_byte_in_reg(left, 0, REG_H, a, i, G) ||
       operand_byte_in_reg(left, 0, REG_L, a, i, G)))
    return (true);

  // Plain assignment between registers
  if (ic->op == CAST && getSize(operandType(IC_RESULT(ic))) == 1 &&
      (operand_in_reg(result, REG_B, ia, i, G) ||
       operand_in_reg(result, REG_C, ia, i, G) ||
       operand_in_reg(result, REG_D, ia, i, G) ||
       operand_in_reg(result, REG_E, ia, i, G) ||
       operand_in_reg(result, REG_A, ia, i, G)) &&
      (operand_in_reg(result, REG_B, ia, i, G) ||
       operand_in_reg(result, REG_C, ia, i, G) ||
       operand_in_reg(result, REG_D, ia, i, G) ||
       operand_in_reg(result, REG_E, ia, i, G)))
  ...
  if (ic->op == CAST && operand_in_reg(right, REG_A, ia, i, G) &&
      ...
      (operand_in_reg(result, REG_C, ia, i, G) ||
       operand_in_reg(result, REG_E, ia, i, G) ||
       operand_in_reg(result, REG_L, ia, i, G)))
    return (true);

  // Last byte of output may be in A.
  if ((ic->op == GET_VALUE_AT_ADDRESS ||
       ic->op == CAST && !operand_in_reg(right, REG_A, ia, i, G)) &&
      IS_ITEMP(result) &&
      operand_byte_in_reg(result, getSize(operandType(IC_RESULT(ic))) - 1,
                          REG_A, a, i, G))
    return (true);
  ...
  if ((ic->op == '+' || ic->op == '-') && IS_OP_LITERAL(right) &&
      ulFromVal(OP_VALUE_CONST(right)) <= 2 &&
      (getSize(operandType(IC_RESULT(ic))) == 2 &&
           operand_is_pair(IC_RESULT(ic), a, i, G) ||
       getSize(operandType(IC_RESULT(ic))) == 1 &&
           operand_in_reg(result, ia, i, G) &&
           operand_in_reg(result, ia, i, G)))
    return (true);

  if (ic->op ==
      GET_VALUE_AT_ADDRESS) // Any register can be assigned from (hl) and (iy),
                            // so we don't need to go through a then.
    return (!IS_BITVAR(getSpec(operandType(result))) &&
            (getSize(operandType(result)) == 1 ||
             operand_is_pair(left, a, i, G) &&
                 (operand_in_reg(left, REG_L, ia, i, G) &&
                      !ulFromVal(OP_VALUE_CONST(right)) ||
                  operand_in_reg(left, REG_IYL, ia, i, G) &&
                      ulFromVal(OP_VALUE_CONST(right)) <= 127)));

  if (ic->op == '=' &&
      POINTER_SET(ic) && // Any register can be assigned to (hl) and (iy), so we
                         // don't need to go through a then.
      !(IS_BITVAR(getSpec(operandType(result))) ||
        IS_BITVAR(getSpec(operandType(right)))) &&
      (getSize(operandType(right)) == 1 ||
       operand_is_pair(result, a, i, G) &&
           (operand_in_reg(result, REG_L, ia, i, G) ||
            operand_in_reg(result, REG_IYL, ia, i, G))))
    return (true);
  ...
  if (ic->op == '=' && POINTER_SET(ic))
    return (dying_A || !(IS_BITVAR(getSpec(operandType(result))) ||
                         IS_BITVAR(getSpec(operandType(right)))));

  if (1) {
    // Variable in A is not used by this instruction
    if (ic->op == '+' && IS_ITEMP(left) && IS_ITEMP(IC_RESULT(ic)) &&
        IS_OP_LITERAL(right) && ulFromVal(OP_VALUE_CONST(right)) == 1 &&
    ...
    if ((ic->op == '=' || ic->op == CAST) && !POINTER_SET(ic) &&
        ...
        (operand_in_reg(right, REG_A, ia, i, G) ||
         operand_in_reg(right, REG_B, ia, i, G) ||
         operand_in_reg(right, REG_C, ia, i, G) ||
         operand_in_reg(right, REG_D, ia, i, G) ||
         operand_in_reg(right, REG_E, ia, i, G) ||
         operand_in_reg(right, REG_H, ia, i, G) ||
         operand_in_reg(right, REG_L, ia, i, G)) &&
  ...
  if (input_in_A && dying_A) {
    if (ic->op != IFX && ic->op != RETURN &&
        !((ic->op == RIGHT_OP || ic->op == LEFT_OP) &&
          (IS_OP_LITERAL(right) || operand_in_reg(right, REG_A, ia, i, G) ||
           getSize(operandType(IC_RESULT(ic))) == 1 &&
               ia.registers[REG_B][1] < 0)) &&
        !((ic->op == '=' || ic->op == CAST) &&
      ...
      // std::cout << "Last use: Dropping at " << i << ", " << ic->key << "(" <<
      // int(ic->op) << ")\n";
      return (false);
    }
  }
  // A is used, and has to be preserved for later use.
    ...
    // std::cout << "Intermediate use: Dropping at " << i << ", " << ic->key <<
    // "(" << int(ic->op) << "\n";
    return (false);
  }
  ...
  if (result_in_A && !POINTER_GET(ic) && ic->op != '+' && ic->op != '-' &&
    ...
    // std::cout << "First use: Dropping at " << i << ", " << ic->key << "(" <<
    // int(ic->op) << "\n";
    return (false);
  }
...
template <class G_t, class I_t>
static bool HLinst_ok(const assignment &a, unsigned short int i, const G_t &G,
                      const I_t &I) {
  const iCode *ic = G[i].ic;
  ...
  case JUMPTABLE:
    ...
    break;
  default:
    input_in_L = operand_in_reg(left, REG_L, ia, i, G) ||
                 operand_in_reg(right, REG_L, ia, i, G);
    input_in_H = operand_in_reg(left, REG_H, ia, i, G) ||
                 operand_in_reg(right, REG_H, ia, i, G);
    break;
  }
  ...
  bool dying_L = result_in_L ||
                 dying.find(ia.registers[REG_L][1]) != dying.end() ||
                 dying.find(ia.registers[REG_L][0]) != dying.end();
  bool dying_H = result_in_H ||
                 dying.find(ia.registers[REG_H][1]) != dying.end() ||
                 dying.find(ia.registers[REG_H][0]) != dying.end();
  ...
  // Due to lack of ex hl, (sp), the generic push code generation fallback
  // doesn't work for gbz80, so we need to be able to use hl if we can't just
  // push a pair or use a.
  if (IS_GB && ic->op == IPUSH && !operand_is_pair(left, a, i, G) &&
      ia.registers[REG_A][1] >= 0 &&
      !(getSize(operandType(left)) == 1 &&
        (operand_in_reg(left, REG_A, ia, i, G) ||
         operand_in_reg(left, REG_B, ia, i, G) ||
         operand_in_reg(left, REG_D, ia, i, G) ||
         operand_in_reg(left, REG_H, ia, i, G))))
    return (false);

  if (IS_GB && !result_only_HL &&
  ...
  // For some operations, the gbz80 stack access using hl will trash the value
  // there.
  if (IS_GB && (ic->op == IPUSH && operand_on_stack(left, a, i, G) ||
                ic->op == IFX && operand_on_stack(IC_COND(ic), a, i, G)))
    return (false);
  if (IS_GB &&
      (operand_on_stack(result, a, i, G) || operand_on_stack(left, a, i, G) ||
       operand_on_stack(right, a, i, G)) &&
      (ic->op == RIGHT_OP || ic->op == LEFT_OP || ic->op == '=' ||
       ic->op == CAST || ic->op == '-' || ic->op == UNARYMINUS ||
       ic->op == BITWISEAND || ic->op == '|') &&
      !(result_only_HL &&
        getSize(operandType(result)) ==
            1)) // Size of result needs to be checked after checking ic->op to
                // esnure that there is a result operand.
    return (false);

  if (IS_GB && ic->op == GET_VALUE_AT_ADDRESS &&
      !(result_only_HL || getSize(operandType(result)) == 1))
    return (false);
  if (IS_GB && POINTER_GET(ic) &&
      !(result_only_HL || getSize(operandType(right)) == 1))
    return (false);
  ...
  if (ic->op == PCALL && ic->prev && ic->prev->op == SEND && input_in_HL &&
      IFFUNC_ISZ88DK_FASTCALL(operandType(IC_LEFT(ic))->next))
    return (false);
  ...
  if (ic->op == '-' && getSize(operandType(result)) == 2 &&
      IS_TRUE_SYMOP(left) && IS_TRUE_SYMOP(right) && result_only_HL)
    return (true);
  ...
  if (exstk &&
      ...
      (result && IS_SYMOP(result) && getSize(operandType(result)) >= 2 ||
       !result_only_HL))
    // Todo: Make this more accurate to get better code when using
    // --fomit-frame-pointer
    return (false);
  if (exstk &&
  ...
  if (ic->op == '+' && getSize(operandType(result)) >= 2 && input_in_HL &&
      ((exstk ? operand_on_stack(left, a, i, G) : IS_TRUE_SYMOP(left)) &&
           (ia.registers[REG_L][1] > 0 || ia.registers[REG_H][1] > 0) ||
       (exstk ? operand_on_stack(right, a, i, G) : IS_TRUE_SYMOP(right)) &&
           (ia.registers[REG_L][1] > 0 || ia.registers[REG_H][1] > 0)))
    return (false);
  ...
  if (ic->op == '+' && getSize(operandType(result)) == 2 &&
      ...
      (operand_in_reg(result, REG_L, ia, i, G) &&
       I[ia.registers[REG_L][1]].byte == 0 &&
       operand_in_reg(result, REG_H, ia, i, G)))
    return (true); // Uses inc hl.

  if (!IS_GB && ic->op == '+' && getSize(operandType(result)) == 2 &&
      !IS_TRUE_SYMOP(result) &&
      (result_only_HL || operand_in_reg(result, REG_IYL, ia, i, G) &&
                             operand_in_reg(result, REG_IYH, ia, i, G)) &&
      (ia.registers[REG_C][1] < 0 && ia.registers[REG_B][1] < 0 ||
       ia.registers[REG_E][1] < 0 &&
           ia.registers[REG_D][1] < 0)) // Can use ld rr, (nn) instead of (hl).
    return (true);
  ...
  if (!IS_GB && ic->op == '+' && getSize(operandType(result)) == 2 &&
      ...
      (IS_OP_LITERAL(right) && ulFromVal(OP_VALUE(IC_RIGHT(ic))) <= 3 ||
       IS_OP_LITERAL(left) && ulFromVal(OP_VALUE(IC_LEFT(ic))) <= 3) &&
      (operand_in_reg(result, REG_C, ia, i, G) &&
           I[ia.registers[REG_C][1]].byte == 0 &&
           operand_in_reg(result, REG_B, ia, i, G) ||
       operand_in_reg(result, REG_E, ia, i, G) &&
           I[ia.registers[REG_E][1]].byte == 0 &&
           operand_in_reg(result, REG_D, ia, i,
                          G))) // Can use ld rr, (nn) followed by inc rr
    return (true);
  ...
  if ((ic->op == '+' || ic->op == '-' || ic->op == UNARYMINUS) &&
      ...
      (IS_TRUE_SYMOP(result) && !operand_on_stack(result, a, i, G) ||
       ...
       (operand_on_stack(right, a, i, G)
            ? exstk
            : IS_TRUE_SYMOP(right)))) // Might use (hl).
    return (false);

  if (ic->op == '+' && input_in_HL &&
      (operand_on_stack(result, a, i, G)
           ? exstk
           : IS_TRUE_SYMOP(result))) // Might use (hl) for result.
    return (false);
  ...
  if ((ic->op == LEFT_OP || ic->op == RIGHT_OP) &&
      (getSize(operandType(result)) <= 1 || !IS_TRUE_SYMOP(result) ||
       !(IS_GB || IY_RESERVED)) &&
      (!exstk ||
  ...
  if (!IS_GB && ic->op == ADDRESS_OF &&
      (operand_in_reg(result, REG_IYL, ia, i, G) &&
           ...
           I[ia.registers[REG_IYL][1]].byte == 0 &&
           operand_in_reg(result, REG_IYH, ia, i, G) ||
       !OP_SYMBOL_CONST(left)->onStack &&
           operand_in_reg(result, REG_C, ia, i, G) &&
           ia.registers[REG_C][1] > 0 && I[ia.registers[REG_C][1]].byte == 0 &&
           operand_in_reg(result, REG_B, ia, i, G) ||
       !OP_SYMBOL_CONST(left)->onStack &&
           operand_in_reg(result, REG_E, ia, i, G) &&
           ia.registers[REG_E][1] > 0 && I[ia.registers[REG_E][1]].byte == 0 &&
           operand_in_reg(result, REG_D, ia, i, G)))
    return (true);
  ...
  if ((!POINTER_SET(ic) && !POINTER_GET(ic) &&
       ((ic->op == '=' || ic->op == CAST || ic->op == UNARYMINUS ||
         ...
         (ic->op == '+' && getSize(operandType(IC_RESULT(ic))) <= 2 &&
          (result_only_HL || !IS_GB)))))) // 16 bit addition on gbz80 might need
                                          // to use add hl, rr.
    return (true);

  if ((ic->op == '<' || ic->op == '>') &&
      (IS_ITEMP(left) || IS_OP_LITERAL(left) || IS_ITEMP(right) ||
       IS_OP_LITERAL(right))) // Todo: Fix for large stack.
    return (true);
  ...
  if ((ic->op == '=') && POINTER_SET(ic) &&
      operand_in_reg(result, REG_IYL, ia, i, G) &&
      I[ia.registers[REG_IYL][1]].byte == 0 &&
      operand_in_reg(result, REG_IYH, ia, i, G)) // Uses ld 0 (iy), l etc
    return (true);

  if ((ic->op == '=' || ic->op == CAST) && POINTER_SET(ic) &&
      !result_only_HL) // loads result pointer into (hl) first.
    return (false);
  ...
  if (ic->op == GET_VALUE_AT_ADDRESS || POINTER_SET(ic) ||
      ic->op == ADDRESS_OF || ic->op == '*' ||
      ic->op == JUMPTABLE) // Some operations always use hl. TODO: See if they
                           // can be changed to save / restore a hl in use or
                           // use hl only when free.
    return (false);
  if (exstk && (operand_on_stack(result, a, i, G) || IS_TRUE_SYMOP(result) ||
                operand_on_stack(left, a, i, G) || IS_TRUE_SYMOP(left) ||
                operand_on_stack(right, a, i, G) ||
                IS_TRUE_SYMOP(right))) // hl used as pointer to operand.
    return (false);
...
template <class G_t, class I_t>
static bool IYinst_ok(const assignment &a, unsigned short int i, const G_t &G,
                      const I_t &I) {
  const iCode *ic = G[i].ic;
  ...
  /*if(ic->key == 40)
    ...
    ia.registers[REG_IYL][0] << ", " << ia.registers[REG_IYL][1] << "), IYH = ("
    << ia.registers[REG_IYH][0] << ", " << ia.registers[REG_IYH][1] << ")inst "
    << i << ", " << ic->key << "\n";*/
  ...
  case JUMPTABLE:
    ...
    break;
  default:
    input_in_IYL = operand_in_reg(left, REG_IYL, ia, i, G) ||
                   operand_in_reg(right, REG_IYL, ia, i, G);
    input_in_IYH = operand_in_reg(left, REG_IYH, ia, i, G) ||
                   operand_in_reg(right, REG_IYH, ia, i, G);
    break;
  }
  ...
  // bool dying_IYL = result_in_IYL || dying.find(ia.registers[REG_IYL][1]) !=
  // dying.end() || dying.find(ia.registers[REG_IYL][0]) != dying.end(); bool
  // dying_IYH = result_in_IYH || dying.find(ia.registers[REG_IYH][1]) !=
  // dying.end() || dying.find(ia.registers[REG_IYH][0]) != dying.end();

  // bool result_only_IY = (result_in_IYL || unused_IYL || dying_IYL) &&
  // (result_in_IYH || unused_IYH || dying_IYH);
  ...
  if (exstk &&
      (operand_on_stack(result, a, i, G) || operand_on_stack(left, a, i, G) ||
       operand_on_stack(right, a, i,
                        G))) // Todo: Make this more accurate to get better code
  ...
  if (ic->op == IPUSH && operand_in_reg(left, REG_IYL, ia, i, G) &&
      operand_in_reg(left, REG_IYH, ia, i, G) &&
      (I[ia.registers[REG_IYL][1]].byte == 0 &&
           I[ia.registers[REG_IYH][1]].byte == 1 ||
       I[ia.registers[REG_IYL][1]].byte == 2 &&
           I[ia.registers[REG_IYH][1]].byte == 3))
    return (true);

  // Code generator mostly cannot handle variables that are only partially in
  // IY.
  if (unused_IYL ^ unused_IYH)
    return (false);
  if (!unused_IYL && I[ia.registers[REG_IYL][1]].size != 2 ||
      !unused_IYH && I[ia.registers[REG_IYH][1]].size != 2 ||
      ia.registers[REG_IYL][0] >= 0 && I[ia.registers[REG_IYL][0]].size != 2 ||
      ia.registers[REG_IYH][0] >= 0 && I[ia.registers[REG_IYH][0]].size != 2)
    return (false);
  if (ia.registers[REG_IYL][1] >= 0 &&
      (ia.registers[REG_IYH][1] <= 0 ||
       I[ia.registers[REG_IYL][1]].v != I[ia.registers[REG_IYH][1]].v))
    return (false);
  if (ia.registers[REG_IYH][1] >= 0 &&
      (ia.registers[REG_IYL][1] <= 0 ||
       I[ia.registers[REG_IYH][1]].v != I[ia.registers[REG_IYL][1]].v))
    return (false);
  if (ia.registers[REG_IYL][0] >= 0 &&
      (ia.registers[REG_IYH][0] <= 0 ||
       I[ia.registers[REG_IYL][0]].v != I[ia.registers[REG_IYH][0]].v))
    return (false);
  if (ia.registers[REG_IYH][0] >= 0 &&
      (ia.registers[REG_IYL][0] <= 0 ||
       I[ia.registers[REG_IYH][0]].v != I[ia.registers[REG_IYL][0]].v))
    return (false);
  if (I[ia.registers[REG_IYL][1]].byte != 0 ||
      I[ia.registers[REG_IYH][1]].byte != 1)
    return (false);
  if (ia.registers[REG_IYL][0] >= 0 && I[ia.registers[REG_IYL][0]].byte != 0 ||
      ia.registers[REG_IYH][0] >= 0 && I[ia.registers[REG_IYH][0]].byte != 1)
    return (false);
  ...
  if (result_in_IY &&
      (ic->op == '=' &&
           !(POINTER_SET(ic) && isOperandInDirSpace(IC_RIGHT(ic))) ||
  ...
  if (!result_in_IY && !input_in_IY &&
      ...
      operand_is_pair(IC_RESULT(ic), a, i,
                      G)) // DirSpace access won't use iy here.
    return (true);
...
template <class G_t, class I_t>
bool DEinst_ok(const assignment &a, unsigned short int i, const G_t &G,
               const I_t &I) {
...
template <class G_t, class I_t>
static void set_surviving_regs(const assignment &a, unsigned short int i,
                               const G_t &G, const I_t &I) {
  iCode *ic = G[i].ic;
  ...
  for (v = G[i].alive.begin(), v_end = G[i].alive.end(); v != v_end; ++v) {
    ...
    if (G[i].dying.find(*v) == G[i].dying.end())
      if (!((IC_RESULT(ic) && !POINTER_SET(ic)) && IS_SYMOP(IC_RESULT(ic)) &&
            OP_SYMBOL_CONST(IC_RESULT(ic))->key == I[*v].v))
        ic->rSurv = bitVectSetBit(ic->rSurv, a.global[*v]);
  }
}

template <class G_t, class I_t>
static void assign_operand_for_cost(operand *o, const assignment &a,
                                    unsigned short int i, const G_t &G,
                                    const I_t &I) {
  if (!o || !IS_SYMOP(o))
    return;
...
template <class G_t, class I_t>
static void assign_operands_for_cost(const assignment &a, unsigned short int i,
                                     const G_t &G, const I_t &I) {
  const iCode *ic = G[i].ic;
  ...
  if (ic->op == SEND && ic->builtinSEND)
    assign_operands_for_cost(
        a, (unsigned short)*(adjacent_vertices(i, G).first), G, I);
}

// Cost function.
template <class G_t, class I_t>
static float instruction_cost(const assignment &a, unsigned short int i,
                              const G_t &G, const I_t &I) {
  iCode *ic = G[i].ic;
  float c;
...
template <class I_t> float weird_byte_order(const assignment &a, const I_t &I) {
  float c = 0.0f;
...
// Check for gaps, i.e. higher bytes of a variable being assigned to regs, while
// lower byte are not.
template <class I_t>
...
// For early removal of assignments that cannot be extended to valid
// assignments.
template <class G_t, class I_t>
static bool assignment_hopeless(const assignment &a, unsigned short int i,
                                const G_t &G, const I_t &I,
                                const var_t lastvar) {
...
template <class T_t>
static void get_best_local_assignment_biased(
    assignment &a, typename boost::graph_traits<T_t>::vertex_descriptor t,
    const T_t &T) {
  ...
  for (ai = ai_best = alist.begin(), ai_end = alist.end(); ai != ai_end; ++ai) {
    if (ai->s < ai_best->s) {
      ...
      for (vi = ai->local.begin(), vi_end = ai->local.end(); vi != vi_end; ++vi)
        if (ai->global[*vi] == REG_A ||
            OPTRALLOC_HL &&
                (ai->global[*vi] == REG_H || ai->global[*vi] == REG_L) ||
            OPTRALLOC_IY &&
                (ai->global[*vi] == REG_IYH || ai->global[*vi] == REG_IYL))
          goto too_risky;
      ai_best = ai;
    }
  too_risky:;
  ...
  varset_t newlocal;
  std::set_union(T[t].alive.begin(), T[t].alive.end(), a.local.begin(),
                 a.local.end(), std::inserter(newlocal, newlocal.end()));
  a.local = newlocal;
}

template <class G_t, class I_t>
static float rough_cost_estimate(const assignment &a, unsigned short int i,
                                 const G_t &G, const I_t &I) {
  const i_assignment_t &ia = a.i_assignment;
  float c = 0.0f;
  ...
  for (v = a.local.begin(), v_end = a.local.end(); v != v_end; ++v) {
    ...
    if (a.global[*v] < 0 &&
        IS_REGISTER(
            sym->type)) // When in doubt, try to honour register keyword.
      c += 32.0f;
    if ((I[*v].byte % 2) &&
        (a.global[*v] == REG_L || a.global[*v] == REG_E ||
         a.global[*v] == REG_C ||
         a.global[*v] == REG_IYL)) // Try not to reverse bytes.
      c += 8.0f;
    if (!(I[*v].byte % 2) && I[*v].size > 1 &&
        (a.global[*v] == REG_H || a.global[*v] == REG_D ||
         a.global[*v] == REG_B ||
         a.global[*v] == REG_IYH)) // Try not to reverse bytes.
      c += 8.0f;
    if (I[*v].byte == 0 && I[*v].size > 1 ||
        I[*v].byte == 2 && I[*v].size > 3) {
      if (a.global[*v] == REG_L && a.global[*v + 1] >= 0 &&
          a.global[*v + 1] != REG_H)
        c += 16.0f;
      if (a.global[*v] == REG_E && a.global[*v + 1] >= 0 &&
          a.global[*v + 1] != REG_D)
        c += 16.0f;
      if (a.global[*v] == REG_C && a.global[*v + 1] >= 0 &&
          a.global[*v + 1] != REG_B)
        c += 16.0f;
    } else if (I[*v].byte == 1 || I[*v].byte == 3) {
      if (a.global[*v] == REG_H && a.global[*v - 1] >= 0 &&
          a.global[*v - 1] != REG_L)
        c += 16.0f;
      if (a.global[*v] == REG_D && a.global[*v - 1] >= 0 &&
          a.global[*v - 1] != REG_E)
        c += 16.0f;
      if (a.global[*v] == REG_B && a.global[*v - 1] >= 0 &&
          a.global[*v - 1] != REG_C)
        c += 16.0f;
    }
  }
...
static bool tree_dec_ralloc(T_t &T, G_t &G, const I_t &I, SI_t &SI) {
  ...
  for (unsigned int v = 0; v < boost::num_vertices(I); v++) {
    ...
    if (winner.global[v] >= 0) {
      ...
      sym->nRegs = I[v].size;
      if (USE_OLDSALLOC)
        sym->isspilt = false; // Leave it to Z80RegFix, which can do some
                              // spillocation compaction.
      else
        z80SpillThis(sym);
    }
  }
...
// Omit the frame pointer for functions with low register pressure and few
// parameter accesses.
template <class G_t> static bool omit_frame_ptr(const G_t &G) {
  ...
  for (unsigned int i = 0; i < boost::num_vertices(G); i++) {
    ...
    if (o && IS_SYMOP(o) && OP_SYMBOL_CONST(o)->_isparm &&
        !IS_REGPARM(OP_SYMBOL_CONST(o)->etype))
      omitcost += 6;
    o = IC_LEFT(ic);
    if (o && IS_SYMOP(o) && OP_SYMBOL_CONST(o)->_isparm &&
        !IS_REGPARM(OP_SYMBOL_CONST(o)->etype))
    ...
    if (omitcost >
        14) // Chosen greater than zero, since the peephole optimizer often can
            // optimize the use of iy into use of hl, reducing the cost.
      return (false);
  }
